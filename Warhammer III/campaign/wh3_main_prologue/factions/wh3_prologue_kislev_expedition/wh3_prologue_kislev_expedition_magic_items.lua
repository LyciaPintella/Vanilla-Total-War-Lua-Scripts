...
core:add_listener(
	"award_random_magical_item_prologue",
...
function attempt_to_award_random_magical_item(context)
	if prologue_check_progression["item_generation"] == false or (context:pending_battle():set_piece_battle_key() == "wh3_prologue_ice_witch_rescue" or context:pending_battle():set_piece_battle_key() == "wh3_prologue_brazen_altar" or context:pending_battle():set_piece_battle_key() == "wh3_prologue_howling_citadel") then
		return true
	end 
	...
	if #prologue_initial_loot == 0 then
		...
		new_ancillary_list = prologue_initial_loot
	end
	...
	if roll <= chance then
		...
		while not can_equip and count < 20 do
			...
			if character:can_equip_ancillary(chosen_ancillary) then
				...
				if character:character_subtype("wh3_main_dae_daemon_prince") then
				...
				if #prologue_initial_loot > 0 then
					for k, v in pairs(prologue_initial_loot) do
						if v == chosen_ancillary then
							table.remove(prologue_initial_loot, k)
						end
					end
				end
			end;
	...
	if faction:is_human() then
		common.dev_ui_log_drop_chance(roll, chance, bv_chance, 0, 0);
	end;
end;