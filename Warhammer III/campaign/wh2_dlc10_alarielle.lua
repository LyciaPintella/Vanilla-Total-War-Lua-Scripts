alarielle = {
	power_of_nature_regions = {};
	power_of_nature_vfx = {full = "scripted_effect7", half = "scripted_effect8"};
	power_of_nature_effect_bundle = "wh2_dlc10_power_of_nature",
	power_of_nature_bundle_duration = 10,

	alarielle_faction_key = "wh2_main_hef_avelorn";
	alarielle_subtype_key = "wh2_dlc10_hef_alarielle";
	hellebron_faction_key = "wh2_main_def_har_ganeth";
...
function alarielle:add_alarielle_listeners()
	...
	core:add_listener(
		...
		"CharacterTurnStart",
		function(context)
			return context:character():character_subtype_key() == self.alarielle_subtype_key;
		end,
		function(context)
			...
			if character:has_region() and not character:is_at_sea() then
				...
				if not region:is_abandoned() and region:owning_faction() == character:faction() then
					...
					if region:has_effect_bundle(self.power_of_nature_effect_bundle) then
						cm:remove_effect_bundle_from_region(self.power_of_nature_effect_bundle, region_key);
					end;
					
					cm:apply_effect_bundle_to_region(self.power_of_nature_effect_bundle, region_key, self.power_of_nature_bundle_duration);
					self.power_of_nature_regions[region_key] = self.power_of_nature_bundle_duration;
	...
	core:add_listener(
		...
		"RegionTurnStart",
		function(context)
			return self.power_of_nature_regions[context:region():name()] ~= nil;
		end,
		function(context)
			...
			cm:remove_garrison_residence_vfx(garrison_residence_cqi, self.power_of_nature_vfx.full);
			cm:remove_garrison_residence_vfx(garrison_residence_cqi, self.power_of_nature_vfx.half);
			
			if region:is_abandoned() or region:owning_faction():culture() ~= "wh2_main_hef_high_elves" then
				cm:remove_effect_bundle_from_region(self.power_of_nature_effect_bundle, region_key);
			end
			
			local turns_remaining = self.power_of_nature_regions[region_key];
			turns_remaining = turns_remaining - 1;
			
			if turns_remaining > 5 then
				-- display full VFX
				cm:add_garrison_residence_vfx(garrison_residence_cqi, self.power_of_nature_vfx.full, false);
				self.power_of_nature_regions[region_key] = turns_remaining;
			elseif turns_remaining > 0 then
				-- switch to half strength VFX
				cm:add_garrison_residence_vfx(garrison_residence_cqi, self.power_of_nature_vfx.half, false);
				self.power_of_nature_regions[region_key] = turns_remaining;
			else
				self.power_of_nature_regions[region_key] = nil;
			end
		end,
		true
	);
	...
	cm:add_faction_turn_start_listener_by_name(
		...
		self.alarielle_faction_key,
		function(context)
			local faction = context:faction();
			local alarielle_char = faction:faction_leader(); 
			...
			if faction:has_effect_bundle("wh2_dlc10_bundle_alarielle_corruption_0") then
				cm:remove_effect_bundle("wh2_dlc10_bundle_alarielle_corruption_0", self.alarielle_faction_key);
			end
			...
			local trait_level = alarielle_char:trait_points("wh2_dlc10_trait_alarielle_chaos");
			trait_level = trait_level - alarielle_char:trait_points("wh2_dlc10_trait_alarielle_chaos_none");
			
			local char_str = cm:char_lookup_str(alarielle_char);
			...
			if highest_average_corruption > 75 then
				...
				if trait_level < 3 then
					cm:trigger_incident_with_targets(faction:command_queue_index(), "wh2_dlc10_incident_hef_alarielle_torment_increase", 0, 0, alarielle_char:command_queue_index(), 0, 0, 0);
				end
			elseif highest_average_corruption > 50 then
				...
				elseif trait_level > 2 then
					cm:trigger_incident_with_targets(faction:command_queue_index(), "wh2_dlc10_incident_hef_alarielle_torment_decrease", 0, 0, alarielle_char:command_queue_index(), 0, 0, 0);
				end
	...
	core:add_listener(
		...
		function(context)
			return context:faction():name() == self.alarielle_faction_key;
		end,
		function(context)
			for _, character in model_pairs(context:confederation():character_list()) do
				if character:character_subtype_key() == self.alarielle_subtype_key then
	...
	if cm:get_faction(self.alarielle_faction_key):is_human() then
		-- DEFENDER OF ULTHUAN
		core:add_listener(
			"defender_of_ulthuan_region_update",
			"RegionFactionChangeEvent",
			function(context)
				return self.ulthuan_regions["all"] and self.ulthuan_regions["all"][context:region():name()];
			end,
			function(context)
				local region = context:region();
				...
				if not region:is_null_interface() then
					...
					if ulthuan_type ~= nil then
						if self.ulthuan_regions[ulthuan_type][region_key] and region:is_abandoned() or region:owning_faction():culture() ~= "wh2_main_hef_high_elves" then
							...
							out("\tRegion was true and is now false - Value " .. ulthuan_type .. "_lost count is " .. tostring(self.ulthuan_regions[ulthuan_type .. "_lost"]) .. " (+1)");
						elseif not self.ulthuan_regions[ulthuan_type][region_key] and not region:is_abandoned() and region:owning_faction():culture() == "wh2_main_hef_high_elves" then
							self.ulthuan_regions[ulthuan_type][region_key] = true;
							self.ulthuan_regions[ulthuan_type .. "_lost"] = self.ulthuan_regions[ulthuan_type .. "_lost"] - 1;
							out("\tRegion was false and is now true - Value " .. ulthuan_type .. "_lost count is " .. tostring(self.ulthuan_regions[ulthuan_type .. "_lost"]) .. " (-1)");
						else
							out("\tNo changes made");
						end
						
						self:defender_of_ulthuan_remove_effects();
						
						if self.ulthuan_regions["inner_lost"] > 0 then
							if self.defender_of_ulthuan_effect == "wh2_dlc10_defender_of_ulthuan_all" or self.defender_of_ulthuan_effect == "wh2_dlc10_defender_of_ulthuan_outer" then
								self:defender_of_ulthuan_show_event(region, "inner_lost");
								core:trigger_event("ScriptEventDefenderOfUlthuanInnerLost");
							end
							
							self.defender_of_ulthuan_effect = "wh2_dlc10_defender_of_ulthuan_inner";
						elseif self.ulthuan_regions["outer_lost"] > 0 then
							if self.defender_of_ulthuan_effect == "wh2_dlc10_defender_of_ulthuan_all" then
								self:defender_of_ulthuan_show_event(region, "outer_lost");
								core:trigger_event("ScriptEventDefenderOfUlthuanOuterLost");
							elseif self.defender_of_ulthuan_effect == "wh2_dlc10_defender_of_ulthuan_inner" then
								core:trigger_event("ScriptEventDefenderOfUlthuanInnerRegained");
							end
							
							self.defender_of_ulthuan_effect = "wh2_dlc10_defender_of_ulthuan_outer";
						else
							if self.defender_of_ulthuan_effect == "wh2_dlc10_defender_of_ulthuan_outer" or self.defender_of_ulthuan_effect == "wh2_dlc10_defender_of_ulthuan_inner" then
								self:defender_of_ulthuan_show_event(region, "united");
								core:trigger_event("ScriptEventDefenderOfUlthuanUnited");
								cm:complete_scripted_mission_objective(self.alarielle_faction_key, "wh_main_short_victory", "ulthuan_controlled", true);
							end
							
							self.defender_of_ulthuan_effect = "wh2_dlc10_defender_of_ulthuan_all";
						end
						
						self.defender_of_ulthuan_level = 1;
						cm:apply_effect_bundle(self.defender_of_ulthuan_effect .. "_" .. self.defender_of_ulthuan_level, self.alarielle_faction_key, 0);
					end
				end
			end,
			true
		...
		cm:add_faction_turn_start_listener_by_name(
			...
			self.alarielle_faction_key,
			function()
				if self.defender_of_ulthuan_level < 10 then
					self.defender_of_ulthuan_level = self.defender_of_ulthuan_level + 1;
				end
		...
		if cm:is_new_game() then
			self:defender_of_ulthuan_initialize(true);
			...
			self:defender_of_ulthuan_initialize(false);
		end
	end
end

function alarielle:defender_of_ulthuan_initialize(new_game)
	...
	for i = 1, #ulthuan_types do
		for region_key, value in pairs(self.ulthuan_regions[ulthuan_types[i]]) do
			...
			if region and (region:is_abandoned() or region:owning_faction():culture() ~= "wh2_main_hef_high_elves") then
				self.ulthuan_regions[ulthuan_types[i]][region_key] = false;
				self.ulthuan_regions[ulthuan_types[i].."_lost"] = self.ulthuan_regions[ulthuan_types[i].."_lost"] + 1;
			end
		end
	end
...
function alarielle:defender_of_ulthuan_remove_effects()
	local effects = {"inner", "outer", "all"};
	local faction = cm:get_faction(self.alarielle_faction_key);
	...
	for i = 1, 10 do
		for j = 1, #effects do
			...
			if faction:has_effect_bundle(effect_bundle) then
				cm:remove_effect_bundle(effect_bundle, self.alarielle_faction_key);
			end
		end
	end
end

function alarielle:defender_of_ulthuan_show_event(region, event_type)
...
cm:add_saving_game_callback(
	function(context)
		cm:save_named_value("defender_of_ulthuan_level", alarielle.defender_of_ulthuan_level, context);
		cm:save_named_value("power_of_nature_regions", alarielle.power_of_nature_regions, context);
	end
);

cm:add_loading_game_callback(
	function(context)
		alarielle.defender_of_ulthuan_level = cm:load_named_value("defender_of_ulthuan_level", 1, context);
		alarielle.power_of_nature_regions = cm:load_named_value("power_of_nature_regions", {}, context);
	end
);