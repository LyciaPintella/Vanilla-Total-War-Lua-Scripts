...
function start_camera_advice()
	...
	local camera_marker_general = intro_battle_camera_marker:new(
		...
		-- Your presence on the field brings fear and obedience in those you command, Lord Queek. Your reputation for bloodletting makes you the master of any battle you enter. Wield your awesome power wisely!
		"wh2.battle.intro.023"
	);
	...
	camera_marker_general:action(
		function()
			...
			cam:move_to(camera_pos_start, pos_officer, 0, false, 45) 
			cam:move_to(camera_pos_end, pos_officer, 8, true, 45) 
		end, 
		4500
	);
	...
	local camera_marker_army = intro_battle_camera_marker:new(
		...
		-- The Clanrats are ready for battle, mightiest of Warlords. Each one stands ready to fight, eager to gain your favour.
		"wh2.battle.intro.013"
	);
	...
	camera_marker_army:action(
		function()
			...
			local pos_officer_start = v_offset(start_unit:position(), 0, 0, 0);
			
			local end_unit = sunit_player_03.unit;
			local pos_officer_end = v_offset(end_unit:position(), 0, 0, 0);
			
			local h_camera_bearing = 45;
			local v_camera_bearing = 10;
			...
			cam:move_to(camera_pos_start, pos_officer_start, 0, false, 45);
			cam:move_to(camera_pos_end, pos_officer_end, 40, true, 45);
		end, 
		6000
	);
	...
	camera_marker_hill:action(function() cam:move_to(v(-84.6, 163.2, -368.1), v(-6.5, 163.6, -361.5), 3, false, 50) end, 0);
	
	camera_marker_hill:action(function() cam:move_to(v(-25.0, 163.2, -375.0), v(48.7, 161.1, -348.2), 0, false, 55) end, 4500);
	camera_marker_hill:action(function() cam:move_to(v(-25.0, 163.2, -375.0), v(-61.6, 158.2, -305.8), 40, true, 55) end, 4500);
	...
	local ca = intro_battle_camera_positions_advice:new(
		-- The scent of Elves hangs in this place, Warlord Headtaker. The enemy are surely close, but unseen. Survey the area carefully.
		"wh2.battle.intro.004",
		sunits_player_start,
...
pos_lord_movement = v(-186.5, 119.5, -194.2);
...
function transition_to_army_selection_advice()
	...
	-- play cutscene
	bm:callback(function() play_cutscene_transition_to_army_selection_advice() end, 3000);
...
function start_maneouvring_advice(marker_pos)
	...
	local ma = intro_battle_maneouvring_advice:new(
		-- Excellent! Your army advance in search of the enemy, my Lord. Be sure to practise maneouvring your forces while not under threat of attack.
...
function start_attacking_advice()
	...
	local aa = intro_battle_attacking_advice:new(
		...
		-- Advance against the foolish Elves, Lord Queek! Let them learn why you are called 'Headtaker'!
		"wh2.battle.intro.093",
		sunits_player_start,
		sunits_enemy_start,
...
pos_cam_reposition_player_phase_two = v(267.8, 148.5, -340.1);
pos_targ_reposition_player_phase_two = v(317.8, 132.5, -288.3);
...
function play_enemy_reinforcements_advice()
	...
	-- The 'Elf-things' send more forces to contest the battle, Lord Queek! They pour forward in great numbers!
	bm:queue_advisor("wh2.battle.intro.133");
...
function phase_two_bring_on_player_missiles()
	...
	bm:watch(
		...
		function()
			...
			for i = 1, sunits_player_missile_reinforcements:count() do
				local current_sunit = sunits_player_missile_reinforcements:item(i);				
				current_sunit.uc:goto_location_angle_width(v(755, current_sunit.unit:position():get_z()), 270, current_sunit.unit:ordered_width(), true);
...
function play_missile_reinforcements_advice()
	-- Allied Skaven, my Lord! Conscript them into your service at once. See that some amongst them bear slings and missiles; such weapons will be of great use against the foe.
	bm:queue_advisor("wh2.battle.intro.153");
	bm:add_infotext(
		1,
...
function play_cavalry_reinforcements_advice()
	-- Rat Ogres, my Lord! Hurry them into battle, for they are sorely needed!
	bm:queue_advisor("wh2.battle.intro.191");
	bm:add_infotext(
		1,
		"wh2.battle.intro.info_160",
		"wh2.battle.intro.info_161"
	);
end;
...
function cutscene_player_cavalry_complete()
	...
	-- show ogres objective
	bm:set_objective("wh2.battle.intro.reinforcements.003");
...
function start_mounted_reinforcement_monitors()
	...
	-- watch for ogres closing with enemy
	bm:watch(
		function()
...
function play_cavalry_hurry_up_advice()
	-- Hurry your mounted troops, my Lord! Those in the fight are beginning to tire!
...
function play_cavalry_closing_advice()
	...
	bm:queue_advisor(
		...
		false,
		function()
			bm:complete_objective("wh2.battle.intro.reinforcements.003");
			...
			bm:add_infotext(
				...
				function()
					bm:remove_objective("wh2.battle.intro.reinforcements.003");
...
function player_has_won_phase_two()
	...
	bm:queue_advisor(
		-- The enemy flee the field, Warlord Queek! The clanrats chitter your triumph. The head of the Elf-thing leader is yours to take. Victory!
		"wh2.battle.intro.243"	
	);
	bm:add_infotext(
		1,
		"wh2.battle.intro.info_150",
...
function override_loading_screen()
	...
	if not bool_battle_is_being_replayed then
		...
		common.set_custom_loading_screen_key("wh2_main_skv_clan_mors_12");
	end;
end;
