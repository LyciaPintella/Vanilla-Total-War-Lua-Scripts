...
function play_cutscene_deployment()
	...
	second_battle_start_cutscene_deployment(
		v(468.0, 591.9, 447.6), 							-- end camera position
		v(353.2, 558.5, 319.5), 							-- end camera target
...
function play_battle_attacker_advice()
	...
	second_battle_start_cutscene_attacker(
		v(11.5, 577.1, 29.3), 								-- start camera pos
		v(182.1, 543.5, 52.2),								-- start camera targ
		v(24.7, 577.1, -55.7), 								-- end camera pos
		v(186.0, 545.5, 5.7),								-- end camera targ
		45,													-- cutscene fov
		sunits_player_all,									-- sunits collection of all players army
...
function play_cutscene_forest_deployment()
	...
	second_battle_start_cutscene_forest(
		...
		v(180.9, 542.7, -77.1),
		sunits_player_all,
		-- This forest may be used to conceal your forces. Arrange your Rat Ogres here, my Lord, and they will remain unseen until the enemy draw close.
		"wh2.battle.intro.521",
		{
			1,
...
function monitor_for_player_deployment_in_forest()
	...
	second_battle_monitor_for_player_deployment_in_forest(
		sunits_player_ogres,																-- sunits to go in forest
		sunits_player_non_ogres,															-- sunits not to go in forest
		sunits_player_all,																	-- all player sunits
		"wh2.battle.intro.forest_deployment.002",											-- objective key
		...
		v(80, 60, 30),																		-- position offset for completion camera
...
function play_cutscene_hill_deployment()
	...
	second_battle_start_hill_deployment(
		v(-25.3, 593.6, 14.8),										-- cutscene start cam
		v(66.7, 545.8, 156.3), 										-- cutscene start targ
		v(276.3, 603.3, 43.3), 										-- cutscene end cam
		v(110.0, 549.9, 58.8), 										-- cutscene end targ
		...
		sunits_player_ogres,										-- player forest sunits
		sunits_player_non_ogres,									-- player non-forest sunits
		area_hill_deployment,										-- convex deployment area
		v(150, 562, 150),											-- marker position
...
function start_battle()
	...
	-- advice
	-- To arms, then! The enemy enter the field. Scamper forward and claim the hilltop, Lord Headtaker. Its slopes will serve your packs well. Keep your Rat Ogres hidden!
	bm:callback(
		function() 
			bm:queue_advisor("wh2.battle.intro.571");
		end, 
		1000
	);
	...
	bm:watch(
		function()
			return not sunits_player_ogres:is_hidden(true)
		end,
		0,
		function()
...
function play_artillery_advice()
	...
	cutscene_artillery:action(
		function()
			...
			local pos_centre = v_offset(sunits_player_ogres:centre_point(), 0, 30, 0);
			...
			-- Employ your hidden forces, my Lord. Advance them through the forest and they will be able to quickly surprise and overwhelm the enemy war machines!
