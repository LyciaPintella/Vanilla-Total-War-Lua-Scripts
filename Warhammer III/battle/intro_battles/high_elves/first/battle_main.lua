...
function start_camera_advice()
	...
	local camera_marker_general = intro_battle_camera_marker:new(
		...
		-- Your presence on the field inspires the elves you command, Lord Tyrion. Under your command they advance with great confidence into battle. Wield your influence wisely!
		"wh2.battle.intro.020"
	);
	...
	local camera_marker_army = intro_battle_camera_marker:new(
		...
		-- Your Elves stand ready for battle, Lord Tyrion. Each one has sworn an oath to drive the Druchii from Ulthuan, or to die trying.
		"wh2.battle.intro.010"
	);
	...
	camera_marker_army:action(
		function()
			cam:move_to(v(-189.8, 125.7, -288.6), v(-258.1, 115.1, -252.0), 0, false, 30);
			cam:move_to(v(-188.7, 124.1, -276.7), v(-257.3, 116.7, -239.8), 70, false, 30);
		end, 
		5000
	);
	...
	local ca = intro_battle_camera_positions_advice:new(
		-- The Druchii were last sighted close to here, Lord Tyrion. Survey the area for yourself.
		"wh2.battle.intro.001",
		sunits_player_start,
...
function start_general_movement_advice()
	...
	local ma = intro_battle_movement_advice:new(
		...
		function() transition_to_army_selection_advice() end
	);
...
function start_attacking_advice()
	...
	local aa = intro_battle_attacking_advice:new(
		...
		-- The Druchii intend to give battle, Lord Tyrion. Send your elves in to attack! Show the enemy no mercy!
		"wh2.battle.intro.090",
		sunits_player_start,
		sunits_enemy_start,
...
pos_cam_reposition_player_phase_two = v(275.8, 136.7, -334.6);
pos_targ_reposition_player_phase_two = v(326.1, 120.7, -283.1);
...
function opening_battle_completed()
	...
	bm:callback(
		function()
			...
			sunits_enemy_phase_two:set_always_visible(true);
...
function play_enemy_reinforcements_advice()
	...
	-- The Druchii re-enter the field, Lord Tyrion, and in great number! Your elves are brave, but this shall be a stern test.
	bm:queue_advisor("wh2.battle.intro.130");
...
function start_phase_two()
	...
	ma:set_show_advisor_close_button(true);
...
function play_missile_reinforcements_advice()
	-- Allied forces approach, Lord Tyrion! The bows they carry will prove useful against the Druchii. Enlist them into your service, and deploy them against the enemy.
	bm:queue_advisor("wh2.battle.intro.150");
	bm:add_infotext(
		1,
...
function start_mounted_reinforcement_monitors()
	...
	bm:watch(
		function()
			return distance_between_forces(sunits_player_cavalry_reinforcements, sunits_enemy_phase_two) < 180
		end,
		0,
		function()
...
function play_cavalry_closing_advice()
	...
	bm:stop_advisor_queue(true);
	bm:queue_advisor(
		-- Your mounted warriors approach the battle. Have them charge the enemy rear! The impact alone may break their courage to fight!
...
function player_has_won_phase_two()
	...
	bm:queue_advisor(
		-- The Druchii are driven from the field, Lord Tyrion! The war to secure Ulthuan continues, but this day belongs to the Asur! Victory!
		"wh2.battle.intro.240"	
	);
	bm:add_infotext(
		1,
		"wh2.battle.intro.info_150",
...
function override_loading_screen()
	...
	if not bool_battle_is_being_replayed then
		...
		common.set_custom_loading_screen_key("wh2_main_hef_eataine_12");
	end;
end;