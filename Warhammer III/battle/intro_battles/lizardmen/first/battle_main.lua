...
function start_camera_advice()
	...
	local camera_marker_general = intro_battle_camera_marker:new(
		...
		-- Your presence on the field inspires those you command, Lord Mazdamundi. Under your wise instruction, they advance against the enemy with confidence. Wield your awesome power wisely!
		"wh2.battle.intro.022"
	);
	...
	camera_marker_general:action(
		function()
			cam:move_to(v(-196.2, 130.5, -170.1), v(-252.1, 119.2, -124.4), 2.5, false, 50);
		end, 
		0
	);
	...
	camera_marker_general:action(
		function()
			...
			local pos_officer = v_offset(unit:position(), 0, 3.5, 0);
		
			local h_camera_bearing_start = 10;
			local h_camera_bearing_end = -10;
			local v_camera_bearing_start = -2;
			local v_camera_bearing_end = -2;
	...
	camera_marker_general:action(
		function()
			...
			local camera_distance_start = 10;
			...
			cam:move_to(camera_pos_start, pos_officer, 0, false, 50) 
			cam:move_to(camera_pos_end, pos_officer, 40, false, 50) 
		end, 
		10000
	);
	...
	local camera_marker_army = intro_battle_camera_marker:new(
		...
		-- Your fearsome cohorts line up ready for battle, revered Lord. They await only your command to advance.
		"wh2.battle.intro.012"
	);
	...
	camera_marker_army:action(function() cam:move_to(v(-178.3, 128.6, -268.3), v(-225.2, 114.6, -214.0), 2.5, false, 0) end, 0);
	
	camera_marker_army:action(
		function()		
			cam:move_to(v(-185.2, 123.8, -263.2), v(-239.8, 114.3, -215.6), 0, false, 25);
			cam:move_to(v(-181.6, 121.5, -233.2), v(-236.2, 112.1, -185.6), 50, true, 25);
		end, 
		6000
	);
	...
	local ca = intro_battle_camera_positions_advice:new(
		-- The scent of warmbloods drifts in the air, Lord Mazdamundi. The enemy are surely close, but unseen. Survey the area yourself.
		"wh2.battle.intro.003",
		sunits_player_start,
...
function start_attacking_advice()
	...
	local aa = intro_battle_attacking_advice:new(
		...
		-- Attack, my Lord Mazdamundi! Attack! Drive the barbarous warmbloods from your sacred jungles!
		"wh2.battle.intro.092",
		sunits_player_start,
		sunits_enemy_start,
...
pos_cam_reposition_player_phase_two = v(256.9, 148.7, -349.6);
pos_targ_reposition_player_phase_two = v(307.6, 132.7, -298.4);
...
function play_enemy_reinforcements_advice()
	...
	-- Look, Lord Mazdamundi! More Norse invaders advance against you. They come forward in great numbers!
	bm:queue_advisor("wh2.battle.intro.132");
...
function play_missile_reinforcements_advice()
	-- A party of Skinks, my Lord! Press these allies into your service. Some amongst their number carry javelins to throw; these will prove most useful against the enemy.
	bm:queue_advisor("wh2.battle.intro.152");
	bm:add_infotext(
		1,
...
function player_has_won_phase_two()
	...
	bm:queue_advisor(
		-- The courage of the Norse trespassers has failed them - they flee for their lives! Victory is yours, eternal Lord!
		"wh2.battle.intro.242"	
	);
	bm:add_infotext(
		1,
		"wh2.battle.intro.info_150",
...
function override_loading_screen()
	...
	if not bool_battle_is_being_replayed then
		...
		common.set_custom_loading_screen_key("wh2_main_lzd_hexoatl_12");
	end;
end;
