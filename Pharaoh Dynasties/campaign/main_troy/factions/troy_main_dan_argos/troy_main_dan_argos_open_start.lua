...
local early_game_mission_overrides = {
	enemy_faction_key = "troy_main_dan_corinthians",
	initial_quest_key = "troy_main_diomedes_quest_stage_0",

	-- Diomedes doesn't have an initial quest currently, so bypass it
	suppress_initial_quest = true,
	defeat_enemy_army_trigger_event = "ScriptEventHowTheyPlayEventDismissed",
	...
	gain_dominance_amount = cm:model():get_campaign_variable_by_name("diplomatic_action_dominance_cost"),
}; 
...
start_early_game_shared_missions(
	...
	"troy_adi_early_game_dan_diomedes",				-- substr that is used to form early game mission keys, when concatenated with strings like "_defeat_enemy_army_1" or "_recruit_full_army_1"
...
start_early_game_missions_diomedes(
	cm:get_local_faction_name(),							-- key of local player faction
	"troy_adi_early_game_dan_diomedes",				-- substr that is used to form early game mission keys, when concatenated with strings like "_defeat_enemy_army_1" or "_recruit_full_army_1"
	early_game_mission_overrides					-- table of overrides
);