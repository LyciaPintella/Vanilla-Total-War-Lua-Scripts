...
local function faction_achieved_victory(context)
	...
	if is_table(achievements.win_coop_game) then
		for achievement_key, achievement_data in pairs(achievements.win_coop_game) do
			...
			for _, it_faction_name in ipairs(human_factions) do
				if cm:model():are_factions_coop_allies(my_faction_name, it_faction_name) and
				   ((my_faction_name == achievement_data.first_faction and it_faction_name == achievement_data.second_faction) or
				   (my_faction_name == achievement_data.second_faction and it_faction_name == achievement_data.first_faction))
				then
					achievement_unlock(faction, achievement_key)
				end
			end
		...
		for achievement_key, achievement_data in pairs(achievements.bad_attitude_from_their_whole_subculture) do
			if faction:name() == achievement_data.play_with_faction then
				...
				for i = 0, other_factions_list:num_items() - 1 do
					...
					if not other_faction:is_excluded_from_cai_diplomacy() and not other_faction:is_dead() then
	...
	if is_table(achievements.have_provinces) then
		for achievement_key, achievement_data in pairs(achievements.have_provinces) do
			if faction:name() == achievement_data.play_with_faction and faction:num_provinces() >= achievement_data.count then
				...
				local provinces_with_minimum_workforce = 0

				for i = 0, province_list:num_items() - 1 do
					local accumulated_workforce = province_list:item_at(i):development_points_for_faction(faction:name())
					if accumulated_workforce >= achievement_data.minimum_workforce then
						provinces_with_minimum_workforce = provinces_with_minimum_workforce + 1
					end
				end

				if provinces_with_minimum_workforce >= achievement_data.count then
					achievement_unlock(faction, achievement_key)
				end
			end
		end
...
local function character_razed_settlement_event(context)
	...
	local get_achievement_progress_keys_from_achievement_data = function(achievement_data)
		if achievement_data.version == 0 then
			return achievement_data.slot_template_key_prefixes
		else
			local progress_keys = {}
			for _, slot_template_key_prefix in ipairs(achievement_data.slot_template_key_prefixes) do
				local achievement_progress_key = "raze_settlements;" .. slot_template_key_prefix
				...
				table.insert(progress_keys, achievement_progress_key)
			end
	...
	for _, achievement_data in pairs(achievements.raze_settlements) do
		local progress_keys = get_achievement_progress_keys_from_achievement_data(achievement_data)
		for _, progress_key in ipairs(progress_keys) do
			table_add_unique(achievement_progress_keys, progress_key)
		end
	end
	...
	for achievement_key, achievement_data in pairs(achievements.raze_settlements) do
		if is_table(achievement_data.faction) and table_contains(achievement_data.faction, faction:name()) then
			local achievement_progress_keys = get_achievement_progress_keys_from_achievement_data(achievement_data)
			local count = 0
			for _, progress_key in ipairs(achievement_progress_keys) do
				count = count + track_achievement_progress(faction, progress_key, false)
			end
