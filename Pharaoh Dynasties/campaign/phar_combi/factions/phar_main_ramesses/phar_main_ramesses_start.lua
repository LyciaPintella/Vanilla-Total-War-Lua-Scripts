out("phar_main_ramesses_start.lua LOADED")
...
if not cm:is_multiplayer() then
	...
	cm:add_linear_sequence_configuration(
		"tutorial_start",                  							-- script name, must be unique and have no spaces
		"_tutorial_start",                         					-- script to load if this loading config selected
		"sbool_player_selected_first_turn_intro_on_frontend",       -- svr boolean that can force this loading config (will be set to false if successful)
		false,                                      				-- load this config if no other config forced
		"SCRIPTED_TWEAKER_01"                   					-- tweaker to force this loading config
	);
