...
cam_start_x = 524.36712646484;
cam_start_y = 842.65515136719;
	...
	function()
		...
		if not random_start_pos_enabled then 
			...
			core:progress_on_loading_screen_dismissed(
				function()
					cm:position_camera_at_primary_military_force("phar_main_bay")
				end
			);
		end
	end
...
function play_cutscene_intro()
	...
	advice_to_play[1] = "phar_cam_faction_intro_merneptah_bay_01" --#1#
	advice_to_play[2] = "phar_cam_faction_intro_merneptah_bay_02" --#19.3#
	advice_to_play[3] = "phar_cam_faction_intro_merneptah_bay_03" --#36.5#
	advice_to_play[4] = "phar_cam_faction_intro_merneptah_bay_04" --#48#

	local cutscene_intro = campaign_cutscene:new(
		"intro_cutscene",									-- string name for this cutscene
		57,												-- length of cutscene in seconds
		function()
	...
	cutscene_intro:action(
		function()
			...
			cutscene_intro:cindy_playback(string.format("script/campaign/phar_combi/factions/phar_camp_flybys/scenes/phar_camp_flyby_bay_r01_s%s.CindyScene", id), 0, 2);
		end,
		0
	)
	...
	cutscene_intro:action(
		function()
			cm:characters_forced_invisible({bay_forename_id}, true);
