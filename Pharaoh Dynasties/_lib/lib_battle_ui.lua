...
-- set battle_ui_manager to be of custom type TYPE_BATTLE_UI_MANAGER
set_class_custom_type_and_tostring(battle_ui_manager, TYPE_BATTLE_UI_MANAGER);

local esc_menu_battle_name = "esc_menu_battle";
...
function battle_ui_manager:new(bm)
	...
	-- set 'ui' to derive from self ('battle_ui_manager')
...
function battle_ui_manager:start_listeners()
	...
	core:add_listener(
		...
		function(context)
			...
			-- Hide/remove certain scripted elements when the ESC menu is shown
			if panel == esc_menu_battle_name then
...
function battle_ui_manager:is_esc_menu_open()
	return self:is_panel_open(esc_menu_battle_name);
end;
...
--- @desc Deselect all unit cards
function battle_ui_manager:deselect_all()
	local battle_hud = core:get_ui_root():SequentialFind("hud_battle")

	if battle_hud then
		battle_hud:InterfaceFunction("deselect_all")
	end
end;

--- @function enable_selection_change
--- @desc Enable or disable selection change
--- @p [opt=true] boolean should_enable selection change
function battle_ui_manager:enable_selection_change(should_enable)
	should_enable = should_enable == nil and true or should_enable
	local battle_hud = core:get_ui_root():SequentialFind("hud_battle")

	if battle_hud then
		battle_hud:InterfaceFunction("enable_selection_change", should_enable)
	end
end;
...
--- @section Unit Cards
----------------------------------------------------------------------------
--- @function is_unit_in_ui_group
--- @desc Check if the specified unit is in UI group (groups that are not locked)
--- @p number unit_unique_ui_id
function battle_ui_manager:is_unit_in_ui_group(unit_unique_ui_id)
	local battle_hud = core:get_ui_root():SequentialFind("hud_battle")

	if battle_hud then
		return battle_hud:InterfaceFunction("is_unit_in_ui_group", unit_unique_ui_id)
	end
