...
intervention_manager = {
	...
	interventions_completed_this_sequence = {},		-- list of interventions that were triggered in the current sequence
	]]
	intervention_currently_active = false,
	intervention_currently_pending = false,
	pending_intervention_has_reduced_pause = false,
	start_next_intervention_waiting = false,
	...
	camera_cache_position_set = false,
	should_cache_camera_position_in_scroll_camera_for_intervention = false			-- should scroll_camera_for_intervention() cache camera position when internal cutscene is completed, so it can potentially be restored when intervention sequence completes
};


-- set intervention_manager to be of custom type TYPE_INTERVENTION_MANAGER
...
function intervention_manager:new()
	...
	-- set 'im' to derive from self ('intervention_manager')
	...
	im.interventions_completed_this_sequence = {};
...
-- returns the active intevention, or false if none is active
function intervention_manager:get_active_intervention()
...
function intervention_manager:sort_interventions()
	...
	local fullscreen_panel_open = uim:get_open_fullscreen_or_event_panel();
...
function intervention_manager:event_triggered(event_name, context)
	...
	if intervention_listeners then
		...
		for i = 1, #copied_intervention_list do
			...
			elseif not current_intervention:passes_precondition_check_for_event_triggered() then			
...
--		we've been waiting for a dilemma choice to be made or a fullscreen panel to be closed (if a battle or end turn sequence ends then these should still wait).
...
function intervention_manager:start_next_intervention(force_no_wait)
	...
	if self.intervention_currently_active and self.intervention_currently_active.is_active then
		if self.intervention_system_debug then
			...
			self:out(separator_str);
			self:out(separator_str);
			self:out("intervention_manager:start_next_intervention() is not starting next intervention as intervention with name [" .. self.intervention_currently_active.name .. "] is currently active");
			self:out(separator_str);
			self:out(separator_str);
		end;
		return;
	...
	--
	-- Do not proceed if a previous call to start_next_intervention() is waiting for something - either an autosave or loading screen
	--

	if self.start_next_intervention_waiting then
		if self.intervention_system_debug then
			self:out("");
			self:out(separator_str);
			self:out(separator_str);
			self:out("intervention_manager:start_next_intervention() is not proceeding as a previous call to this function is waiting for something");
			self:out(separator_str);
			self:out(separator_str);
		end;
		return;
	...
	--
	-- if we are waiting for a turn-start autosave, and saving has not been disabled by script, then we must wait for the autosave to complete
	--

	if cm:model():is_start_turn_autosave_pending() and not cm:is_saving_disabled() then
		if self.intervention_system_debug then
			self:out("an autosave is pending, waiting for it to complete");
			self:dec_tab();
			self:out(separator_str);
			self:out(separator_str);
		end;
		self.start_next_intervention_waiting = true;
		core:add_listener(
			"intervention_system_waiting_for_autosave_completion",
			"TurnStartAutosaveFinished",
			true,
			function()
				self.start_next_intervention_waiting = false;
				self:start_next_intervention(force_no_wait);
			end,
			false
		);
		return;
	...
	if force_no_wait then
		-- force_no_wait flag is set - we have either been waiting for a UI or game event to finish (fullscreen panel, battle sequence etc) or another intervention has just completed
		if self.intervention_system_debug then
	...
	if (next_intervention.wait_for_dilemma and CampaignUI.DoesEventTypeRequireResponse("faction_event_dilemmaevent_feed_target_dilemma_faction"))
		or (self.intervention_currently_active and self.intervention_currently_active.is_active)
		and not string.find(self.intervention_currently_active.name, "advice")
	then 
		if self.intervention_system_debug then
		...
		core:add_listener(
			...
			true, -- no need for filter as this is a one time listener on the current dilemma(it must be present, if we are here)
			function(context)
	...
	if next_intervention.wait_for_fullscreen_panel_dismissed then
		local open_fullscreen_panel = uim:get_open_fullscreen_or_event_panel();
		
		if open_fullscreen_panel then
			local should_postpone_intervention
			if open_fullscreen_panel ~= "events" then
				-- any other fullscreen panel blocks interventions
				should_postpone_intervention = true
			else
				local open_panel_component = core:get_ui_root():SequentialFind(open_fullscreen_panel)
				if open_panel_component then
					should_postpone_intervention = open_panel_component:InterfaceFunction("RequiresResponse")
				end
			end

			if should_postpone_intervention then
				if self.intervention_system_debug then
					self:out("a fullscreen panel [" .. open_fullscreen_panel .. "] is showing and the next intervention [" .. next_intervention.name .. "] has to wait. Starting progress_on_fullscreen_panel_dismissed() listener and exiting.");
					self:dec_tab();
					self:out(separator_str);
					self:out(separator_str);
				end;
				
				-- listen for the fullscreen panel being closed
				cm:progress_on_fullscreen_panel_dismissed(
					function()
						if self.intervention_system_debug then
							self:out("");
							self:out(separator_str);
							self:out("start_next_intervention() :: fullscreen panel closed, re-calling start_next_intervention()");
							self:out(separator_str);
						end;
						self:start_next_intervention(true);			-- trigger the next intervention immediately
					end,
...
function intervention_manager:start_context_change_monitor()
	...
	local function context_changed(reason)
		if self.intervention_system_debug then
			...
			if #self.current_eligible_interventions == 1 then
				self:out("\tclosing and restarting 1 pending intervention:");
				self:out("\t\t" .. self.current_eligible_interventions[1].name);
			else
				self:out("\tclosing and restarting " .. tostring(#self.current_eligible_interventions) .. " pending interventions:");
		...
		self:stop_context_change_monitor();
		
		-- clear any eligible interventions that haven't yet been triggered
		self:clear_enqueued_interventions();

		-- complete the current intervention
		--[[
		if self.intervention_currently_active then
			self.intervention_currently_active:complete();
		else
			script_error("WARNING: start_context_change_monitor() is responding to a change of context but no intervention is currently active to complete - is this valid?");
		end;

		-- un-highlight the advisor close button
	...
	for i = 0, character_list:num_items() - 1 do
		...
		cm:notify_on_character_movement(
			...
			function()
				context_changed("player has moved character with cqi " .. current_character_cqi);
			end
		);
	end;
end;
...
function intervention_manager:stop_context_change_monitor()
...
function intervention_manager:lock_ui(value)
	...
	if value then
		if not self.has_locked_ui then
			if self.intervention_system_debug then
				self:out("intervention manager is locking the ui");
			end;
		...
		if self.has_locked_ui then
			if self.intervention_system_debug then
				self:out("intervention manager is unlocking the ui");
			end;
...
function intervention_manager:intervention_completed(intervention)
	...
	table.insert(self.interventions_completed_this_sequence, intervention);
	self.intervention_currently_active = false
...
function intervention_manager:all_interventions_completed()
	...
	-- Trigger a script event to indicate that a sequence of interventions has completed
	-- Attach a copy of the table that lists interventions completed in this sequence, and clear that table
	local interventions_completed_this_sequence_copy = table.copy(self.interventions_completed_this_sequence);
	self.interventions_completed_this_sequence = {};
	core:trigger_custom_event("ScriptEventInterventionSequenceCompleted", {intervention_list = interventions_completed_this_sequence_copy});
end;
...
function intervention_manager:state_from_string(str)
	...
	out("* intervention_manager:state_from_string() called");
	self:out("");
...
--
---	@class intervention Interventions
...
intervention = {
	...
	is_debug = false,
	min_advice_level = 0,									-- minimum advice level at which this intervention is allowed to play: 0 = minimal, 1 = low, 2 = high. Keep the default to minimal
...
-- set intervention to be of custom type TYPE_INTERVENTION, with a custom tostring() response
...
function intervention:new(name, cost, callback, is_debug, is_transient)
	...
	-- set 'i' to derive from self ('intervention')
...
--- @desc Multiple preconditions may be added to an intervention. The optional precondition name is used to show debug output if the precondition fails.
--- @p @function precondition callback
--- @p [opt="<untitled>"] @string precondition name
function intervention:add_precondition(precondition_callback, precondition_name)
	if not is_function(precondition_callback) then
		script_error(self.name .. " ERROR: add_precondition() called but supplied precondition [" .. tostring(precondition_callback) .. "] is not a function");
		return false;
	end;

	if precondition_name and not validate.is_string(precondition_name) then
		return false;
	end;

	local precondition_record = {
		callback = precondition_callback,
		name = precondition_name or "<untitled>"
	}
	
	table.insert(self.precondition_list, precondition_record);
end;
...
function intervention:add_advice_key_precondition(advice_key)
	...
	self:add_precondition(
		...
		"advice with key " .. advice_key .. " must not have been shown before"
	);
end;
...
function intervention:add_precondition_unvisited_page(page_name)
	...
	self:add_precondition(
		...
		"help page " .. page_name .. " must be unvisited"
	);
end;
...
function intervention:add_player_faction_precondition(faction_key)
	...
	self:add_precondition(
		...
		"local player faction's key must be " .. faction_key
	);
end;
...
function intervention:add_player_not_faction_precondition(faction_key)
	...
	self:add_precondition(
		...
		"local player faction's key must not be " .. faction_key
	);
end;
...
function intervention:add_player_subculture_precondition(subculture)
	...
	self:add_precondition(
		...
		"local player faction's subculture must be " .. subculture
	);
end;
...
function intervention:add_player_not_subculture_precondition(subculture)
	...
	self:add_precondition(
		...
		"local player faction's subculture must not be " .. subculture
	);
end;
...
function intervention:start(is_restarting, starting_from_other_intervention)
	...
	local passes_precondition_check, failed_precondition_name = self:passes_precondition_check();
	if not passes_precondition_check then
		self.im:out(self.name .. " not starting as check failed for precondition with name [" .. failed_precondition_name .. "]");
...
function intervention:passes_precondition_check()
	for i = 1, #self.precondition_list do
		if not self.precondition_list[i].callback() then
			return false, self.precondition_list[i].name;
		end;
	end;
...
function intervention:passes_precondition_check_for_event_triggered()
	local passes_precondition_check, failed_precondition_name = self:passes_precondition_check();
	if not passes_precondition_check then
		if self.intervention_system_debug then
			self:out("\t\tintervention has failed precondition check with name [" .. failed_precondition_name .. "], stopping it");
		end;
	end;

	return passes_precondition_check;
end;
...
function intervention:should_add_grace_pause_at_start()
	...
	return self.im:first_intervention_in_sequence() and not (
		...
		(uim:get_open_fullscreen_or_event_panel() and not self.wait_for_fullscreen_panel_dismissed)	-- fullscreen panel is open and we don't have to wait for it
	);
end;
...
function intervention:scroll_camera_to_character_for_intervention(char_cqi, advice_key, infotext, mm, duration, scroll_complete_callback, continuation_callback)
	...
	if not is_number(char_cqi) then
		...
		self:play_advice_for_intervention(advice_key, infotext, mm, continuation_callback);
		return false;
	end;
...
function intervention:scroll_camera_for_intervention_action(targ_region, targ_x, targ_y, advice_key, infotext, mm, duration, scroll_complete_callback, continuation_callback)
		...
		function() 
			cm:callback(
				function()
					...
					if self.im.should_cache_camera_position_in_scroll_camera_for_intervention then
	...
	if not cm:get_active_cutscene() then
		...
		cm:repeat_callback(
			function()
				if not cm:get_active_cutscene() then
					cm:remove_callback("callback_intervention_wait_for_camera_scroll")
					cutscene:start();
				end
			end, 
			0.1, 
			"callback_intervention_wait_for_camera_scroll"
		)
	end	
end;
...
function intervention:play_advice_for_intervention(advice_key, infotext, mm, mission_delay)
	
	if not is_string(advice_key) then
		script_error("play_advice_for_intervention() called but supplied advice key [" .. tostring(advice_key) .. "] is not a string");
		return false;
	end;
	
	if infotext and not is_table(infotext) then
		script_error("play_advice_for_intervention() called but supplied infotext list [" .. tostring(infotext) .. "] is not a table or nil");
		return false;
	end;
	
	if mm and not is_missionmanager(mm) then
		script_error("play_advice_for_intervention() called but supplied mission manager [" .. tostring(mm) .. "] is not a mission manager");
		return false;
	end;
	...
	if mission_delay and (not is_number(mission_delay) or mission_delay < 0) then
		script_error("play_advice_for_intervention() called but supplied mission delay [" .. tostring(mission_delay) .. "] is not a positive number");
		return false;
	end;
	...
	if self:should_add_grace_pause_at_start() then
		out.interventions(intervention_stamp .. "waiting " .. wait_period .. "s as this is the first intervention in the sequence");
		cm:callback(function() self:play_advice_for_intervention_action(advice_key, infotext, mm, mission_delay) end, wait_period);
	else
		self:play_advice_for_intervention_action(advice_key, infotext, mm, mission_delay);
	end;
end;


function intervention:play_advice_for_intervention_action(advice_key, infotext, mm, mission_delay)
