...
--- @p [opt=nil] callback to use when generating random numbers, this is a must when using the function in the model, otherwise the game will desync
--- @return vector target position
function get_position_near_target(pos, min_dist, max_dist, min_angle, max_angle, generate_random_number_callback)
	...
	local dist = 0
	local angle = 0
	if generate_random_number_callback then
		dist = generate_random_number_callback(min_dist, max_dist)
		angle = generate_random_number_callback(min_angle, max_angle)
	else
		dist = math.random(min_dist, max_dist);
		angle = math.random(min_angle, max_angle);
	end
...
--	CONVEX AREA
--
--- @class convex_area Convex Areas
...
-- set convex_area to be of custom type TYPE_CONVEX_AREA
...
function convex_area:new(point_list)	
	...
	-- set 'ca' to derive from self ('convex_area')
