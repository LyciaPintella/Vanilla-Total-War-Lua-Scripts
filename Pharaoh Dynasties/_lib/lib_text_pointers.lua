...
text_pointer = {
    ...
    highlight_close_button_focusing_highlight = false,
...
-- set text_pointer to be of custom type TYPE_TEXT_POINTER, with a custom tostring() response
...
function text_pointer:new_from_component(name, pointer_display_mode, pointer_length, uicomponent_obj, x_proportion, y_proportion)
    ...
    if is_uicomponent(uicomponent_obj) then
		if not uicomponent_obj:IsValid() then
			script_error("ERROR: new_from_component() called but supplied uicomponent object [" .. tostring(uicomponent_obj) .. "] is not a valid uicomponent")
			return false
		end
    ...
    elseif is_function(uicomponent_obj) then
        ...
        uicomponent_callback = function()
            local uic, suppress_error = uicomponent_obj();
            
            if not is_uicomponent(uic) or not uic:IsValid() then
				if not to_bool(suppress_error) then
                	script_error(tostring(name) .. " ERROR: text_pointer attempting to determine uicomponent to point at from supplied callback but returned value [" .. tostring(uic) .. "] is not a uicomponent");
				end
                return false;
            end;
...
function text_pointer:new_action(name, pointer_display_mode, pointer_length, pointed_pos_callback)
	...
	-- set 'tp' to derive from self ('text_pointer')
...
--- @p @number seconds
function text_pointer:set_auto_hide_time(seconds)
    if not is_number(seconds) then
        return
    end

    self.auto_hide_time = seconds
end
...
--- @p [opt=false] @boolean focusing highlight, Set to <code>true</code> to start from a bigger highlight which gradually resizes to the dimensions of the close button
--- @p_long_desc This is disregarded if the text pointer is already showing at the time this function is called.
function text_pointer:set_highlight_close_button(wait_offset, focusing_highlight)
    focusing_highlight = not not focusing_highlight
    self.highlight_close_button_focusing_highlight = focusing_highlight
...
function text_pointer:show(force_display)
	...
	if not (is_number(pointed_pos_x) and is_number(pointed_pos_y)) then
		return false
	end
    ...
    for i = 1, #component_overrides do
        ...
        if pointer_display_mode == "worldspace" then
            localised_text = parser:parse_for_no_links(common.get_localised_string(override_text));
        else
            localised_text = parser:parse_for_tooltips(common.get_localised_string(override_text));
        end;
    ...
    -- Setup closing if auto hide is set
    self.uic_text_label:InterfaceFunction("set_auto_hide_time", self.auto_hide_time)
    if self.auto_hide_time > 0 then

        local auto_hide_callback = function()
            core:remove_listener(self.full_name .. "_auto_hide_listener");
            self.registered_listeners[self.full_name .. "_auto_hide_listener"] = nil
            ...
            if self.close_button_callback then
                if self.close_button_callback_delay == 0 then
                    ...
                    tm:callback(function() self.close_button_callback() end, self.close_button_callback_delay);
                end;
            end;
        end;

        core:add_listener(
            self.full_name .. "_auto_hide_listener",
            "ContextTriggerEvent",
            function(context)
                local params = context.string:split(":")
                return params[1] == "ScriptedTextPointerCallbackAutoHide" and
                    params[2] == self.uic_text_label:Id()
            end,
            auto_hide_callback,
            true
        )
        self.registered_listeners[self.full_name .. "_auto_hide_listener"] = true
    end
    ...
    if self.show_close_button then
        ...
        if uic_close_button then
            ...
            local close_callback = function()
                ...
                self.registered_listeners[self.full_name .. "_close_button_clicked"] = nil
            ...
            core:add_listener(
                ...
                function(context)
					...
					if uic:IsValid() then
						...
						if uic_parent:IsValid() then
							local uic_gparent = UIComponent(uic_parent:Parent());
							return uic_gparent:IsValid() and uic_gparent == self.uic_text_label;
						end;
					end;
                end,
                function(context)
            ...
            self.registered_listeners[self.full_name .. "_close_button_clicked"] = true
            for k, v in pairs(self.registered_listeners) do
                out("registered_listeners key: " .. k)
            end
            -- cm:steal_escape_key_with_callback(
            --     self.full_name .. "text_pointer_steal",
            --     function()
            --         if self.close_button_callback then
            --             core:remove_listener(self.full_name .. "_close_button_clicked")
            --             cm:release_escape_key_with_callback(self.full_name .. "text_pointer_steal")
            --         end
            --     end
            -- );
    ...
    if uic_docker then
        out.design("\tpositioning docker in worldspace at [" .. tostring(pointed_pos_x) .. ", " .. tostring(self.worldspace_display_height) .. ", " .. tostring(pointed_pos_y) .. "]");
...
function text_pointer:highlight_close_button(value)
    ...
    if value and not self.close_button_is_highlighted then
        ...
        uic_close_button:Highlight(true, false, self.component_priority + 100, self.highlight_close_button_focusing_highlight);
    ...
    if not value and self.close_button_is_highlighted then
        ...
        uic_close_button:Highlight(false, false, self.component_priority + 100, self.highlight_close_button_focusing_highlight);
        ...
        if override_record.is_streaming then
            ...
            if bookmark + chars_per_update > override_record.localised_text_len then
                ...
                if bookmark_floor <= override_record.localised_text_len then
                    ...
                    override_record.uic:SetStateText(override_record.localised_text, "STEVE_VIRGO_TODO");
                ...
                override_record.uic:SetStateText(text_to_display, "STEVE_VIRGO_TODO");
...
function text_pointer:hide(immediately, suppress_event)
    ...
    -- hide immediately without playing any animations if we should
    if immediately then
        if uic_docker then
            ...
            if self.uic_pointer_line_parent then
                self.uic_pointer_line_parent:SetVisible(false);
...
function text_pointer:set_style(style, ...)
    ...
    --- @desc <tr><td><code>semitransparent_highlight</code></td><td>Sets the "semitransparent" style and sets the close button to highlight two seconds after it is shown. The text pointer will not close when the close button is clicked, however.</td></tr>
    elseif style == "semitransparent_highlight_close" then
        self:set_style("semitransparent");
        self:set_highlight_close_button(2);
    
    --- @desc <tr><td><code>semitransparent_highlight_dont_close</code></td><td>Sets the "semitransparent" style and sets the close button to highlight two seconds after it is shown. The text pointer will close when the close button is clicked.</td></tr>
    elseif style == "semitransparent_highlight_dont_close" then
        self:set_style("semitransparent_highlight_close");
...
function text_pointer:get_full_name()
    return self.full_name
end

function text_pointer:stop_all_listeners()
    for k, v in pairs(self.registered_listeners) do
        out.design("Removing text point listener: " .. k)
        core:remove_listener(k)
    end
    self.registered_listeners = nil
end