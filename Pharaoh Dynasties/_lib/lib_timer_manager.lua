...
function timer_manager:setup_after_campaign_world_created()
...
--- @desc Creates and returns a timer manager in campaign. This function should be supplied a <code>campaign_manager</code>. The function is automatically called by the @campaign_manager so there should be no need for game scripts to call this function.
--- @p @campaign_manager campaign manager
...
function timer_manager:new_campaign(cm)
	...
	-- Determine if we can proceed with full setup or not
	if cm:is_model_created() then
		-- Campaign model is already created, so call the full setup function
		self:setup_after_campaign_world_created();

	else
		-- Campaign model is not yet created - the full setup function will be called when the WorldCreated event is received by the campaign manager
		

		-- Temporary implementations of callback(), repeat_callback() and remove_callback() that throw script errors as the game is not ready to register timers yet. These will be overwritten by the proper callbacks when the game is ready.

		function timer_manager:callback(callback, interval, name)
			script_error("ERROR: callback() called but the campaign model is not yet created. No callback will be registered. Ensure this call happens later in the load sequence.");
		end;
		...
		-- Campaign implementation of repeat_callback()
		function timer_manager:repeat_callback(callback, interval, name)
			script_error("ERROR: repeat_callback() called but the campaign model is not yet created. No callback will be registered. Ensure this call happens later in the load sequence.");
		end;
		...
		-- Campaign implementation of remove_callback()
		function timer_manager:remove_callback(name)
			script_error("ERROR: remove_callback() called but the campaign model is not yet created. No callback will be registered. Ensure this call happens later in the load sequence.");
		end;
