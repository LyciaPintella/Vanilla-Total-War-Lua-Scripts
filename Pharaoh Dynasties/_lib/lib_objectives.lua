...
function objectives_manager:new()
	local o = core:get_static_object("objectives_manager");
	if o then
		return o;
	end;
	...
	core:add_static_object("objectives_manager", o);
...
local function perform_new_objective_action_with_leader(om, function_name, objective_key, objective_action_callback, on_show_callback)
	return perform_objective_action_with_leader(om, "new_objective", "random_localisation_strings_string_new_objective", objective_key, function_name, objective_action_callback, on_show_callback);
end;
...
--- @p [opt=false] @boolean param is_conceptual, If conceptual then acts more as a header and doesn't have a checkbox icon
function objectives_manager:set_objective(new_obj_name, obj_param_a, obj_param_b, is_conceptual)
	...
	is_conceptual = not not is_conceptual
	obj_param_a = obj_param_a or -1
	obj_param_b = obj_param_b or -1

	self:objective_out("[OBJECTIVES] performing set_objective action, key is [" .. new_obj_name .. "], params are [" .. (obj_param_a == -1 and "nil" or tostring(obj_param_a))  .. ", " .. (obj_param_b == -1 and "nil" or tostring(obj_param_b)).. "]" .. ", is_conceptual = [" .. tostring(is_conceptual) .. "]");
	self.uic_objectives:InterfaceFunction("set_objective", new_obj_name, obj_param_a, obj_param_b, is_conceptual);

	-- if we have objective parameters then dont try and complete, but add this objective to the list if it's not already there
	if obj_param_a ~= -1 then
...
--- @function complete_all_objectives_except_most_recent
--- @desc Marks all scripted objectives except the most_recent as completed for the player to see. Note that they will remain on the scripted objectives panel until removed with @objectives_manager:remove_objective.
function objectives_manager:complete_all_objectives_except_most_recent()
	self:create_objectives_panel();
	self:objective_out("[OBJECTIVES] complete_all_objectives_except_most_recent() called");

	for i = 1, #self.objectives_list-1 do
		local current_obj = self.objectives_list[i];
		local current_obj_name = current_obj.name
		self:objective_out("[OBJECTIVES] performing complete_objective action, key is [" .. current_obj_name .. "]");
		self.uic_objectives:InterfaceFunction("complete_objective", current_obj_name);
	end;
end;
...
function objectives_manager:update_objective_chain(name, objective, opt_a, opt_b)
	...
	self:complete_all_objectives_except_most_recent()
