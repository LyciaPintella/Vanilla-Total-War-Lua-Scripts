...
--- @example bm = battle_manager:new(empire_battle:new());
--- @example  
...
-- set generated_battle to be of custom type TYPE_GENERATED_BATTLE
...
function generated_battle:new(screen_starts_black, prevent_deployment_for_player, prevent_deployment_for_ai, intro_cutscene, is_debug)
	...
	-- set 'gb' to derive from self ('generated_battle')
	...
	gb.cutscene_during_deployment = cutscene_during_deployment;
		...
		for j = 1, #current_alliance do
			...
			for k = 1, #current_armies do
				...
				for m = 1, current_army.sunits:count() do
					bm:out("\t\t\t\t" .. m .. ": " .. current_army.sunits:item(m).unit:type() .. " at position " .. v_to_s(current_army.sunits:item(m).unit:position()));
				end;
	...
	for i = 1, alliances:count() do
		...
		for j = 1, armies:count() do
			...
			for k = 1, num_units do
				local new_sunit = script_unit:new_from_army(current_army, k);
...
--- @desc &emsp;- Supply an alliance number and an optional script name. This supports the randomised ordering of armies in an alliance that we see from campaign.
	...
	self.sm:add_listener(
		...
		function()
			...
			bm:callback(
				function()
					if group_name then
						self.bm:out("generated_battle:start_terrain_composite_scene_on_message() is responding to message " .. message .. " and is now enqueuing composite scene with key " .. comp_scene_key .. " from group " .. tostring(group_name) .. wait_offset_str);
					else
...
--- @p [opt="battle_started"] @string start message, Start message which begins the wait time countdown.
	...
	local function start_countdown()
		if not message_cancelled then
			...
			if start_message == true then
				self.bm:out("generated_battle:message_on_time_offset() is starting timer immediately, will trigger message " .. message .. " in " .. wait_time .. "ms");
			else
...
--- @p [opt=false] boolean force attack, If set to true will use script planner attack, instead of move. Use with caution, this can get tripped by visibility.
function generated_army:attack_force(enemy_force, force_attack)
	-- ensure we have built our allied and enemy forces
	self:get_allied_and_enemy_forces();
	...
	-- use move_to_force instead of attack_force unless forced to
	if force_attack then
		self.script_ai_planner:attack_force(enemy_force.sunits);
	else
		self.script_ai_planner:move_to_force(enemy_force.sunits);
	end
end;
...
--- @p [opt=0] number wait offset, Time to wait after receipt of the message before issuing the attack order.
--- @p [opt=false] boolean force attack, If set to true will use script planner attack, instead of move. Use with caution, this can get tripped by visibility.
function generated_army:attack_force_on_message(message, enemy_force, wait_offset, force_attack)
	if not is_string(message) then
		...
		function()
			bm:callback(
				function()
					...
					self:attack_force(enemy_force, force_attack or false);
				end,
				wait_offset
			);
		end
	...
	self.sm:add_listener(
		...
		function()
			...
			if should_release then
				self.bm:out("\talso releasing script control");
				self.sunits:release_control();
			end;
		end
	);
...
--- @p string message
--- @p [opt=true] boolean invincibility
function generated_army:set_invincible_on_message(message, invincibility)
	if not is_string(message) then
	...
	self.sm:add_listener(
		message,
		function()
			bm:out(self.id .. " responding to message " .. message .. ", setting invincibility for all currently standing units to " .. tostring(invincibility));
			self.sunits:invincible_if_standing(false, invincibility);
		end
	);
end;
	...
	self.sm:add_listener(
		...
		function()
			if not cancel_message_received then
					bm:out(self.id .. " responding to message " .. message .. ", starting to deploy reinforcements at random intervals");
...
function generated_army:message_on_commander_dead_or_routing(message)
	...
	bm:out(self.id .. " Registering message_on_commander_dead_or_routing() with message " .. tostring(message));
	...
	bm:watch(
		...
		0,
		function()
			bm:out(self.id .. " commander is dead or routing, triggering script message " .. message);
			self.sm:trigger_message(message);
		end,
		self.id
	);
...
function generated_army:message_on_commander_dead_or_shattered(message)
	...
	bm:out(self.id .. " Registering message_on_commander_dead_or_shattered() with message " .. tostring(message));
	...
	bm:watch(
		...
		0,
		function()
			bm:out(self.id .. " commander is dead or shattered, triggering script message " .. message);
			self.sm:trigger_message(message);
		end,
		self.id
	);
...
function generated_cutscene:set_troy_subtitles()
	self.use_troy_subtitles = true;
end;
...
function generated_battle:enqueue_cutscene_elements(cutscene_intro, gc)
	...
	for i,element in ipairs(gc.elements) do	
		...
		if gc.is_debug then
			bm:out(element.id .. "= " .. tostring(speech) .. "," .. tostring(subtitle) .. "," .. tostring(camera) .. "," .. tostring(min_length) .. "," .. tostring(loop_camera) .. "," .. tostring(wait_for_vo_start) .. "," .. tostring(wait_for_vo_end));
		end
