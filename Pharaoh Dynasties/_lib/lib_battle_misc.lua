...
--- @return boolean all units routing or dead
function is_routing_or_dead(obj, shattered_only, permit_rampaging)
	...
	elseif is_army(obj) then
		...
		local result = is_routing_or_dead(obj:units());
		...
		if not result then
			...
			for i = 1, obj:num_reinforcement_units() do
				...
				result = is_routing_or_dead(r_units);
...
--- @return boolean all units shattered or dead
...
--- @p [opt=false] boolean shattered only, Only count shattered units
--- @p [opt=false] boolean permit rampaging, Don't automatically count rampaging units, check if they are actually routing too.
--- @return integer number of routing or dead units
		...
		elseif is_scriptunits(obj) then
			...
			for i = 1, #obj do
				...
				elseif is_scriptunit(current_obj) then
					...
					count = count + num_units_routing(obj[i], shattered_only);
				end;
			end;
		end;
			
	elseif is_army(obj) then
		count = num_units_routing(obj:units(), shattered_only);
		...
		for i = 1, obj:num_reinforcement_units() do
			...
			if is_units(r_units) then
				count = count + num_units_routing(r_units, shattered_only);
			end;
		end;
		
	elseif is_armies(obj) then
		for i = 1, obj:count() do
			count = count + num_units_routing(obj:item(i):units(), shattered_only);
		end;

	elseif is_alliance(obj) then
		count = num_units_routing(obj:armies(), shattered_only);
...
--- @p object collection to test, Object or collection to test. Supported objects/collections are units, scriptunits, army, armies, alliance or a table containing any of the above, or units/scriptunits.
--- @p [opt=false] boolean permit rampaging, Don't automatically count rampaging units, check if they are actually routing too.
--- @return integer number of shattered or dead units
...
--- @return number of units engaged in melee
...
--- @p object collection to test, Collection to test. Supported collection types are units, scriptunits, army, armies, alliance and table.
--- @return number of units under fire
...
--- @return integer number within range of position
...
--- @return unit closest unit
--- @return number distance of closest unit
--- @return scriptunit closest scriptunit if applicable, nil if not
...
--- @desc Alias of <code>get_closest_unit(obj, pos, <strong>true</strong>, test)</code>. Returns the closest unit in the supplied container to a supplied position, and its distance to that position.
...
--- @return number average altitude in m
--- @return integer number of units in collection
...
--- @p unit subject unit
--- @return boolean collection contains unit
...
--- @return integer number of passing units
--- @return integer total number of units
...
--- @return table table containing passing units
...
--- @return integer number of soldiers currently alive
--- @return integer initial number of soldiers
...
--- @p alliance alliance
--- @return boolean visibility result
...
--- @return boolean are all deployed
