...
function infotext_manager:new()
	...
	-- Setup handles to uicomponents when UI is created
	if core:is_battle() then
		i:setup_infotext_uicomponents();
	else
		core:add_listener(
			"create_infotext",
			"UICreated",
	...
	-- Listen for clicks on any infotext buttons
	core:add_listener(
		...
		function(context)
			local uic_infotext_line_id = UIComponent(UIComponent(context.component):Parent()):Id();
			local button_state_override_record = i.button_state_overrides[uic_infotext_line_id];
...
function infotext_manager:set_button_state_override(infotext_key, on_click_callback, on_display_callback, event_to_disable_button_on)
	...
	self.button_state_overrides[infotext_key] = {
		...
		button_has_already_been_disabled = false
	};
end;
...
--- @desc Adds one or more lines of infotext to the infotext panel. The infotext box will expand to the final required size and then individual infotext lines are faded on sequentially with an interval between each.
--- @p ... infotext @string, A vararg of one or more infotext strings to be shown. Each should be a key from the <code>advice_info_texts</code> database table. Alternatively, a @function may be supplied, which will be called as the infotext is finished displaying.
function infotext_manager:add_infotext(...)
	self:add_infotext_action(false, false, ...);
end;


--- @function add_infotext_with_leader
--- @desc Adds one or more lines of infotext to the infotext panel, with a @topic_leader. The first string argument will be used as the topic leader text.
--- @desc The infotext box will expand to the final required size and then individual infotext lines are faded on sequentially with an interval between each.
--- @p ... infotext @string, A vararg of one or more infotext strings to be shown. Each should be a key from the <code>advice_info_texts</code> database table. Alternatively, a @function may be supplied, which will be called as the infotext is finished displaying.
function infotext_manager:add_infotext_with_leader(...)
	self:add_infotext_action(false, true, ...);
end;


--- @function add_infotext_simultaneously
--- @desc Adds one or more lines of infotext to the infotext panel. All text will appear on the infotext panel simultaneously, rather than animating on one entry at a time. The infotext box will expand to the final required size and then all infotext lines are faded on at the same time.
--- @p ... infotext @string, A vararg of one or more infotext strings to be shown. Each should be a key from the <code>advice_info_texts</code> database table. Alternatively, a @function may be supplied, which will be called as the infotext is finished displaying.
function infotext_manager:add_infotext_simultaneously(...)
	self:add_infotext_action(true, false, ...);
end;


--- @function add_infotext_simultaneously_with_leader
--- @desc Adds one or more lines of infotext to the infotext panel, with a @topic_leader. All text will appear on the infotext panel simultaneously, rather than animating on one entry at a time. The first string argument will be used as the topic leader text.
--- @desc The infotext box will expand to the final required size and then individual infotext lines are faded on sequentially with an interval between each.
--- @p ... infotext @string, A vararg of one or more infotext strings to be shown. Each should be a key from the <code>advice_info_texts</code> database table. Alternatively, a @function may be supplied, which will be called as the infotext is finished displaying.
function infotext_manager:add_infotext_simultaneously_with_leader(...)
	self:add_infotext_action(true, true, ...);
end;


-- Internal function to action the adding of infotext
function infotext_manager:add_infotext_action(show_simultaneously, show_topic_leader, ...)
	local infotext_records = {};
	infotext_records.initial_delay = 0;
	infotext_records.show_simultaneously = show_simultaneously;
	
	-- make a copy of the inputs
	for i = 1, arg.n do
		local current_arg = arg[i];
		if is_string(current_arg) or is_function(current_arg) then
			table.insert(infotext_records, current_arg);
		else
			script_error("ERROR: infotext_manager:add_infotext_action() couldn't recognise argument [" .. i .. "] in supplied argument list - value [" .. tostring(current_arg) .. "] is of type [" .. type(current_arg) .. "], only strings and functions are supported");
			return false;
		end;
	end;
	...
	if self.infotext_currently_being_added then
		-- We are already in the process of adding infotext, so just add this infotext to the queue to be displayed
		table.insert(self.infotext_queue, infotext_records);
		return;
	end;
	...
	if show_topic_leader then
		...
		if uic_advice and uic_infotext_panel then
			-- We are showing a topic leader ahead of the infotext
			local tl = topic_leader:new(
				"new_info",
				"advice_info_texts_localised_text_" .. infotext_records[1]
			);

			local width = uic_infotext_panel:Width();

			-- Zoom-to target is 50% the width of the panel, and 60px down from the top
			tl:set_shrink_target_uicomponent(uic_infotext_panel, width / 2, 60);

			tl:set_hold_duration(TOPIC_LEADER_HOLD_DURATION_FOR_INFOTEXT);
			tl:start();

			-- Wait a longer period before actually adding the infotext to allow the tl to animate
			core:get_tm():real_callback(
				function()
					self:add_infotext_from_records(infotext_records)
				end, 
				TOPIC_LEADER_HOLD_DURATION_FOR_INFOTEXT,
				"add_infotext"
			);
...
--	Actually shows infotext from a list of records - for internal use
function infotext_manager:add_infotext_from_records(infotext_records, end_callbacks)
	end_callbacks = end_callbacks or {};
	local initial_delay = infotext_records.initial_delay;
	local show_simultaneously = infotext_records.show_simultaneously;
	...
	for i = 1, #infotext_records do
		local current_infotext_record = infotext_records[i];
		...
		if is_function(current_infotext_record) then
			table.insert(end_callbacks, current_infotext_record);
		...
		elseif string.sub(current_infotext_record, 1, 1) == "-" then
			...
			core:get_tm():real_callback(
				function()
					...
					self:add_single_infotext_entry(key, true);
			...
			if show_simultaneously then
				if #infotext_records * 0.1 > self.min_add_to_show_interval then
					t = (#infotext_records * 0.1 + initial_delay) * time_multiplier;
				else
			...
			core:get_tm():real_callback(
				function()
					...
					if not uic_entry then
						...
						print_all_uicomponent_children(uic_advice_interface);
						self:add_single_infotext_entry(key, false);
					end;
	...
	core:get_tm():real_callback(
		function()
			if #self.infotext_queue > 0 then
				local new_infotext_records = self.infotext_queue[1];
				table.remove(self.infotext_queue, 1);
				self:add_infotext_from_records(new_infotext_records, end_callbacks);
			else
				self:show_infotext_finish(end_callbacks);
			end;
		end,
...
function infotext_manager:add_single_infotext_entry(key, show_hidden)
	...
	if self.button_state_overrides[key] then
		...
		if button_state_override_record and button_state_override_record.event_to_disable_button_on then
			core:add_listener(
				...
				function()
					...
					button_state_override_record.button_has_already_been_disabled = true
				end,
				false
			)
		...
		if button_state_override_record.button_has_already_been_disabled then
			uic_button:SetDisabled(true)
			uic_button:SetState("inactive")
		end
...
--	for internal use
function infotext_manager:show_infotext_finish(end_callbacks)
	self.infotext_currently_being_added = false;
	
	for i = 1, #end_callbacks do
		end_callbacks[i]();
	end;
end;
...
--- @function set_auto_clear_timer
--- @desc Clears all infotexts after the specified time. Mouse over the infotext panel will freeze the timer. A value <= 0 will cancel any current timer
--- @p @number seconds
--- @p [opt=false] @boolean should_dismiss_advice_on_infotext_auto_hide_time_elapsed
function infotext_manager:set_auto_clear_timer(seconds, should_dismiss_advice_on_infotext_auto_hide_time_elapsed)
	should_dismiss_advice_on_infotext_auto_hide_time_elapsed = to_bool(should_dismiss_advice_on_infotext_auto_hide_time_elapsed)

	if self.uic_advice_interface then
		self.uic_advice_interface:InterfaceFunction("set_auto_clear_info_text_timer", seconds, should_dismiss_advice_on_infotext_auto_hide_time_elapsed);
	end;
end;
...
function infotext_manager:show_infotext_for_advice_from_history(advice_key)
	...
	for i = 1, #infotext_list do
		if not find_uicomponent(self.uic_advice_interface, infotext_list[i]) then
			self:add_single_infotext_entry(infotext_list[i], false);
		end;
	end;
end;
