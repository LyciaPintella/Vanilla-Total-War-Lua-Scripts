...
narrative_event = {
	...
	--[[
	start_callbacks = {},											-- Callbacks to call when the narrative event starts
	trigger_callbacks = {},											-- Trigger callbacks to call when the narrative event triggers. This can be used to trigger actions to happen as the mission is issued, or something other than a mission entirely.
	completion_callbacks = {},										-- Trigger callbacks to call when the narrative event completes
...
-- set narrative_event to be of custom type TYPE_CAMPAIGN_NARRATIVE_EVENT, with a custom tostring() response
...
--- @function new
--- @desc Creates and returns a narrative event object. An advice key, infotext, and mission key may optionally be specified. Alternatively, narrative elements (e.g. cutscenes, event messages) can be triggered by adding them as an on-trigger callback using @narrative_event:add_trigger_callback.
...
function narrative_event:new(name, faction_key, advice_key, infotext, mission_key, messages_on_mission_completed, suppress_mission_complete_event_msg)
	...
	-- set 'ne' to derive from self ('narrative_event')
	...
	ne.start_callbacks = {};
	ne.trigger_callbacks = {};
	ne.completion_callbacks = {};
	...
	if mission_key then
		...
		local function completion_callback(mission_completion_status)
			...
			for i = 1, #ne.completion_callbacks do
				ne.completion_callbacks[i](ne);
			end;
...
----------------------------------------------------------------------------
--- @section Progression Callbacks
----------------------------------------------------------------------------


--- @function add_start_callback
--- @desc Adds a callback to be called when this narrative event is started. This can be used to in certain situations to start other related scripts.
--- @desc Multiple start callbacks may be added by calling this function multiple times.
--- @p @function callback
function narrative_event:add_start_callback(callback)
...
--- @desc When called, the callback will be passed two arguments - the string message that triggered the narrative event, and an <code>allow_issue_completed_callback</code> function. By calling this latter function with <code>false</code> as a single argument the narrative event is prevented from completing immediately, allowing the trigger callback to perform actions over time (e.g. show a cutscene). The narrative event can be allowed to trigger again by calling the <code>allow_issue_completed_callback</code> function again with <code>true</code> as a single argument.
--- @desc Multiple trigger callbacks may be added by calling this function multiple times.
--- @p @function callback
function narrative_event:add_trigger_callback(callback)
	if not validate.is_function(callback) then
		return false;
	end;

	table.insert(self.trigger_callbacks, callback);
end;


--- @function add_completion_callback
--- @desc Adds a callback to be called when this narrative event is completed.
--- @desc Multiple completion callbacks may be added by calling this function multiple times.
--- @p @function callback
function narrative_event:add_completion_callback(callback)
	if not validate.is_function(callback) then
		return false;
	end;
...
local function narrative_event_call_start_callbacks(ne)
	for i = 1, #ne.start_callbacks do
		ne.start_callbacks[i](ne);
	end;
end;
...
function narrative_event:start(trigger_immediately)
	...
	if trigger_immediately then
		...
		narrative_event_call_start_callbacks(self);
...
function narrative_event:trigger(trigger_condition_index)
	...
	local issue_completion_blocked_count = 0;
	...
	if #self.trigger_callbacks > 0 then
		function allow_issue_completed_callback(allow_completion)
			if allow_completion == false then
				issue_completion_blocked_count = issue_completion_blocked_count + 1;
				return;
			end;
			
			issue_completion_blocked_count = issue_completion_blocked_count - 1;

			if issue_completion_blocked_count == 0 then
	...
	local function trigger_basic(inv)
		...
		for i = 1, #self.trigger_callbacks do
			self.trigger_callbacks[i](self.trigger_conditions[trigger_condition_index].message, allow_issue_completed_callback);
		end;
	...
	cm:trigger_transient_intervention(
		...
		function(inv)
			...
			-- Call any trigger callbacks that have been set up
			for i = 1, #self.trigger_callbacks do
				self.trigger_callbacks[i](self.trigger_conditions[trigger_condition_index].event);
			end;
...
-- set narrative_query to be of custom type TYPE_CAMPAIGN_NARRATIVE_QUERY, with a custom tostring() response
...
function narrative_query:new(name, faction_key, trigger_messages, query_records)
	...
	-- set 'nq' to derive from self ('narrative_query')
...
-- set narrative_trigger to be of custom type TYPE_CAMPAIGN_NARRATIVE_TRIGGER, with a custom tostring() response
...
function narrative_trigger:new(name, faction_key, target_messages, main_event_and_condition_records, start_messages, cancel_messages, intervention_priority)
	...
	-- set 'nt' to derive from self ('narrative_trigger')
...
--- @example 	{
--- @example 		{
--- @example 			event = "ScriptEventPlayerFactionTurnStart",
