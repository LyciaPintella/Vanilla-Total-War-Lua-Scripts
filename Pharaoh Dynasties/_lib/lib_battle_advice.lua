...
-- set advice manager to be of custom type TYPE_ADVICE_MANAGER
...
function advice_manager:new(is_debug)	
	...
	-- set 'am' to derive from self ('advice_manager')
...
function advice_manager:trigger_next_advice()
	...
	-- also record the highest absolute priority, regardless of whether we picked this advice monitor (we would not pick it if we are only considering advice that can interrupt)
	local highest_absolute_priority = -1;
	local highest_absolute_priority_advice_monitor = false;
	
	self:inc_tab();
	
	for i = 1, #queue_to_trigger do
		local current_monitor = queue_to_trigger[i];
		local current_priority = current_monitor.priority;
		
		current_monitor.is_enqueued_to_trigger = false;
		
		if self.is_debug then
			self:out("querying advice " .. current_monitor.name .. " with priority " .. current_priority .. (current_monitor.can_interrupt_other_advice and ", this monitor can interrupt other advice so we're not going to consider advice that cannot interrupt on this pass" or ""));
		end;
		...
		if current_priority > highest_absolute_priority then
			highest_absolute_priority = current_priority;
			highest_absolute_priority_advice_monitor = current_monitor;
		end;
	end;
	
	-- add all advice in the queue other than the highest priority advice to the to-restart list
	for i = 1, #queue_to_trigger do
		if queue_to_trigger[i] ~= highest_priority_advice_monitor then
	...
	if self.is_debug then
		...
		if highest_absolute_priority > highest_priority then
			self:out("\tnote that advice monitor " .. highest_absolute_priority_advice_monitor.name .. " with a higher priority " .. highest_absolute_priority .. " was passed over as it cannot interrupt");
		end;
...
-- set advice monitor to be of custom type TYPE_ADVICE_MONITOR, with a custom tostring() response
