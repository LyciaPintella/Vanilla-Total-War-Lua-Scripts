...
active_pointer = {
	...
	orientations = {
		...
		topleftedge = {
			text_pointer_orientation = "left",
			calculate_x_offset = function(width)
				return 0;
			end,
			calculate_y_offset = function(height)
				return height - 10;
			end;
		},
		topright = {
		...
		toprightedge = {
			text_pointer_orientation = "right",
			calculate_x_offset = function(width)
				return 0;
			end,
			calculate_y_offset = function(height)
	...
	hide_all_text_pointers = false,								-- hide all other text pointers when shown
	should_pulse_panel = false,									-- pulse-highlight the text pointer panel when shown
	show_if_other_active_pointers_showing = false				-- Lets this active pointer show if another active pointer is currently showing (this can also be overriden by the show command)
};


-- set active_pointer to be of custom type TYPE_ACTIVE_POINTER, with a custom tostring() response
...
function active_pointer:new(name, orientation, uic_specifier, localised_text_key, x_proportion, y_proportion, width, suppress_record_in_advice_history)
	...
	tp:set_label_offset(orientation_record.calculate_x_offset(width), orientation_record.calculate_y_offset(height));
	...
	-- Add on-show and on-hide callbacks that set up and destroy polling processes/listeners
	tp:add_show_callback(
		function()
			ap:on_show(process_name);
		end
	);
	
	tp:add_hide_callback(
		function()
			ap:on_hide(process_name);
		end
	);
...
-- Internal function to test whether the uicomponent has moved
function active_pointer:has_uic_moved_from_cached_position(uic, cached_x, cached_y)
	local current_x, current_y = uic:Position();

	return math.abs(current_x - cached_x) > ACTIVE_TEXT_POINTER_COMPONENT_MOVEMENT_THRESHOLD or math.abs(current_y - cached_y) > ACTIVE_TEXT_POINTER_COMPONENT_MOVEMENT_THRESHOLD
end;


-- Internal function to be called when the active pointer is shown
function active_pointer:on_show(process_name)
	local tm = core:get_tm();
	local uic = self:get_uicomponent();
	local tp = self.text_pointer;
	...
	if not uic:Visible(true) then
		script_error(self.output_name .. " WARNING: showing active pointer but the uicomponent being pointed at is invisible, so the pointer will hide straight away. uicomponent is " .. uicomponent_to_str(uic));
		tp:hide(true);
		return false;
	end;

	-- Record usage in advice history if we should
	if self.record_in_advice_history then
		-- remove all current trigger listeners
		core:remove_listener(self.name .. "_trigger_listener");
		self:record_usage_in_advice();
	end;

	-- Tell core that we're showing
	core:register_active_pointer_showing(self, true);
	
	-- Hide all other text pointers if we should
	if self.hide_all_text_pointers then
		core:hide_all_text_pointers();
	end;

	-- Pulse panel if we should
	if self.should_pulse_panel then
	...
	-- Repeatedly poll the uic being pointed at - if it moves, or changes 
	tm:repeat_callback(
		function()	
			...
			if uic:IsValid() then
				...
				if self.hide_immediately_on_parent_with_hide_animation and uicomponent_has_parent_filter(uic, function(uic_parent) return uic_parent:CurrentAnimationId() == "hide" end) then
					
					should_hide = true;
					force_hide_immediately = true;

				-- If the uic being pointed at is priority locked, is not visible, or has moved, then hide it
				elseif uic:IsPriorityLocked() or not uic:Visible(true) or self:has_uic_moved_from_cached_position(uic, cached_x, cached_y) then
					should_hide = true;
				end;
			end;
			...
			if should_hide then
				if tp:is_showing() then
					if force_hide_immediately then
						self:out("hiding text pointer immediately");
					else
						self:out("hiding text pointer");
					end;
	...
	for i = 1, #self.hide_on_event_records do
		local current_hide_on_event_record = self.hide_on_event_records[i];
		...
		core:add_listener(
			...
			function()
				...
				if current_hide_on_event_record.hide_immediately then 
					self:out("received " .. current_hide_on_event_record.event .. " event and condition passes so hiding");
				else
					self:out("received " .. current_hide_on_event_record.event .. " event and condition passes so hiding immediately");
				end;
...
-- Internal function to be called when the active pointer is hidden
function active_pointer:on_hide(process_name)
	core:register_active_pointer_showing(self, false);

	core:remove_listener(process_name);
	core:get_tm():remove_callback(process_name);
end;
...
--- @function set_auto_hide_time
--- @desc Sets a timer after which the pointer will be automatically closed
--- @p @number seconds
function active_pointer:set_auto_hide_time(seconds)
	self.text_pointer:set_auto_hide_time(seconds)
end
...
--- @function set_hide_all_text_pointers
--- @desc Sets whether this active pointer should hide all other text pointers when it is shown. By default other text pointers are not hidden - use this function to enable this behaviour.
--- @p [opt=true] @boolean should hide
function active_pointer:set_hide_all_text_pointers(value)
	if value == false then
		self.hide_all_text_pointers = false;
	else
		self.hide_all_text_pointers = true;
	end;
end;


--- @function set_pulse_panel
--- @desc Sets whether the text pointer panel should pulse-highlight when it's shown. By default the panel is not highlighted - use this function to enable this behaviour.
--- @p [opt=true] @boolean should highlight
function active_pointer:set_pulse_panel(value)
	if value == false then
		self.should_pulse_panel = false;
	else
		self.should_pulse_panel = true;
	end;
end;


--- @function set_show_if_other_active_pointers_showing
--- @desc Sets whether this active pointer can be shown even if another active pointer is already being displayed onscreen. By default, active pointers won't display when another is present on-screen (unless they're forced to) - use this function to bypass this behaviour for this active pointer.
--- @p [opt=true] @boolean should allow
function active_pointer:set_show_if_other_active_pointers_showing(value)
	if value == false then
		self.show_if_other_active_pointers_showing = false;
	else
		self.show_if_other_active_pointers_showing = true;
	end;
end;
...
--- @desc When directly triggering an active pointer with either @active_pointer:show or @active_pointer:show_when_ready, the following preconditions must be met for the active pointer to actually display:
--- @desc <ul><li>The active pointer must not have already been shown, or it should be set to disregard advice history with the suppress flag set on @active_pointer:new.</li>
--- @desc <li>If any higher-priority active pointers have been registered with @active_pointer:wait_for_active_pointer, these must have previously been displayed.</li>
--- @desc <li>No other active pointer should currently be displaying, unless this active pointer has been set to disregard showing active pointers with @active_pointer:set_show_if_other_active_pointers_showing.</li></ul>
--- @desc When calling the @active_pointer:show function, the active pointer can be forced to disregard the preconditions above by setting the disable-preconditions boolean argument.
----------------------------------------------------------------------------


-- local function to test, for a given active pointer, whether all active pointers that were registered as higher priority than it have been displayed or not
local function test_if_all_higher_priority_pointers_shown_when_showing(ap, calling_function_name)
	local can_show, higher_priority_advice_not_shown = ap:all_higher_priority_pointers_shown();
	if not can_show then
		if calling_function_name then
			ap:out(calling_function_name .. "() called but not showing as a higher-priority active pointer [" .. tostring(higher_priority_advice_not_shown) .. "] has yet to show");
		end;
		return false;
	end;
...
-- local function to test, for a given active pointer, whether it has previously been displayed or not
local function test_advice_history_when_showing(ap, calling_function_name)
	if ap.record_in_advice_history and ap:is_usage_recorded_in_advice() then
		if calling_function_name then
			ap:out(calling_function_name .. "() called but not showing as it has been shown before");
		end;
		return false;
	end;
...
-- local function to test whether another active pointer is not already being displayed at this time
local function test_active_pointer_not_already_displayed(ap, calling_function_name)
	if ap.show_if_other_active_pointers_showing then
		-- We are allowed to show this active pointer when other active pointers are showing, so just return true
		return true;
	end;

	if core:is_any_active_pointer_showing() then
		if calling_function_name then
			local names_other_pointers_showing = core:get_showing_active_pointer_names();
			local num_other_pointers_showing = #names_other_pointers_showing;
	
			ap:out(calling_function_name .. "() called but not showing as active pointer" .. (num_other_pointers_showing == 1 and "" or "s") .. " [" .. table.concat(names_other_pointers_showing, ", ") .. "] " .. (num_other_pointers_showing == 1 and "is" or "are") .. " currently_showing");
		end;
...
-- local function to test all preconditions above
local function test_all_preconditions(ap, calling_function_name)
	if not test_advice_history_when_showing(ap, calling_function_name) or not test_if_all_higher_priority_pointers_shown_when_showing(ap, calling_function_name) or not test_active_pointer_not_already_displayed(ap, calling_function_name) then
		return false;
	end;
...
--- @function show
--- @desc Immediately shows the text pointer, assuming it passes its precondition tests. The precondition tests are listed in the documentation above.
--- @desc Alternatively, the precondition tests can be disabled by setting the boolean argument for this function to <code>true</code>.
--- @p [opt=false] @boolean disable precondition tests
function active_pointer:show(disable_precondition_tests)
	-- Scripted tours can and will open new UI panels sometimes, causing active text pointers to show up. 
	-- We do not want this behavior by default and I don't have time to write out a smarter solution right now :(
	if core:is_campaign() and core:get_active_scripted_tour() then
		return 
	end

	-- Test our precondition functions, unless we've been told not to
	if not disable_precondition_tests and not test_all_preconditions(self, "show") then
		return false;
	end;

	self.text_pointer:show();

	-- Setup scripts that were previously here have been moved to the on_show method
end;


--- @function show_when_ready
--- @desc Shows the active pointer when the target uicomponent is not moving, is visible, and is fully onscreen. The target uicomponent is the uicomponent that was specified when the active pointer was created. If the target uicomponent does not enter the correct state within the timeout period (default 5 seconds) then the process is cancelled.
--- @desc Additionally, when calling this function the preconditions listed at the top of this section must pass. If this active pointer has already been shown, or if it's still waiting for another higher-priority active pointer, then this function will immediately return. If another active pointer is on screen then this function will wait, although it will obey the timeout period as if the target uicomponent was not ready.
...
function active_pointer:show_when_ready(delay, timeout)
	...
	if self.text_pointer:is_showing() or self.is_enqueued_to_show then
		self:out("not showing as this active pointer is already showing or is enqueued to show");
		return false;
	end;

	-- Test higher-priority and advice history preconditions and exit immediately if either are false
	if not test_if_all_higher_priority_pointers_shown_when_showing(self, "show_when_ready") or not test_advice_history_when_showing(self, "show_when_ready") then
		return false;
	end;
	...
	local another_active_pointer_message_shown = false;

	-- cancel any previous show_when_ready process
	tm:remove_callback(process_name);

	-- Start polling process to determine when the target uicomponent has stopped moving and is visible
	tm:repeat_callback(
		function()
			-- stop if we've exceeded the max number of tries
			if num_tries > max_tries then
				self:out("show_when_ready() process is stopping as the timeout period has elapsed and the target uicomponent hasn't become valid/visible and stopped moving, or there is a blocking active pointer still on screen");
				tm:remove_callback(process_name);
				return;
			end;

			-- Test to see if another active pointer is showing (or if we're allowed to show anyway)
			if test_active_pointer_not_already_displayed(self) then
				local uic = self:get_uicomponent();
				...
				if uic and uic:IsValid() then
					...
					if not previous_x then
						-- First time we check, just cache the position
						previous_x = current_x;
						previous_y = current_y;
					else
						if current_x == previous_x and current_y == previous_y and uic:VisibleFromRoot() and is_fully_onscreen(uic) then
							-- The component has not moved, and is visible/onscreen, so go to show the active pointer
							if delay == 0 then
								self:show(true);
							else
								...
								tm:callback(
									function()
										...
										if uic and uic:IsValid() then
											...
											self:out("show_when_ready() process was unable to show the pointer after a delay of " .. delay .. (core:is_battle() and "ms" or "s") .. " as the target uicomponent no longer exists");
										end;
									end, 
									delay
								);
							...
							-- The component has moved or is not visible, so just update the cached position
							previous_x = current_x;
							previous_y = current_y;
						end;
					end;
				...
				if not another_active_pointer_message_shown then
					another_active_pointer_message_shown = true;
					local names_other_pointers_showing = core:get_showing_active_pointer_names();
					local num_other_pointers_showing = #names_other_pointers_showing;
					self:out("show_when_ready() process is unable to show the active pointer at this time as active pointer" .. (num_other_pointers_showing == 1 and "" or "s") .. table.concat(names_other_pointers_showing, ", ") .. "] " .. (num_other_pointers_showing == 1 and "is" or "are") .. " currently_showing. It will continue trying until the alloted timeout is exceeded.");
				end;
			end;
