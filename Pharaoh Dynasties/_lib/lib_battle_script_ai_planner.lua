...
--- @class script_ai_planner Script AI Planner
...
-- set script_ai_planner to be of custom type TYPE_SCRIPT_AI_PLANNER
...
--- @return script_ai_planner
function script_ai_planner:new(new_name, new_sunits, is_debug)
	local sai = {};
	
	-- set 'sai' to derive from self ('script_ai_planner')
...
function script_ai_planner:release()
	...
	-- make a copy of our sunit list to pass to remove_sunits
	local sunit_list = {};
	for i = 1, #self.sunit_list do
		table.insert(sunit_list, self.sunit_list[i]);
	end;
...
function script_ai_planner:attack_force(enemy_force, reorder)
	...
	if not is_table(enemy_force) then
		script_error(self.name .. " ERROR: attack_force() called but supplied force [" .. tostring(enemy_force) .. "] is not a table!");
		return false;
	end;
	...
	if is_routing_or_dead(self.sunit_list) then
		if self.is_debug then
			bm:out(self.name .. ": attack_force() called but all controlled units are routing, doing nothing");
			return;
		end;
	end;
	...
	for i = 1, #enemy_force do
		...
		if is_scriptunit(curr_enemy_sunit) then
			...
			script_error(self.name .. " ERROR: attack_force() called but item " .. i .. " [" .. tostring(curr_enemy_sunit) .. " ]  of supplied force [" .. tostring(enemy_force) .. "] is not a scriptunit");
			return false;
		end;
	end;
	...
	if not closest_enemy then
		...
		if not non_routing_enemy_found then
			if self.is_debug then
				bm:out(self.name .. ":attack_force() called but couldn't find any targets that are not routing - exiting");
			end;			
		else	
			if self.is_debug then
				bm:out(self.name .. ":attack_force() called but couldn't find any targets that are visible and not routing - will move to the enemy force instead, and try attacking again in 30 seconds");
		...
		if self.is_debug then
			bm:out(self.name .. " attack_force() now attacking closest target " .. closest_enemy.name .. " at " .. v_to_s(closest_enemy.unit:position()));
		end;
	...
	bm:watch(
		...
		function()
			if self.is_debug then
				bm:out(self.name .. ": attack_force() is stopping as all controlled units are routing");
			end;
			bm:remove_process(self.name);
		end,
		self.name
	...
	bm:watch(
		...
		function()
			if self.is_debug then
				bm:out(self.name .. ": attack_force() can no longer find any visible and non-routing enemies - re-issuing order in an attempt to cope");
			end;
			bm:remove_process(self.name);
			self:attack_force(enemy_force, true);
		end,
		self.name
	);
end;
