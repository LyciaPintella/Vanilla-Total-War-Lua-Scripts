...
---	@class intro_campaign_scripts Tutorial Helper Scripts
...
-- set intro_campaign_camera_marker to be of custom type TYPE_INTRO_CAMPAIGN_CAMERA_MARKER, with a custom tostring() response
...
function intro_campaign_camera_marker:new(name, marker_x, marker_y, cutscene_duration, distance_override)
	...
	-- set 'cam_marker' to derive from self ('intro_campaign_camera_marker')
...
--
---	@class intro_campaign_camera_positions_advice Intro Campaign Camera Positions Advice
--- @page intro_campaign_scripts
--- @desc During campaign tutorial gameplay it is common to have scripted sections where the player has to inspect camera positions in order to progress. Camera markers are placed down in set positions, and when the player moves the camera close to them a cutscene plays which delivers narrative regarding the marker. The intention behind this is that the player learns about camera controls whilst narrative content is delivered to them.
...
intro_campaign_camera_positions_advice = {
	...
	help_panel_camera_info_callback = function() show_campaign_controls_cheat_sheet(true) end
};


-- set intro_campaign_camera_positions_advice to be of custom type TYPE_INTRO_CAMPAIGN_CAMERA_POSITIONS_ADVICE
...
function intro_campaign_camera_positions_advice:new(objective_key, completion_callback, markers)
	...
	-- set 'pa' to derive from self ('intro_campaign_camera_positions_advice')
...
--- @example 	"troy_intro_camp_camera_advice_001",
...
function intro_campaign_camera_positions_advice:start()
	cm:reset_objective_chain(self.objective_key)
...
--
---	@class intro_campaign_select_and_attack_advice Intro Campaign Select And Attack Advice
--- @page intro_campaign_scripts
...
-- set intro_campaign_select_and_attack_advice to be of custom type TYPE_INTRO_CAMPAIGN_SELECT_AND_ATTACK_ADVICE
...
--- @p string selection objective, Initial selection objective key, from the <code>scripted_objectives</code> table. This is delivered when the objective is issued to the player, and should set up the task narratively.
--- @p table markers, a table containing one or more @intro_campaign_camera_marker objects to be associated with this camera positions advice object.
--- @return intro_campaign_camera_positions_advice camera positions advice
function intro_campaign_select_and_attack_advice:new(player_cqi, enemy_cqi, initial_advice, selection_objective, attack_advice, attack_objective, completion_callback)
	...
	if attack_advice and not is_string(attack_advice) then
	...
	-- set 'sa' to derive from self ('intro_campaign_select_and_attack_advice')
...
function intro_campaign_select_and_attack_advice:add_initial_infotext(...)	
	for i = 1, arg.n do
...
function intro_campaign_select_and_attack_advice:add_attack_infotext(...)	
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
function intro_campaign_select_and_attack_advice:start()
	cm:reset_objective_chain(self.listener_name)
	self.cm = cm
	...
	cm:callback(
		function() 
			cm:show_advice(self.initial_advice, false, false, function() cm:hide_advisor_without_infotexts()  end, 7);
			...
			if #initial_infotext == 0 then
				...
				cm:add_infotext(unpack(initial_infotext));
			end;		
		end, 
		1
	);
...
function intro_campaign_select_and_attack_advice:show_attacking_advice()	
	if not self.attack_advice_shown then
		if self.attack_advice and self.attack_advice ~= "" then 
			cm:show_advice(self.attack_advice, false, false, function() cm:hide_advisor_without_infotexts()  end, 7);
		end
		self.attack_advice_shown = true;

		if #self.attack_infotext > 0 then
			cm:clear_infotext();
			cm:add_infotext(unpack(self.attack_infotext));
		end;
	end;
end;
...
intro_campaign_select_advice = {
	...
	proceed_immediately_if_character_selected = true
};


-- set intro_campaign_select_advice to be of custom type TYPE_INTRO_CAMPAIGN_SELECT_ADVICE
...
function intro_campaign_select_advice:new(player_cqi, advice_key, objective_key, completion_callback)
	...
	-- set 'sa' to derive from self ('intro_campaign_select_advice')
	set_object_class(sa, self);
...
function intro_campaign_select_advice:add_infotext(...)	
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
function intro_campaign_select_advice:set_scroll_to_character(value)
	if value == false then
		self.scroll_to_character = false;
	else
		self.scroll_to_character = true;
	end;
end;
...
function intro_campaign_select_advice:set_proceed_immediately_if_character_selected(value)
	if value == false then
		self.proceed_immediately_if_character_selected = false;
	else
		self.proceed_immediately_if_character_selected = true;
	end;
end;
...
function intro_campaign_select_advice:start()
	...
	-- If the relevant player character is already selected and we're set to proceed immediately then don't show any objective at all
	if self.proceed_immediately_if_character_selected and cuim:is_char_selected(char_player) then
		self:army_selected(true);
		return;
	end;
	
	-- Show advice if we have any
	if self.advice_key then
		cm:clear_infotext();

		cm:show_advice(self.advice_key, false, false, function() cm:hide_advisor_without_infotexts() end, 7);
	end;
	
	-- Show infotext if we have any, and the objective
	if #self.infotext == 0 then
		self:activate_objective();
	else
		table.insert(self.infotext, function() self:activate_objective() end);
		cm:add_infotext(unpack(self.infotext));
	end;
	
	-- If we've not been set to allow character selection at the time the objective is shown, then allow it now (optionally after a delay)
	if not self.allow_selection_with_objective then
		if self.selection_delay == 0 then
			self:allow_selection();
		else
			cm:callback(function() self:allow_selection() end, self.selection_delay);
		end;
	end;

	-- Scroll the camera to the character if we should
	if self.scroll_to_character then
		local duration = self.selection_delay > 0 and self.selection_delay or 2;
		cm:scroll_camera_to_character(self.player_cqi, duration);
	end;
end;


function intro_campaign_select_advice:allow_selection()
	-- allow selection of the player's army
	cuim:add_character_selection_whitelist(self.player_cqi);
		
	-- progress on selection	
	local player_char = cm:get_character_by_cqi(self.player_cqi);
	
	if cuim:is_char_selected(player_char) then
		self:army_selected();
	else
		cuim:highlight_character_for_selection(player_char, function() self:army_selected() end, self.highlight_altitude);
	end;
end;


function intro_campaign_select_advice:activate_objective()
...
function intro_campaign_select_advice:army_selected(proceeding_immediately)

	if not proceeding_immediately then
		cm:complete_objective(self.objective_key);
		cm:callback(function() cm:end_objective_chain(self.listener_name) end, 2);
	end;
...
intro_campaign_movement_advice = {
	player_cqi = -1,
	selection_advice_key = false,
	selection_infotext = {},
	selection_objective_key = "",
	movement_advice_key = false,
	movement_objective_key = "",
	movement_infotext = {},
	movement_advice_content_shown = false,
	wait_seconds_before_next_step = 2,
	log_x = 0,
	log_y = 0,
	...
	should_show_initial_camera_pan = true,
	camera_position_unary_proportion = 0.25,
	highlight_altitude = 2,

	-- optional action points advice/infotext/text pointer
	action_points_advice_key = "",
	action_points_infotext = {},
	action_points_text_pointer_creator_callback = nil,						-- a callback which, when called, creates and returns a text pointer
	action_points_advice_content_shown = false
};


-- set intro_campaign_movement_advice to be of custom type TYPE_INTRO_CAMPAIGN_MOVEMENT_ADVICE
set_class_custom_type_and_tostring(intro_campaign_movement_advice, TYPE_INTRO_CAMPAIGN_MOVEMENT_ADVICE);


function intro_campaign_movement_advice:new(player_cqi, selection_advice_key, selection_objective_key, movement_advice_key, movement_objective_key, log_x, log_y, completion_callback)
	if not is_number(player_cqi) then
	...
	if selection_advice_key and not is_string(selection_advice_key) then
		script_error("ERROR: attempt made to create intro_campaign_movement_advice but supplied selection advice key [" .. tostring(selection_advice_key) .. "] is not a string or nil");
		return false;
	end;
	...
	if movement_advice_key and not is_string(movement_advice_key) then
		script_error("ERROR: attempt made to create intro_campaign_movement_advice but supplied movement advice key [" .. tostring(movement_advice_key) .. "] is not a string or nil");
		return false;
	end;
	...
	-- set 'ma' to derive from self ('intro_campaign_movement_advice')
	set_object_class(ma, self);
	
	ma.player_cqi = player_cqi;
	ma.selection_advice_key = selection_advice_key;
	ma.selection_objective_key = selection_objective_key;
	ma.movement_advice_key = movement_advice_key;
	...
	ma.selection_infotext = {};
	ma.movement_infotext = {};
	ma.action_points_infotext = {};
	ma.action_points_advice_content_shown = cm:get_saved_value("action_points_advice_content_shown") or false

	-- Generate a unique listener name
	ma.listener_name = ma.listener_name .. "_" .. core:get_unique_counter();
...
function intro_campaign_movement_advice:add_selection_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) and not (is_number(current_infotext) and i == 1) then
			script_error("ERROR: add_selection_infotext() called but supplied item [" .. i .. "] is not a string or a number, it is a [" .. tostring(current_infotext) .. "]");
			return false;
		else
			table.insert(self.selection_infotext, current_infotext);
		end;
	end;
end;


function intro_campaign_movement_advice:add_movement_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) and not (is_number(current_infotext) and i == 1) then
			script_error("ERROR: add_movement_infotext() called but supplied item [" .. i .. "] is not a string or a number, it is a [" .. tostring(current_infotext) .. "]");
			return false;
		else
			table.insert(self.movement_infotext, current_infotext);
		end;
	end;
end;


function intro_campaign_movement_advice:set_action_points_advice_key(advice_key)
	if not validate.is_string(advice_key) then
		return false;
	end;

	self.action_points_advice_key = advice_key;
end;


function intro_campaign_movement_advice:add_action_points_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) and not (is_number(current_infotext) and i == 1) then
			script_error("ERROR: add_movement_infotext() called but supplied item [" .. i .. "] is not a string or a number, it is a [" .. tostring(current_infotext) .. "]");
			...
			table.insert(self.action_points_infotext, current_infotext);
		end;
	end;
end;


function intro_campaign_movement_advice:set_action_points_text_pointer_creator_callback(callback)
	if not validate.is_function(callback) then
		return false;
	end;

	self.action_points_text_pointer_creator_callback = callback;
end;
...
function intro_campaign_movement_advice:show_initial_camera_pan(value)
	if value == false then
		self.should_show_initial_camera_pan = false;
	else
		self.should_show_initial_camera_pan = true;

		if is_number(value) then
			self.camera_position_unary_proportion = value;
		else
			if value ~= true then
				script_error("WARNING: intro_campaign_movement_advice:show_initial_camera_pan() called but supplied value [" .. tostring(value) .. "] is not a boolean or a number. The initial camera pan will be shown, to a position which is a unary proportion of 0.25 along the line from character to target");
			end;
			self.camera_position_unary_proportion = 0.25;
		end;
	end;
end;
...
function intro_campaign_movement_advice:start()
	...
	-- show the initial camera pan if we should
	if self.should_show_initial_camera_pan then
		...
		local targ_x = self.initial_x or (player_char_x + ((self.display_x - player_char_x) * self.camera_position_unary_proportion));
		local targ_y = self.initial_y or (player_char_y + ((self.display_y - player_char_y) * self.camera_position_unary_proportion));
		
		-- pan camera to calculated target
		cm:scroll_camera_with_cutscene(
			self.initial_t, 
			function() self:intro_pan_finished(true) end,
			{targ_x, targ_y, self.initial_d, self.initial_b, self.initial_h}
		);
	else
		self:intro_pan_finished(false);
	end;
	
	-- show advice, infotext and objective
	if self.selection_advice_key then
		cm:callback(
			function() 
				cm:show_advice(self.selection_advice_key, false, false, function() cm:hide_advisor_without_infotexts() end, 7);
				
				-- build table of initial infotext and add a call to show the objective chain at the end
				local infotext_to_add = {};
				
				for i = 1, #self.selection_infotext do
					infotext_to_add[i] = self.selection_infotext[i];
				end;
				
				table.insert(infotext_to_add, function() cm:activate_objective_chain(self.listener_name, self.selection_objective_key) end);
				
				cm:add_infotext(unpack(infotext_to_add));
			end, 
			3
		);
	else
		cm:activate_objective_chain(self.listener_name, self.selection_objective_key);
	end;

	--Disable player movement before the character is selected just in case 
	local player_faction_key = cm:get_character_by_cqi(self.player_cqi):faction():name();
	cm:disable_movement_for_faction(player_faction_key);
end;


function intro_campaign_movement_advice:intro_pan_finished(cutscene_shown)
	...
	-- allow player to move army
	if (self.action_points_advice_key == "" and not self.action_points_text_pointer_creator_callback) or self.action_points_advice_content_shown then
--		cm:enable_movement_for_character(player_char_str);
	end
	-- detect when the player's army starts moving
...
function intro_campaign_movement_advice:highlight_character_for_selection()
	...
	cm:callback(
		...
		self.wait_seconds_before_next_step
	)
end;
...
function intro_campaign_movement_advice:army_selected()
	...
	-- If we have action points advice to give, or a text pointer to show, and haven't shown it already, then show it, otherwise proceed to show movement advice
	if (self.action_points_advice_key ~= "" or self.action_points_text_pointer_creator_callback) and not self.action_points_advice_content_shown then 

		cm:set_saved_value("action_points_advice_content_shown", true)

		local player_faction_key = cm:get_character_by_cqi(self.player_cqi):faction():name();

		-- Prevent movement or selection change at this time
		cm:disable_movement_for_faction(player_faction_key);
		uim:override("selection_change"):lock();

		cm:dismiss_advice();

		cm:complete_objective(self.selection_objective_key);

		local uic_action_points_bar = find_uicomponent(core:get_ui_root(), "info_panel_holder", "primary_info_panel_holder", "info_panel_background", "character_content", "container_action_bar", "action_points_parent");
		if uic_action_points_bar then
			uic_action_points_bar:Highlight(
				true,
				true,
				0,
				true
			...
			pulse_uicomponent(uic_action_points_bar, true, 20);
		else
			script_error("WARNING: intro_campaign_movement_advice:army_selected() could not find uic_action_points_bar");
		end;

		-- Show advice and infotext if we have any
		if self.action_points_advice_key then
			core:call_once(
				listener_name .. "_call_action_points_advice_once", 
				function()
					cm:show_advice(self.action_points_advice_key, false, false, function() cm:hide_advisor_without_infotexts() end, 7);
				end
			)
		

			if #self.action_points_infotext > 0 then
				cm:add_infotext(unpack(self.action_points_infotext));
			end;
		end;

		-- Show a text pointer if we have one
		local tp;
		if self.action_points_text_pointer_creator_callback then
			tp = self.action_points_text_pointer_creator_callback();

			if not validate.is_textoractivepointer(tp) then
				script_error("ERROR: intro_campaign_movement_advice:army_selected() failed to generate a text or active pointer from the supplied callback");
				tp = false;
			end;
		end;
		...
		if tp then
			tp:add_hide_callback(
				function()
					if uic_action_points_bar then
						...
						pulse_uicomponent(uic_action_points_bar, false);
					end;

					cm:callback(
						function()
							self:show_movement_advice();
						end,
						0.5
					);
				end
			...
			return;
		elseif self.action_points_advice_key then
			cm:progress_on_advice_dismissed(
				self.listener_name .. "_army_selected",
				function()
					self:show_movement_advice();
				end,
				0.5,
...
function intro_campaign_movement_advice:show_movement_advice()
	local listener_name = self.listener_name;
	local player_char = cm:get_character_by_cqi(self.player_cqi);
	local player_faction_key = player_char:faction():name();
	...
	core:add_listener(
		...
		function()		
			...
			local player_char = cm:get_character_by_cqi(self.player_cqi) -- slightly clumsy redefinition
			if not uim:is_char_selected(player_char) then
				self:highlight_character_for_selection(cm:char_lookup_str(self.player_cqi));
			else
				self:show_movement_advice()
			end
		end,
	...
	-- Ensure that the player can move and selection is unlocked
	cm:enable_movement_for_faction(cm:get_character_by_cqi(self.player_cqi):faction():name());

	uim:override("selection_change"):unlock();

	self:show_movement_marker(true);

	-- Show movement advice and infotext the first time, otherwise just update the scripted objective
	if self.movement_advice_key and not self.movement_advice_content_shown then
		self.movement_advice_content_shown = true;

		cm:show_advice(self.movement_advice_key, false, false, function() cm:hide_advisor_without_infotexts() end, 7);
					
		-- build table of initial infotext and add a call to show the objective chain at the end
		local infotext_to_add = {};
		
		for i = 1, #self.movement_infotext do
			infotext_to_add[i] = self.movement_infotext[i];
		end;
...
function intro_campaign_movement_advice:army_is_moving()
	if (self.action_points_advice_key or self.action_points_text_pointer_creator_callback) then
		local uic_action_points_bar = find_uicomponent(core:get_ui_root(), "info_panel_holder", "primary_info_panel_holder", "info_panel_background", "character_content", "container_action_bar", "action_points_parent");
		if uic_action_points_bar then
			pulse_uicomponent(uic_action_points_bar, false)
		end
...
function intro_campaign_movement_advice:army_has_arrived(arrived_cleanly)
	...
	local char_lookup = cm:char_lookup_str(self.player_cqi)
	local character = cm:get_character_by_cqi(self.player_cqi)

	-- zero player army AP, so that it can't move again
	cm:zero_action_points(char_lookup);
	...
	core:remove_listener(listener_name)
	uim:unhighlight_character_for_selection(character)
...
intro_campaign_recruitment_advice = {
	...
	listener_name_recruitment_cancel = "recruitment_cancel_on_unit_tutorial",
	num_recruited = 0,
	recruitment_limit = 0,
	unit_cards_highlighted = false,
	highlight_altitude = 2,
	all_units_recruited_callback = false,
	should_disable_saving = true,
	should_reenable_saving_on_end = false
};


-- set intro_campaign_recruitment_advice to be of custom type TYPE_INTRO_CAMPAIGN_RECRUITMENT_ADVICE
...
function intro_campaign_recruitment_advice:new(player_cqi, start_advice, selection_objective, panel_objective, recruitment_objective, recruitment_limit, end_advice, completion_callback)
	...
	-- set 'rt' to derive from self ('intro_campaign_recruitment_advice')
...
function intro_campaign_recruitment_advice:add_initial_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
...
function intro_campaign_recruitment_advice:set_should_disable_saving(value)
	if value == false then
		self.should_disable_saving = false;
	else
		self.should_disable_saving = true;
	end;
end;
...
function intro_campaign_recruitment_advice:start()
	...
	if self.should_disable_saving then
		if not cm:is_saving_disabled() then
			self.should_reenable_saving_on_end = true;
		end;
	...
	-- advice
	cm:show_advice(self.start_advice, false, false, function() cm:hide_advisor_without_infotexts()  end, 7);
...
function intro_campaign_recruitment_advice:show_recruitment_panel_infotext()
	...
	if #self.recruitment_panel_infotext > 0 then
		cm:add_infotext(unpack(self.recruitment_panel_infotext));
	end;
end;


local function highlight_recruitment_button_on_army_panel_opened()
	uim:override("recruit_units"):set_allowed(true);
...
function intro_campaign_recruitment_advice:character_selected()
	...
	-- Check if the player cancels recruitment on any units prior to recruitment panel open, since he can deselect and reselect a hero with enqueed units
	local listener_name_recruitment_cancel = self.listener_name_recruitment_cancel;
	core:remove_listener(listener_name_recruitment_cancel);
	core:add_listener(
		listener_name_recruitment_cancel,
		"ComponentLClickUp",
	...
	if uim:is_panel_open("units_panel") then
		...
		if uim:is_panel_open("units_recruitment") then
			core:remove_listener(listener_name);
			highlight_component(false, true, "button_group_army", "button_recruitment");
			self:player_opens_recruitment_panel();
		else
			highlight_recruitment_button_on_army_panel_opened();
		end
	else
		core:add_listener(
			"intro_campaign_recruitment_advice_character_selected",
			"PanelOpenedCampaign",
			function(context)
...
function intro_campaign_recruitment_advice:show_unit_card_infotext()
	...
	self.unit_card_infotext_shown = true;

	if #self.unit_card_infotext > 0 then
		cm:add_infotext(unpack(self.unit_card_infotext));
	end;
end;
...
function intro_campaign_recruitment_advice:player_opens_recruitment_panel()
	...
	core:add_listener(
		...
		function(context)
			...
			if self.num_recruited >= self.recruitment_limit then
				core:remove_listener(listener_name);
				core:remove_listener(self.listener_name_recruitment_cancel);
...
function intro_campaign_recruitment_advice:highlight_all_unit_cards(value)
	if value then
		...
		local uic_recruitment_list = find_uicomponent(core:get_ui_root(), "main_units_panel", "recruitment_options", "recruitment_listbox", "local1", "listview", "list_clip", "list_box", "natives_clip");
...
function intro_campaign_recruitment_advice:show_unit_recruited_infotext()
	...
	if #self.unit_recruited_infotext > 0 then
		cm:add_infotext(unpack(self.unit_recruited_infotext));
	end;
end;
...
function intro_campaign_recruitment_advice:all_units_recruited()
	...
	if self.should_reenable_saving_on_end then
	...
	cm:callback(
		function()
			cm:show_advice(self.end_advice, false, false, function() cm:hide_advisor_without_infotexts() end, 7);
			
			cm:progress_on_advice_dismissed(
				"units_recruited_advice_dismissed",
				function()
...
-- set intro_campaign_select_settlement_advice to be of custom type TYPE_INTRO_CAMPAIGN_SELECT_SETTLEMENT_ADVICE
...
function intro_campaign_select_settlement_advice:new(region_name, province_name, initial_advice, selection_objective, completion_callback)
	...
	-- set 'sa' to derive from self ('intro_campaign_select_settlement_advice')
	set_object_class(sa, self);
...
function intro_campaign_select_settlement_advice:add_initial_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_initial_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
-- set intro_campaign_panel_highlight_advice to be of custom type TYPE_INTRO_CAMPAIGN_PANEL_HIGHLIGHT_ADVICE
...
function intro_campaign_panel_highlight_advice:new(initial_advice, completion_callback, ...)
	...
	-- set 'sa' to derive from self ('intro_campaign_panel_highlight_advice')
	set_object_class(pa, self);
...
function intro_campaign_panel_highlight_advice:add_infotext(...)	
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
function intro_campaign_panel_highlight_advice:start()
	...
	if #self.initial_infotext > 0 then
		cm:add_infotext(unpack(self.initial_infotext));
	end;
	
	cm:progress_on_advice_dismissed("initial_advice_campaign_panel_dismissed" ,function() self:advice_dismissed() end, 0, self.show_advisor_dismiss_button);
end;
...
-- set intro_campaign_building_construction_advice to be of custom type TYPE_INTRO_CAMPAIGN_BUILDING_CONSTRUCTION_ADVICE
...
function intro_campaign_building_construction_advice:new(region_name, province_name, start_advice, start_objective, base_building_card, chain_building_card, upgrade_building_card, upgrade_objective, end_advice, completion_callback)
	...
	-- set 'ba' to derive from self ('intro_campaign_building_construction_advice')
...
function intro_campaign_building_construction_advice:add_initial_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_initial_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
function intro_campaign_building_construction_advice:add_completion_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
...
function intro_campaign_building_construction_advice:settlement_selected(do_not_update_objective)
	...
	core:add_listener(
		...
		function(context) return 
			uicomponent_descended_from(UIComponent(context.component), self.base_building_card) or 
			uicomponent_descended_from(UIComponent(context.component), "construction_popup") 
		end,
		function()
			core:remove_listener(listener_name);
			self:highlight_base_building_card(false);
...
function intro_campaign_building_construction_advice:player_completes_upgrade()
	...
	cm:progress_on_advice_dismissed(
		"building_construction_advice_dismissed",
		function()
			self.completion_callback();
		end,
		1,
...
-- set intro_campaign_technology_advice to be of custom type TYPE_INTRO_CAMPAIGN_TECHNOLOGY_ADVICE
...
function intro_campaign_technology_advice:new(start_advice, tech_panel_objective, tech_objective, completion_advice, completion_callback)
	...
	if completion_advice and not is_string(completion_advice)  then
	...
	-- set 'ba' to derive from self ('intro_campaign_technology_advice')
...
function intro_campaign_technology_advice:add_initial_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_initial_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
function intro_campaign_technology_advice:add_completion_infotext(...)	
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_completion_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
function intro_campaign_technology_advice:set_should_disable_saving(value)
	if value == false then
		self.should_disable_saving = false;
	else
		self.should_disable_saving = true;
...
function intro_campaign_technology_advice:cache_and_set_advisor_priority()
	if is_number(self.desired_priority) then
...
function intro_campaign_technology_advice:highlight_technology_button()
	...
	highlight_component(true, false, "hud_campaign", "faction_widget_list", "button_technology");
	...
	core:add_listener(
		...
		function()
			highlight_component(false, false, "hud_campaign", "faction_widget_list", "button_technology");
...
function intro_campaign_technology_advice:is_valid_technology_name(name)
	if string.find(name, "tech_") then
		return true;
	end;
	return false;
end;
	...
	if self.player_is_currently_researching_technology then
		...
		for technology_uic, value in pairs(available_technologies) do
			if not highlighted_technologies[technology_uic] then
				technology_uic:Highlight(true, false, 0);
		...
		function()
			...
			cm:callback(
				function()
					...
					local close_button_uic = find_uicomponent(core:get_ui_root(), "technology_panel", "button_close_panel")
					if close_button_uic then
						close_button_uic:Highlight(true, false, 0, true)
					end
				end,
				0.2
			);
...
function intro_campaign_technology_advice:research_started()
	...
	local close_button_uic = find_uicomponent(core:get_ui_root(), "technology_panel", "button_close_panel")
	if close_button_uic then
		close_button_uic:Highlight(false, false)
	end
	...
	-- completion advice
	if self.completion_advice then
		cm:show_advice(self.completion_advice, false, false, function() cm:hide_advisor_without_infotexts()  end, 7);
		...
		cm:progress_on_advice_dismissed(
			"technology_research_completed_advice_dismissed",
			function()
				cm:callback(
					function()
						if self.should_reenable_saving_on_end then
...
-- set intro_campaign_end_turn_advice to be of custom type TYPE_INTRO_CAMPAIGN_END_TURN_ADVICE
...
function intro_campaign_end_turn_advice:new(initial_advice, end_turn_objective, completion_callback)
	...
	-- set 'ea' to derive from self ('intro_campaign_end_turn_advice')
...
function intro_campaign_end_turn_advice:add_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
function intro_campaign_end_turn_advice:start()
	...
	cm:get_campaign_ui_manager():override("end_turn"):set_allowed(true);
...
TYPE_INTRO_START_TUTORIAL_BATTLE_ADVICE = "intro_start_tutorial_battle_advice";

function is_introstarttutorialbattleadvice(obj)
	if tostring(obj) == TYPE_INTRO_START_TUTORIAL_BATTLE_ADVICE then
		return true;
	end;
...
intro_start_tutorial_battle_advice = {
	tutorial_menu_root = "options_tutorial_menu",
	button_tutorial_menu = "button_tutorial_menu",
	battle = {
		BASIC = {
			start = function() start_tutorial_battle("BASIC", true) end,
			button_id = "tutorial_battle_1"

		},
		ADVANCED = {
			start = function() start_tutorial_battle("ADVANCED", true) end,
	...
	listener_name = "tutorial_battles",
	completion_callback = nil,
};


function intro_start_tutorial_battle_advice:new(battle_type, advice_key, objective_key, completion_callback)
	
	if not is_string(battle_type) or self.battle[battle_type] == nil then
		script_error("ERROR: attempt made to create intro_start_tutorial_battle_advice but supplied advice key [" .. tostring(battle_type) .. "] is not a string or IS NOT VALID TUTORIAL BATTLE");
		return false;
	end;

	if not is_string(advice_key) then
		script_error("ERROR: attempt made to create intro_start_tutorial_battle_advice but supplied advice key [" .. tostring(advice_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(objective_key) then
		script_error("ERROR: attempt made to create intro_start_tutorial_battle_advice but supplied objective key [" .. tostring(objective_key) .. "] is not a string");
		return false;
	end;
	
	if not is_function(completion_callback) then
		script_error("ERROR: intro_start_tutorial_battle_advice:new() was called but supplied completion callback [" .. tostring(completion_callback) .. "] is not a function");
		return false;
	end;
	...
	self.__index = self;
	self.__tostring = function() return TYPE_INTRO_CAMPAIGN_start_tutorial_battle_advice end;
	
	sa.battle_type = battle_type;
	sa.advice_key = advice_key;
	sa.objective_key = objective_key;
	sa.infotext = {};
	sa.completion_callback = completion_callback
...
function intro_start_tutorial_battle_advice:add_infotext(...)
	for i = 1, arg.n do
		local current_infotext = arg[i];
		if not is_string(current_infotext) then
			script_error("ERROR: add_infotext() called but supplied item [" .. i .. "] is not a string, it is a [" .. tostring(current_infotext) .. "]");
...
function intro_start_tutorial_battle_advice:start()
	local uim = cm:get_campaign_ui_manager();
	local tutorial_menu = core:get_ui_root():SequentialFind(self.tutorial_menu_root)
	local button_tutorial_menu = core:get_ui_root():SequentialFind("button_tutorial_menu")

	core:cache_and_set_advisor_priority(1500, true);

	local start_battle = nil
	local open_menu = nil

	open_menu = function()
		button_tutorial_menu:Highlight(true, false, 0)

		cm:show_advice(self.advice_key);
        if #self.infotext >= 1 then
            cm:add_infotext(unpack(self.infotext))
        end

        core:add_listener(
            "intro_open_tutorial_menu",
            "TutorialMenuOpened",
            function(context) return context.is_opening end,
            function(context)
				button_tutorial_menu:Highlight(false, false, 0)
	...
	start_battle = function()
		if tutorial_menu then
			local button = tutorial_menu:SequentialFind(self.battle[self.battle_type].button_id)
			self:enable_tutorial_menu_buttons(false)
			button:Highlight(true, true, 0)

			-- Nasty workaround to not show the confirmation box while doing the advice
			-- Remove tutorial menu listeners
			for k, v in pairs(tutorial_battles) do
				core:remove_listener(v.button_listener_id)
			end

			core:add_listener(
                "intro_battle_button_clicked",
                "ComponentLClickUp",
                function(context) return context.string == button:Id() end,
                function(context)
                    self:enable_tutorial_menu_buttons(true)
                    cm:steal_escape_key(false)
                    button:Highlight(false, true, 0)
                    self:battle_started()
                    self.battle[self.battle_type].start()
                end,
                false
            )
		end
	...
	cm:callback(open_menu, 0.5)
end;


function intro_start_tutorial_battle_advice:battle_started()
...
-- Enable all buttons or disable all except the button for the selected tutorial battle
function intro_start_tutorial_battle_advice:enable_tutorial_menu_buttons(should_enable)
	local tutorial_menu = core:get_ui_root():SequentialFind(self.tutorial_menu_root)
	local battle_button = tutorial_menu:SequentialFind(self.battle[self.battle_type].button_id)
	local buttons_list = UIComponent(tutorial_menu:SequentialFind(self.battle[self.battle_type].button_id):Parent())

	local state = should_enable and "active" or "inactive"
	local child_count = buttons_list:ChildCount()

	for i = 0, child_count - 1 do
		UIComponent(buttons_list:Find(i)):SetState(state)
	end

	local button_back = tutorial_menu:SequentialFind("button_back")
	button_back:SetState(state)

	battle_button:SetState("active")
end;