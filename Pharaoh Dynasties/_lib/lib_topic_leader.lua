...
function topic_leader:set_shrink_target_uicomponent(uicomponent, x_offset, y_offset)
	...
	self.shrink_target_x = x_offset;
	self.shrink_target_y = y_offset;
...
--- @desc Returns the topic leader @uicomponent, or <code>false</code> if it's not currently created.
--- @r @uicomponent topic leader
function topic_leader:get_uicomponent()
	return find_uicomponent("topic_leader_" .. self.name);
end;
...
function topic_leader:start(error_if_already_exists)
	...
	-- Start a listener for a message to destroy all topic leaders
	core:add_listener(
		self.name .. "_hide",
		"ScriptEventHideAllTopicLeaders",
		true,
		function()
			self:hide()
		end,
...
function topic_leader:shrink()
	...
	local uic_topic_leader = self:get_uicomponent();
	if not uic_topic_leader then
...
function topic_leader:hide()
	...
	core:remove_listener(self.name .. "_hide");

	core:get_tm():remove_real_callback(self.name);
end;