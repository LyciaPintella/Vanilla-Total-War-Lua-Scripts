...
local is_campaign = 	__game_mode == __lib_type_campaign;
local is_battle = 		__game_mode == __lib_type_battle;
local is_frontend = 	__game_mode == __lib_type_frontend;
local is_autotest = 	common.is_autotest()

-- Forward declarations for event system
local event_callback;
local protected_event_callback;
local unprotected_event_callback;
...
core_object = {
	...
	-- progress on loading screen dismissed re-entrancy guard
	progress_on_loading_screen_dismissed_reentrancy_count = 0,
	...
	registered_overlay_movies = {},
	active_pointers = {},
	active_pointers_showing = {},

	active_scripted_tour = false,
...
-- set core_object to be of custom type TYPE_CORE
set_class_custom_type_and_tostring(core_object, TYPE_CORE);



local function console_execute_command(_text)
    local text = _text;
	local func, err = loadstring(text);
	
	if not func then 
		script_error("ERROR: console attempted to run a script command but an error was reported when loading the command string into a function. Command and error will follow this message.");
		out("Command:");
		out(text);
		out("Error:");
		out(err);
	...
	if not ok then 
		script_error("ERROR: console attempted to run a script command but an error was reported when executing the function. Command and error will follow this message.");
		out("Command:");
		out(text);
		out("Error:");
		out(result);
...
function core_object:new()
	...
	-- set 'c' to derive from self ('core_object')
	...
	-- If the tweaker FORCE_PROTECTED_EVENT_CALLBACKS is set to true, or a variable FORCE_PROTECTED_EVENT_CALLBACKS is defined, then we use protected_event_callback() 
	-- to handle callbacks - this will catch the error, throw a script_error(), and log it in campaign.
	-- Otherwise we use unprotected_event_callback() which will allow scripts to fail. We do this to allow these errors to be logged in autotests.
	if c:is_tweaker_set("FORCE_PROTECTED_EVENT_CALLBACKS") or FORCE_PROTECTED_EVENT_CALLBACKS then
		event_callback = protected_event_callback;
	else
		event_callback = unprotected_event_callback;
	end;
	...
	c:add_listener(
			"core_console_command_execution",
			"LuaExecutionInConsole", 
			true,
			function(context)
				out("LuaExecutionInConsole:\n" .. context.string)
				console_execute_command(context.string);
			end,
			true
		);
	...
	-- store what the previous game mode was, and then set it to what this game mode is
	c.previous_game_mode = c.svr:LoadPersistentString("__previous_game_mode");

	local current_game_mode;
	if is_campaign then
		current_game_mode = "campaign";
	elseif is_battle then
		current_game_mode = "battle";
	elseif is_frontend then
		current_game_mode = "frontend";
	elseif is_autotest then
		current_game_mode = "autotest";
	end;

	c.current_game_mode = current_game_mode;
	c.svr:SavePersistentString("__previous_game_mode", current_game_mode);		-- this will be read by core when the script next reloads
...
--- @function is_autotest
--- @desc Returns whether this is actually an autotest script environment.
--- @return @boolean is autotest
function core_object:is_autotest()
	return is_autotest;
end;


--- @function get_current_game_mode
--- @desc Returns a @string indicating the current game mode - currently either <code>campaign</code>, <code>battle</code>, <code>frontend</code> or <code>autotest</code>.
--- @return @string current game mode
function core_object:get_current_game_mode()
...
----------------------------------------------------------------------------
--- @section Previous Game Mode
--- @desc Functions that return what mode the previous game environment was. For example, if a campaign was quit back to frontend the previous game environment would be <code>campaign</code>, or f a custom battle is loaded from the frontend the previous game environment would be <code>frontend</code>.
----------------------------------------------------------------------------


--- @function previous_game_mode_was_campaign
--- @desc Returns whether the game environment before the current game environment was campaign.
--- @return @boolean previous game mode was campaign
function core_object:previous_game_mode_was_campaign()
	return self.previous_game_mode == "campaign";
end;


--- @function previous_game_mode_was_battle
--- @desc Returns whether the game environment before the current game environment was battle.
--- @return @boolean previous game mode was battle
function core_object:previous_game_mode_was_battle()
	return self.previous_game_mode == "battle";
end;


--- @function previous_game_mode_was_frontend
--- @desc Returns whether the game environment before the current game environment was frontend.
--- @return @boolean previous game mode was frontend
function core_object:previous_game_mode_was_frontend()
	return self.previous_game_mode == "frontend";
end;


--- @function get_previous_game_mode
--- @desc Returns a @string indicating the current game mode - currently either <code>campaign</code>, <code>battle</code>, <code>frontend</code>, <code>autotest</code>, or a blank string if the current environment is the first game environment in the session (e.g. we're in the frontend after launching the game).
--- @return @string previous game mode
function core_object:get_previous_game_mode()
	return self.previous_game_mode;
end;
...
function core_object:load_mods(...)
	...
	for i = 1, arg.n do
		...
		local file_str = common.filesystem_lookup(path, "*.lua");
...
function core_object:set_float_preference(preference_name, value)
	if not validate.is_string(preference_name) or not validate.is_number(value) then
		return false;
	end;
...
function core_object:cache_float_preference(preference_name)
	...
	if validate.is_number(value) then
		self.cached_preferences[preference_name] = value;
		return value;
	end;
end;
...
--- @desc A fullscreen highlight is an effect where the screen is masked out with a semi-opaque layer on top, apart from a window in the middle through which the game interface can be seen. This can be used by tutorial scripts to draw the player's attention to a particular part of the screen. The fullscreen highlight prevents the player from clicking on those portions of the screen that are masked off.
--- @desc A fullscreen highlight effect may be established around a supplied set of components with @core:show_fullscreen_highlight_around_components. Once established, a fullscreen highlight effect must later be removed with @core:hide_fullscreen_highlight.
--- @desc This fullscreen highlight functionality wraps the @uicomponent:FullScreenHighlight function. It's recommended to use this wrapper rather than calling the @uicomponent function directly.
----------------------------------------------------------------------------

-- Destroys any fullscreen highlight
local function destroy_fullscreen_highlight(immediately)
	local root = core:get_ui_root()
	for i = 0, root:ChildCount() - 1 do
		local uic_child = UIComponent(root:Find(i));
		
		if uic_child:CallbackId() == "FullscreenHighlight" then
	
		if immediately then
				uic_child:Destroy();
		else
				uic_child:TriggerAnimation("destroy");
		end;
		end
	end;
end;
...
--- @desc Shows a fullscreen highlight around multiple components at the same time. An invisible dummy uicomponent is created, and positioned/sized so that it extends from the top-left corner to the bottom-right corner of the supplied uicomponents. Therefore, the window in the fullscreen highlight will reveal all of the supplied components.
--- @desc Once established, this highlight must later be removed with @core:hide_fullscreen_highlight.
--- @desc An integer padding value must also be supplied, which specifies how much additional visual padding to give the components shown in the window. The higher the supplied value, the bigger the visual window around the supplied components.
--- @desc The function also supports showing text from the <code>random_localisation_strings</code> database table on the fullscreen highlight while it's being displayed.
--- @p number padding, Padding value, must be 0 or greater
--- @p [opt=nil] string highlight text key, Key of text to show on the fullscreen highlight, if text is desired. If set, this should be the key of an entry from the <code>random_localisation_strings</code> database table. @nil or an empty string can be supplied to specify that no text should be shown.
--- @p [opt=false] boolean allow interaction, Allow the components shown in the central fullscreen highlight window to be interacted with while the highlight is active.
--- @p ... uicomponent list, Vararg (variable number of arguments) of @uicomponent objects. The components may also be specified here as either @string names, in which case they will be looked up from the ui root, or as @table paths of @string names, in which case the paths will be passed to @global:find_uicomponent_from_table in order to look up the intended uicomponent. See the examples given below.
--- @p_long_desc The uicomponents can be specified in any order - which one is top-left and which one is bottom-right will be computed automatically.
--- @new_example usage of show_fullscreen_highlight_around_components with uicomponent specifiers
--- @example local uic_example_top_left = find_uicomponent("parent_panel", "example_top_left")
--- @example local uic_example_bottom_right = find_uicomponent("parent_panel", "example_bottom_right")
--- @example local uic_example_additional_component_1 = find_uicomponent("parent_panel", "example_additional_1")
--- @example local uic_example_additional_component_2 = find_uicomponent("parent_panel", "example_additional_2")
--- @example 
--- @example core:show_fullscreen_highlight_around_components(
--- @example 	25,							-- 25 px padding
--- @example 	nil,						-- no text
--- @example 	true,						-- allow interaction
--- @example 	-- list of uicomponents
--- @example 	uic_example_top_left,
--- @example 	uic_example_bottom_right,
--- @example 	uic_example_additional_component_1,
--- @example 	uic_example_additional_component_2,
--- @example )
--- @new_example usage of show_fullscreen_highlight_around_components with string specifiers
--- @example core:show_fullscreen_highlight_around_components(
--- @example 	0,							-- no padding
--- @example 	"example_rls_text_key",		-- text key from "random_localisation_strings" database table
--- @example 	false,						-- prevent interaction
--- @example 	-- list of uicomponent names: they will be looked up from the ui root, so will need to be unique
--- @example 	"unique_component_name_1",
--- @example 	"unique_component_name_2",
--- @example 	"unique_component_name_3"
--- @example )
--- @new_example usage of show_fullscreen_highlight_around_components with table path specifiers
--- @example core:show_fullscreen_highlight_around_components(
--- @example 	100,		-- 100px padding
--- @example 	nil,		-- no text
--- @example 	true,		-- allow interaction
--- @example 	-- list of uicomponent paths: they will be looked up from the ui root
--- @example 	{"grandparent", "parent", "unique_component_name_1"},
--- @example 	{"grandparent", "parent", "unique_component_name_2"},
--- @example 	{"grandparent", "parent", "unique_component_name_3"}
--- @example )
function core_object:show_fullscreen_highlight_around_components(padding, highlight_text, allow_window_interaction, ...)
	...
	-- destroy any existing fullscreen highlight
	destroy_fullscreen_highlight(false);

	local ui_root = self:get_ui_root();
	
	for i = 1, arg.n do
		local current_component_identifier = arg[i];
		local current_component;
		
		if is_uicomponent(current_component_identifier) then
			current_component = current_component_identifier;
			if not current_component:IsValid() then
				script_error("ERROR: show_fullscreen_highlight_around_components() called but parameter " .. i .. " in supplied list is [" .. tostring(current_component_identifier) .. "], not a valid uicomponent");
				return false;
			end
		
		elseif is_string(current_component_identifier) then
			current_component = find_uicomponent(current_component_identifier);

			if not current_component then
				script_error("ERROR: show_fullscreen_highlight_around_components() called but argument [" .. i .. "] in supplied list is string [" .. tostring(current_component) .. "], but a lookup using this as a component name failed to find a uicomponent");
				return false;
			end;

		elseif is_table(current_component_identifier) then
			current_component = find_uicomponent_from_table(ui_root, current_component_identifier);
			
			if not current_component then
				script_error("ERROR: show_fullscreen_highlight_around_components() called but argument [" .. i .. "] in supplied list is table [" .. tostring(current_component) .. "], but this is not a valid path to a uicomponent");
				return false;
			end;

		else
			script_error("ERROR: show_fullscreen_highlight_around_components() called but parameter " .. i .. " in supplied list is [" .. tostring(current_component_identifier) .. "], not a uicomponent, or a string or table of strings that can be used to look up a uicomponent");
			return false;
		end;
...
--- @p [opt=false] @boolean immediate, Hide the fullscreen highlight effect immediately instead of over a short period.
function core_object:hide_fullscreen_highlight(immediate)
	destroy_fullscreen_highlight(immediate);
...
--- @p string listener name, String name for this listener, by which it may be later removed if necessary with @core:remove_listener.
...
--- @p function target callback, Target callback to call when the conditional test passes.
--- @p @boolean persistent, Listener persists after target callback called.
--- @p [opt=false] @boolean prevent duplicates, Prevents adding listeners with a duplicate name. If this argument is <code>true</code>, and there is already a listener with the same name listening for the same event, this new listener will not be added.
function core_object:add_listener(new_name, new_event, new_condition, new_callback, new_persistent, prevent_duplicates)
	...
	if prevent_duplicates == true then
		local event_listeners = self.event_listeners[new_event]
		if event_listeners then
			for _, listener in ipairs(event_listeners) do
				if listener.name == new_name then
					out("WARNING: event_handler:add_listener() attempted to add a listener with a duplicate ID: '" .. new_name .. "' on the same event '" .. new_event .. "'.")
					return false
				end
			end
		end
...
local last_error_msg, last_traceback;

local function error_handler(err)
	last_error_msg = err;
	last_traceback = debug.traceback("", 2);
end;


-- Calls an event callback in protected mode and acts on the result. This function will be used to process event callbacks if the FORCE_PROTECTED_EVENT_CALLBACKS tweaker is set.
-- This function will catch lua errors, throw a script_error(), and in campaign the error will be logged in the savegame. It is used during game development.
function protected_event_callback(eventname, listener_record, context, call_event_condition)
	local condition_result, call_succeeded;
	...
	if is_campaign then
		cm:log_event_error(eventname, last_error_msg, last_traceback, listener_record.callstack, call_event_condition);
...
function unprotected_event_callback(eventname, listener_record, context, call_event_condition)
	if call_event_condition then
...
function core_object:event_triggered(eventname, context)	
	-- self:list_events();
	...
	for i = 1, #listeners do
		...
		-- event_callback() is set in core_object:new() and will be either protected_event_callback() or unprotected_event_callback()
		if current_listener.condition == true or event_callback(eventname, current_listener, context, true) then			
	...
	if not self.show_performance_report then
		for i = 1, #callbacks_to_call do
			-- event_callback() is set in core_object:new() and will be either protected_event_callback() or unprotected_event_callback()
			event_callback(eventname, callbacks_to_call[i], context, false);
		end;
	else
...
-- core:event_triggered() when the event is triggered
function core_object:attach_to_event(eventname)
	...
	self.add_func(eventname, function(context) self:event_triggered(eventname, context) end);
end;
...
function core_object:trigger_custom_event(event, context_data)
	...
	if context_data and not is_table(context_data) then
...
--- @function register_active_pointer_showing
--- @desc Registers that a particular active pointer is currently showing or not. Not to be called externally.
--- @p @active_pointer active pointer
function core_object:register_active_pointer_showing(ap, is_showing)
	if not validate.is_activepointer(ap) then
		return false;
	end;

	if is_showing then
		self.active_pointers_showing[ap.name] = ap;
	else
		self.active_pointers_showing[ap.name] = nil;
	end;
end;


--- @function is_any_active_pointer_showing
--- @desc Returns whether any active pointers are currently being displayed.
--- @return @boolean active pointer is showing
function core_object:is_any_active_pointer_showing()
	return not not next(self.active_pointers_showing);
end;


--- @function get_showing_active_pointer_names
--- @desc Returns a sorted indexed table of the names of active pointers that are currently showing. There will usually be only zero or one active pointers being displayed at any given time, but in certain circumstances there could be more. The table will be sorted so that the active pointer names are in alphabetical order.
--- @return @table names of active pointers currently showing
function core_object:get_showing_active_pointer_names()
	local retval = {};
	
	for ap_name, ap in pairs(self.active_pointers_showing) do
		table.insert(retval, ap_name);
	end;
...
--- @function hide_all_topic_leaders
--- @desc Immediately hide any @topic_leader objects currently visible.
function core_object:hide_all_topic_leaders()
	self:trigger_event("ScriptEventHideAllTopicLeaders");
end;
...
--- @desc Immediately close any open info overlay.
function core_object:close_info_overlays()
	self:trigger_event("ScriptEventCloseInfoOverlays");
end;
...
--- @function get_active_scripted_tour
--- @desc Returns the scripted tour or navigable tour object that's currently playing. If no tour is currently playing then <code>false</code> is returned.
--- @r @scripted_tour active tour
function core_object:get_active_scripted_tour()
	return self.active_scripted_tour;
end;


-- For internal use, for scripted tours to register/deregister themselves with core
function core_object:register_active_scripted_tour(tour)
	self.active_scripted_tour = tour;
end;
...
-- internal function to increment guard counter before calling the progression callback, and decrement it afterwards
function core_object:progress_on_loading_screen_dismissed_reentrancy_guard(callback)
	self.progress_on_loading_screen_dismissed_reentrancy_count = self.progress_on_loading_screen_dismissed_reentrancy_count + 1;
	callback();
	self.progress_on_loading_screen_dismissed_reentrancy_count = self.progress_on_loading_screen_dismissed_reentrancy_count - 1;
end;
...
function core_object:progress_on_loading_screen_dismissed(callback, suppress_wait_for_animation)
	...
	-- If progress_on_loading_screen_dismissed_reentrancy_count is greater than zero then we are already in the process of progressing when a loading screen is dismissed, and 
	-- have re-entered the function after calling a progression callback. In this case, proceed straight away, as the loading screen may still be visible and testing for it
	-- would otherwise cause this function to wait indefinitely
	if self.progress_on_loading_screen_dismissed_reentrancy_count > 0 then
		out("=== progress_on_loading_screen_dismissed() called, we are already in the process of progressing from a loading screen being dismissed - continuing immediately");
		self:progress_on_loading_screen_dismissed_reentrancy_guard(callback);
		return;
	end;
	...
	local force_wait_for_loading_screen_dismissed_in_campaign = is_campaign and not cm:is_game_running();
...
--- @function is_loading_screen_being_dismissed_this_tick
--- @desc Returns whether the game is in the process of progressing after a loading screen has been dismissed. This will <code>true</code> if a @core:progress_on_loading_screen_dismissed callback is currently in the callstack, <code>false</code> otherwise. This test is provided because at the point where the progress callback is called, testing the loading screen visibility is not a reliable indicator of whether the loading screen has been dismissed (it is visible, but has been dismissed and is about to disappear).
--- @r @boolean loading screen is currently being dismissed
function core_object:is_loading_screen_being_dismissed_this_tick()
	return self.progress_on_loading_screen_dismissed_reentrancy_count > 0;
end;
...
function core_object:restore_tooltip_for_component_state(uic, state)
	...
	uic:SetTooltipText(cached_state_text, "STEVE_VIRGO_TODO", false);
