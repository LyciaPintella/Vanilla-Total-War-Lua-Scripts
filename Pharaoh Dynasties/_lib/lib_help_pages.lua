...
-- set help_page_manager to be of custom type TYPE_HELP_PAGE_MANAGER
...
function help_page_manager:new()
	...
	-- set 'hm' to derive from self ('help_page_manager')
	...
	core:add_listener(
		...
		"ComponentLClickUp",
		function(context) return context.string == "button_info_pages" or context.string == "button_info" end,
		function(context)
...
function help_page_manager:get_index_button()	
	local uic = find_uicomponent(core:get_ui_root(), "menu_bar", "button_tutorial_menu");
...
-- clears the index page and regenerates it. No need to call this more than one time
function help_page_manager:generate_index_page()
	self:get_uicomponent():InterfaceFunction("generate_index_page");
end;

-- Called when a link is clicked to instantly update its color in the index page
function help_page_manager:update_index_page_link_visited(link)
	-- local uic_help_panel = self:get_uicomponent();
	-- local uic_list = find_uicomponent(uic_help_panel, "index_listview", "list_box");
	-- local parser = self.parser;

	-- local link_key = string.sub(link, #"script_link_" + 1)
	-- local uic = find_uicomponent(uic_list, link_key)

	-- if uic then
	--     local unparsed_text, source = uic:GetStateText();
	--     -- out("unparsed_text: " .. unparsed_text)
	--     local parsed_text = unparsed_text:gsub(parser.link_colour_str, parser.visited_link_colour_str)
...
function help_page_manager:show_panel(value)
	...
	if value then
		if not tutorial_menu:Visible() then
			tutorial_menu:InterfaceFunction("toggle_tutorial_menu")
		end

		tutorial_menu:InterfaceFunction("show_help_pages")
		self.panel_is_visible = true;
	else
		if tutorial_menu:Visible() then
			tutorial_menu:InterfaceFunction("toggle_tutorial_menu")
		end
...
function help_page_manager:page_finished_displaying()
	...
	-- self:dock_help_panel_if_appropriate(); -- in Troy the position of the help panel is static
...
function help_page_manager:display_text_line(key, uic_state)
	local uic_help_panel = self:get_uicomponent();

	uic_help_panel:InterfaceFunction("add_info_text_entry", key, uic_state);
	
	return find_uicomponent(self.uic_help_panel, key);
end;

-- displays a line of text and returns a link to the newly created uicomponent for the line
function help_page_manager:display_unit_line(unit_key)
	local uic_help_panel = self:get_uicomponent();

	uic_help_panel:InterfaceFunction("add_unit_entry", unit_key);
	
	return find_uicomponent(self.uic_help_panel, unit_key .. "_line");
end;

-- displays a line of text and returns a link to the newly created uicomponent for the line
function help_page_manager:display_building_line(building_level_key, building_culture_variant_key, faction_key)
	local uic_help_panel = self:get_uicomponent();

	uic_help_panel:InterfaceFunction("add_building_entry", building_level_key, building_culture_variant_key, faction_key);
	
	return find_uicomponent(self.uic_help_panel, string.format("%s:%s", building_level_key, building_culture_variant_key));
end;
...
function help_page_manager:help_page_index_generated()
	...
	for i = 0, uic_list:ChildCount() - 1 do
		...
		local text = UIComponent(uic_entry:Find("text"));
		
		local unparsed_text, text_source = text:GetStateText();
		local parsed_text = parser:parse_for_links(unparsed_text);
		
		text:SetStateText(parsed_text, text_source);
	end;
	
	if not self.next_page_request_is_from_history then
		-- self:add_help_page_to_history("__index"); -- in Troy the index page has a dedicated place
	end;
...
function help_page_manager:dock_help_panel_if_appropriate()
	...
	if __game_mode == __lib_type_campaign then
		...
		if (not related_panel_open or self:panel_should_cede_priority(related_panel_open)) and self.panel_is_docked then			
			...
			local uic_tabgroup_parent = find_uicomponent(core:get_ui_root(), "panel_buttons_tab_sidemenu");
...
function help_page_manager:enable_help_panel_index_button(value)
	...
	if value == false then
		...
		if self.help_panel_index_button_tooltip_text then
			uic:SetTooltipText(self.help_panel_index_button_tooltip_text, "STEVE_VIRGO_TODO", true);
		end;
	end;
end;
...
function help_page_manager:enable_menu_bar_index_button(value)
	...
	if value == false then
		...
		if self.menu_bar_index_button_tooltip_text then
			uic:SetTooltipText(self.menu_bar_index_button_tooltip_text, "STEVE_VIRGO_TODO", true);
		end;
	end;
end;
...
function help_page_manager:expand_index_page(script_link)
	local index_page_entry = string.sub(script_link, #"script_link_" + 1)
	self:get_uicomponent():InterfaceFunction("expand_index_page", index_page_entry)
end
...
function help_page_manager:start_hyperlink_click_listener()
	...
	core:add_listener(
		...
		true,
		function(context)
			local link = context.string
			out.help_pages("ComponentLinkClicked: " .. link);

			if __help_page_mappings and __help_page_mappings[link] then
				link = __help_page_mappings[link]
			end

			self:check_hyperlink_click(link);

			local index_listview = self:get_uicomponent():SequentialFind("index_listview")
			local entry_id = string.sub(link, #"script_link_" + 1)
			local index_entry = index_listview:SequentialFind(entry_id)
			local is_link_on_index_page = index_entry and index_entry:Address() == context.component

			if not is_link_on_index_page then
				self:expand_index_page(link)
			end
		end,
		true
	);
...
function help_page_manager:info_button_clicked(uicomponent)
	...
	for i = 1, #info_button_to_page_map do
		...
		if uicomponent_descended_from(uicomponent, current_mapping.parent_id) then
			if not current_mapping.component_test or current_mapping.component_test(uicomponent) then
				self:check_hyperlink_click(current_mapping.help_page);
				self:expand_index_page(current_mapping.help_page)
				return;
			end;
		end;
	end;
...
function hpr_building(building_level_key, building_culture_variant_key, faction_key)
	local retval = {};
	retval.key = "not_used";
	retval.building_level_key = building_level_key;
	retval.building_culture_variant_key = building_culture_variant_key;
	retval.faction_key = faction_key;
	retval.is_building = true;
	return retval;
end;

function hpr_unit(unit_key)
	local retval = {};
	retval.key = "not_used";
	retval.unit_key = unit_key;
	retval.is_unit = true;
	return retval;
end;
...
-- set help_page to be of custom type TYPE_HELP_PAGE, with a custom tostring() response
...
function help_page:new(link, ...)
	...
	-- set 'hp' to derive from self ('help_page')
...
function help_page:link_clicked()
	...
	if not hpm:is_help_page_history_empty() and hpm:get_last_help_page() == link and not hpm:forward_help_pages_exist() and not hpm.next_page_request_is_from_history then
		if hpm:is_panel_visible() then
			-- hpm:show_panel(false); -- In Troy we don't have the index button. If the player double cliks on a link, we hit this case which causes a bug where the help_panel is being hidden
			return;
		else
	...
	if not common.get_advice_history_string_seen(link) then
		hpm:update_index_page_link_visited(link)
...
function help_page:append_help_page_record(record)
	...
	if not record.game_guide or (record.game_guide and hpm:display_game_guide_links()) then
		...
		local uic_state = get_record_state(record, use_small_text)
		-- display the line
		local uic_entry = nil
		if record.is_unit then
			uic_entry = hpm:display_unit_line(record.unit_key)
		elseif record.is_building then
			uic_entry = hpm:display_building_line(record.building_level_key, record.building_culture_variant_key, record.faction_key)
		else
			uic_entry = hpm:display_text_line(key, uic_state)
		end
		...
		-- Now unit record uses totally different component with secial callbacks, so we don't want to mess with it
		if not (record.is_unit or record.is_building) then

			local localised_text, source = ""
			
			if record.show_links then
				localised_text, source = uic_entry:GetStateText();
...
-- function help_page:set_record_state(uic, record, state)
--
...
-- 		if record.is_building then
-- 			uic:SetImagePath(record.image_path, uic:NumImages() - 1)
-- 		end
-- 	end;
-- end;
...
-- set tooltip_listener to be of custom type TYPE_TOOLTIP_LISTENER, with a custom tostring() response
...
function tooltip_listener:new(link, mouseon_callback, mouseoff_callback)
	...
	-- set 'tl' to derive from self ('tooltip_listener')
...
-- set tooltip_patcher to be of custom type TYPE_TOOLTIP_PATCHER, with a custom tostring() response
...
function tooltip_patcher:new(layout_key)
	...
	-- set 'tp' to derive from self ('tooltip_patcher')
...
-- Setup campaign or battle help pages
function setup_help_pages(help_pages, mappings, help_page_mappings)
	out.help_pages("");
	out.help_pages("***");
	out.help_pages("*** setup_help_pages() called");
	out.help_pages("***");
	...
	for idx, hp in pairs(help_pages) do
		local text = {}

		for _, entry in ipairs(hp.text) do
			local hpr

			if entry[1] == "header" then
				hpr = hpr_header(entry[2])
			elseif entry[1] == "normal" then
				hpr = hpr_normal(entry[2])
			elseif entry[1] == "bulleted" then
				hpr = hpr_bulleted(entry[2])
			elseif entry[1] == "linked" then
				hpr = hpr_linked(entry[2])
			elseif entry[1] == "game_guide" then
				hpr = hpr_game_guide(entry[2])
			elseif entry[1] == "building" then
				if not core:is_campaign() then
					script_error("help page building entries can be used only in Campaign")
				else
					local building_level = entry[2]
					local building_culture_variant = entry[3]
					local faction = is_string(entry[4]) and entry[4] or ""

					if entry.faction_override then
						local local_faction_key = cm:get_local_faction_name(true)

						if entry.faction_override[local_faction_key] then
							faction = local_faction_key
							building_level = entry.faction_override[local_faction_key][1]
							building_culture_variant = entry.faction_override[local_faction_key][2]
						end
					end

					hpr = hpr_building(building_level, building_culture_variant, faction)
				end
			elseif entry[1] == "unit" then
				hpr = hpr_unit(entry[2])
			end

			if hpr then
				table.insert(text, hpr)
			end
		end

		hp.page = help_page:new("script_link_" .. idx, unpack(text))

		parser:add_record(idx, "script_link_" .. idx, "tooltip_" .. idx)
		-- hp.tp = tooltip_patcher:new("tooltip_" .. idx)
		-- hp.tp:set_layout_data("tooltip_title_and_text", "ui_text_replacements_localised_text_hp_title_" .. idx, "ui_text_replacements_localised_text_hp_description_" .. idx)


		if hp.tooltip_listener then
			hp.tl = tooltip_listener:new("tooltip_" .. idx, hp.tooltip_listener.on_mouse_on, hp.tooltip_listener.on_mouse_off)
		end
	...
	if mappings then
		for _, mapping in ipairs(mappings) do
			...
			hpm:register_help_page_to_info_button_mapping(mapping.help_page, mapping.panel, mapping.condition)
		end
	end

	hpm:generate_index_page() -- help pages loading optimization

	__help_page_mappings = help_page_mappings
	--
	...
	core:add_listener(
		"ui_reloaded_help_pages", 
		"UIReloaded", 
		true, 
		function(context)
			__help_page_manager.uic_help_panel = nil
			__link_parser = nil
			__help_page_mappings = help_page_mappings
			setup_help_pages(help_pages, mappings)
		end,
		false
	)
end

__help_page_mappings = nil