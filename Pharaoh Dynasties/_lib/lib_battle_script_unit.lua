...
-- set script_unit to be of custom type TYPE_SCRIPT_UNIT, with a custom tostring() response
...
--- @desc New script unit objects may nevertheless be created using @script_unit:new.
----------------------------------------------------------------------------


--- @function new
--- @desc Creates a new script unit from a supplied @battle_unit or @battle_army object. An reference name may be passed which is used for debug output (this is mandatory if a @battle_army is supplied, as it is used to look up the unit).
--- @p @battle_unit unit, Unit object to be contained. A @battle_army may also be supplied, in which case @script_unit:new_from_army is invoked.
--- @p [opt=nil] @string name, Reference name for the unit.
--- @r script_unit
function script_unit:new(unit_or_army, unit_ref)
	...
	if is_unit(unit_or_army) then
		return script_unit:new_impl(unit_or_army, unit_ref);
	elseif is_army(unit_or_army) then
		return script_unit:new_from_army(unit_or_army, unit_ref);
	end;

	script_error("ERROR: script_unit:new() called but supplied creation object [" .. tostring(unit_or_army) .. "] is not a valid unit or object");
	return false;
end;


--- @function new_from_army
...
--- @r script_unit
function script_unit:new_from_army(army, unit_ref)
	-- type check our startup parameters
	if not is_army(army) then
		script_error("ERROR: script_unit:new_from_army() called but supplied army [" .. tostring(army) .. "] is not a valid army object");
		return false;
	end;
	
	if not is_string(unit_ref) then
		if not is_number(unit_ref) then
			script_error("ERROR: script_unit:new_from_army() called but supplied unit reference " .. tostring(unit_ref) .. " is not a string or a number");
			return false;
		end;
	end;
	...
	if not unit_found then
		script_error("ERROR: script_unit:new_from_army() called but no unit with name or reference [" .. unit_ref .. "] could be found in army [" .. tostring(army) .. "]");
		return false;
	end;
...
function script_unit:new_impl(unit, unit_ref)
	...
	-- if the battle manager exists and script_units are loaded then it should already have created a handle to this scriptunit during construction, so let's return that
	if bm and bm.script_units_loaded then
...
function script_unit:take_control()
	find_uicomponent(core:get_ui_root(), "hud_battle"):InterfaceFunction("deselect_all")
...
function script_unit:set_invincible_for_time_proportion(time_proportion)
...
--- @desc Makes the subject unit invincible/fearless if it's not already routing, or reverses the effect
...
--- @p [opt=true] boolean invincibility, Set to true to turn the unit invincible, false to reverse the invincibility
function script_unit:invincible_if_standing(should_release, invincibility)
	if is_nil(invincibility) then
		invincibility = true
	end
...
function script_unit:prevent_rallying_if_routing(perpetual)
	...
	bm:out(self.name .. ":prevent_rallying_if_routing() called, perpetual flag is " .. tostring(perpetual));
...
--- @p [opt=false] boolean should release, Set to true to release script control of this unit after <code>morale_behavior_fearless</code> makes a change. Set this primarily if the unit is under player control.
function script_unit:morale_behavior_fearless(should_release)
	local should_release = should_release or false;
...
--- @p [opt=true] boolean ignore_pause, Ignore action when game is paused.
function script_unit:kill_proportion(proportion, preserve, hide_bodies, ignore_pause)
	local unit = self.unit;
	...
	ignore_pause = ignore_pause == nil and true or false
	...
	unit:kill_number_of_men(unit:initial_number_of_men() * proportion_to_kill, hide_bodies, ignore_pause);
end;
...
-- set script_units object to be of custom type TYPE_SCRIPT_UNITS, with a custom tostring() response
...
function script_units:new(new_name, ...)	
	...
	-- set 'su' to derive from self ('script_units')
	set_object_class(su, self);
...
--- @function is_any_firing_missiles
--- @desc Is any of the @script_unit objects in the collection firing missiles.
--- @return boolean is_any_firing_missiles
function script_units:is_any_firing_missiles()
	local sunit_list = self.sunit_list;

	for i = 1, #sunit_list do
		if sunit_list[i].unit:is_firing_missiles() then
			return true;
		end;	
	end;
...
--- @function are_all_firing_missiles
--- @desc Are all of the @script_unit objects in the collection firing missiles.
--- @return boolean is_any_firing_missiles
function script_units:are_all_firing_missiles()
	local sunit_list = self.sunit_list;

	for i = 1, #sunit_list do
		if not sunit_list[i].unit:is_firing_missiles() then
			return false;
		end;	
	end;
...
function script_units:deploy_at_random_intervals(min_units, max_units, min_period, max_period, force_output, spawn_first_wave_immediately)
	...
	self:enqueue_deploy_at_random_intervals_action(min_units, max_units, min_period, max_period, force_output, spawn_first_wave_immediately)
end;


function script_units:enqueue_deploy_at_random_intervals_action(min_units, max_units, min_period, max_period, force_output, spawn_first_wave_immediately)
	...
	if self.is_debug or force_output then
		self.bm:out(self.name .. ":deploy_at_random_intervals() waiting for " .. wait_time .. " ms.");
	end;
	bm:callback(
		function() 
...
function script_units:deploy_at_random_intervals_action(min_units, max_units, min_period, max_period, force_output)
	...
	if self.is_debug or force_output then
		if number_to_deploy == 1 then
			bm:out(self.name .. ":deploy_at_random_intervals() has deployed 1 reinforcement unit");
		else
			bm:out(self.name .. ":deploy_at_random_intervals() has deployed " .. number_to_deploy .. " reinforcement units");
		end;
	end;
	
	-- deploy more reinforcements in the future if we still have reinforcements to deploy
	if not is_last_pass then
		self:enqueue_deploy_at_random_intervals_action(min_units, max_units, min_period, max_period, force_output, false);
	end;
end;
...
function script_units:kill_aura_action(enemy_sunits, range, proportion_per_second)
	...
	for i = 1, #casualty_list do
		...
		current_enemy_sunit:kill_proportion(proportion_per_second, false, false, false);
	end;
