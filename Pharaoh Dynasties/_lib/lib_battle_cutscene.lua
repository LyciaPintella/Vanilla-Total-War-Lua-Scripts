...
--- @class cutscene Battle Cutscenes
...
cutscene = {
	...
	-- advisor_uicomponents_to_make_visible_on_end = {},
...
-- set cutscene object to be of custom type TYPE_CUTSCENE_MANAGER, with a custom tostring() response
...
--- @p [opt=nil] @number duration, Cutscene duration in milliseconds. If nil or 0 is specified then the cutscene will be set to not end, and will only finish when skipped by the player or ended by an external process - see @cutscene:set_do_not_end.
...
function cutscene:new(name, players_army, cutscene_length, end_callback)
	...
	-- set 'c' to derive from self ('cutscene')
...
--- @return cutscene cutscene object
function cutscene:new_from_cindyscene(name, players_army, end_callback, cindy_scene, duration, blend_in_duration, blend_out_duration)
	...
	bm:cindy_preload(cindy_scene)

	c:action(function() bm:cindy_playback(cindy_scene, blend_in_duration, blend_out_duration) end, 0);
...
function cutscene:action(new_callback, new_delay, new_is_terminator)
	...
	if not self.do_not_end and new_delay > self.cutscene_length then
...
function cutscene:start()
	...
	self.advisor_uicomponents_to_make_visible_on_end = {};
	...
	if bm:is_benchmarking_mode() then
		...
		local advisor_control_buttons_list = get_all_advisor_controls();
		for i = 1, #advisor_control_buttons_list do
			local current_uic = advisor_control_buttons_list[i];

			if current_uic and current_uic:Visible(true) and current_uic:CurrentState() == "inactive" then
				current_uic:SetVisible(false);
				table.insert(self.advisor_uicomponents_to_make_visible_on_end, current_uic);
			end;
		end;
	...
	bm:callback(
		function()
			...
			if self.skip_cam_pos and self.skip_cam_target then
				bm:camera():move_to(self.skip_cam_pos, self.skip_cam_target, 0, false, 0, false);
			else
...
function cutscene:restore_camera_and_release(should_cut)
	...
	if restore_time == 0 then
		...
		bm:callback(function() self:release() end, (restore_time * 1000), self.name);
	end;
end;
...
function cutscene:release()
	...
	-- Undo the aide-de-camp suspension placed when the cutscene started
	...
	-- unhide the advisor close button if we should
	if not bm:is_benchmarking_mode() then
		for i = 1, #self.advisor_uicomponents_to_make_visible_on_end do
			self.advisor_uicomponents_to_make_visible_on_end[i]:SetVisible(true);
		end;
	end;
