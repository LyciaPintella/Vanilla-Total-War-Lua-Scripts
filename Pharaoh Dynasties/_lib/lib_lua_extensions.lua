...
--- @desc Compares two strings lexicographically byte by byte and returns the difference in the first byte that differs between the two, or zero if the two are equal. Hence a negative value if the first string precedes the second, a positive value if the second string precedes the first and zero if the two strings are equal.
--- @desc This can be used to compare unicode strings encoded in UTF-8, as UTF-8 is designed in a way that allows for this. See https://en.wikipedia.org/wiki/UTF-8 section "Comparison with other encodings", subsection "Sorting order".
--- @p @string first string to compare.
--- @p @string second string to compare.
--- @r @number comparison result
--- @example print(string.byte_compare("string2", "string1"))
--- @result 1
function string.byte_compare(str1, str2)
	local str1_byte_len = #str1	-- We want the raw byte length of the string,
	local str2_byte_len = #str2	-- We want the raw byte length of the string,
	local shorter_len = math.min(str1_byte_len, str2_byte_len)
	for i = 1, shorter_len do
		local str1_byte = str1:byte(i)
		local str2_byte = str2:byte(i)
		if str1_byte ~= str2_byte then
			return str1_byte - str2_byte
		end
	end
	if str1_byte_len < str2_byte_len then
		return str2:byte(str1_byte_len + 1)
	elseif str1_byte_len > str1_byte_len then
		return str1:byte(str2_byte_len + 1)
	else
		return 0
	end
end
...
--- @function byte_greater_than
--- @desc Compares two strings lexicographically byte by byte and returns whether the second string precedes the first.
--- @desc This can be used to compare unicode strings encoded in UTF-8, as UTF-8 is designed in a way that allows for this. See https://en.wikipedia.org/wiki/UTF-8 section "Comparison with other encodings", subsection "Sorting order".
--- @p @string first string to compare.
--- @p @string second string to compare.
--- @r @boolean second string precedes first string
--- @example print(string.byte_greater_than("string2", "string1"))
--- @result true
function string.byte_greater_than(str1, str2)
	return string.byte_compare(str1, str2) > 0
end


--- @function byte_less_than
--- @desc Compares two strings lexicographically byte by byte and returns whether the first string precedes the second.
--- @desc This can be used to compare unicode strings encoded in UTF-8, as UTF-8 is designed in a way that allows for this. See https://en.wikipedia.org/wiki/UTF-8 section "Comparison with other encodings", subsection "Sorting order".
--- @p @string first string to compare.
--- @p @string second string to compare.
--- @r @boolean first string precedes second string
--- @example print(string.byte_less_than("string2", "string1"))
--- @result false
function string.byte_less_than(str1, str2)
	return string.byte_compare(str1, str2) < 0
end
...
--- @function find
--- @desc Returns the position of the first occurrence of a supplied substring within the subject string. If the supplied substring is found within the subject string then the start and end character positions of the substring are returned. @nil is returned if the substring is not found.
--- @desc This is a modified version of the original lua language implementation of <code>find</code>, which does not support utf8 strings but which does support pattern matching. See the lua wiki for more information about lua patterns. See also @string:find_utf8 for an implementation that supports utf8 strings, but does not support pattern matching.
--- @desc <strong>However, please note:</strong> in our library implementation, pattern-matching is disabled by default. Supply <code>false</code> as the fourth boolean argument to turn it on. Alternatively, use @string:find_lua instead - this is the default string.find provided by the lua language.
...
--- @p [opt=true] @boolean disable pattern matching, Disables pattern matching if set to <code>true</code>. <strong>Unlike the default lua library implementation, this is <code>true</code> by default.</strong> Supply <code>false</code> here to enable pattern matching.
--- @r @number first character index
--- @r @number last character index
--- @example local str = "This is a test string"
--- @example print(string.find(str, "is"));
--- @result	3	4
--
--- @new_example Example using pattern matching
--- @example local str = "This is a test string"
--- @example print(string.find(str, "%s", nil, false));			-- find first space
--- @result	5	5
--
-- See all_scripted.lua for the wrapper function which turns pattern-matching off by default.
--


--- @function find_lua
--- @desc Direct exposure of the lua-provided <code>string.find</code> function, which returns the position of the first occurrence of a supplied substring within the subject string. If the supplied substring is found within the subject string then the start and end character positions of the substring are returned. @nil is returned if the substring is not found.
--- @desc This is the original lua language implementation of <code>find</code>, unaltered. This function does not support utf8 strings, but does support pattern matching, and unlike our implementation of <code>string.find</code> pattern-matching is turned on by default. See the lua wiki for more information about lua patterns.
--- @desc This function directly exposes the lua-language implementation of string.find. It's provided mainly for performance, as calling string.find goes through a wrapper redirect function.
--- @p @string string, Subject string.
--- @p @string substring, String pattern to search for.
--- @p [opt=1] @number start index, Position of character at which to start the search. Supply a negative number to specify a character from the end of the string, counting back.
...
--- @r @number last character index
--
--- @new_example Pattern matching enabled by default
--- @example local str = "This is a test string"
--- @example print(string.find(str, "%s"));						-- find first space
--- @result	5	5
--
-- See all_scripted.lua for the implementation of string.find_lua
--
...
--- @desc This implementation of <code>len</code> is provided by our game code to support utf8 strings. It does not support pattern matching - see @string:find for the original language implementation which does.
--- @p @string string, Subject string.
--- @p @string substring, String pattern to search for.
--- @p [opt=1] @number start index, Position of character at which to start the search. Supply a negative number to specify a character from the end of the string, counting back.
--- @r @number first character index
--- @r @number last character index
--- @example local str = "This is a test string"
--- @example print(string.find_utf8(str, "test"))
--- @result 11	14
...
--- @r @number length
--- @example str = "hello"
--- @example print(string.len(str))
--- @result 5


--- @function len_utf8
--- @desc Returns the number of characters in the supplied string. This implementation of <code>len</code> is provided by our game code and supports utf8 strings, see @string:len for the original language implementation.
--- @p @string input
--- @r @number length
--- @example str = "hi"
--- @example print(string.len_utf8(str))
--- @result 2
...
--- @desc This is the original lua language implementation of <code>sub</code>, which does not support utf8 strings. See @string:sub_utf8 for an implementation that does.
...
--- @function sub_utf8
--- @desc Returns a section of the supplied string, specified by start and (optionally) end character positions. The substring will include the characters specified by the start and end positions.
--- @desc This implementation of <code>sub</code> is provided by our game code, see @string:sub for the original language implementation. This function is slightly slower than @string:sub.
--- @p @string input, Subject string.
--- @p @number start index, Position of the starting character of the substring. If a negative number is specified then the function counts back from the end of the string.
--- @p [opt=nil] @number end index, Position of the end character of the desired substring. If omitted, then the end of the supplied string is used as the end of the substring. If a negative number is specified then the function counts back from the end of the string to find this character.
--- @new_example From character 11 until the end
...
--- @function_separator .
--- @page lua
--- @desc Tables are the only data type in lua that can be used to contain other data. As such, they are the only mechanism available for building complex data structures.
--- @desc Lua tables are associative arrays, which means they store sets of key/value pairs. A <code>value</code> in a table is stored in a record corresponding to a <code>key</code>, and that value may later accessed or modified using that same key.
--- @desc An example representation of a table:
--- @desc <table class="simple"><tr><th>Key</th><th>Value</th></tr>
--- @desc <tr><td><code>1</code></td><td>"first"</td></tr>
--- @desc <tr><td><code>2</code></td><td>"second place"</td></tr>
--- @desc <tr><td><code>3</code></td><td>"third place"</td></tr>
--- @desc <tr><td><code>4</code></td><td>"also ran"</td></tr>
--- @desc <tr><td><code>5</code></td><td>"also ran"</td></tr></table>
--- @desc If a value is written to a table at a key which is already associated with a value, then that previous value is overwritten, just like normal variables.
--- @desc Table keys are most commonly or usefully @number or @string values, but may be of any data type other than @nil. Values stored at keys within a table can be of any type, including @function or @table (allowing tables to be nested). If a value of @nil is stored at a particular key, then that is equivalent to deleting that key/value combination. If a key is looked up in a table at which no value has been stored, then @nil is returned.
--- @desc Values in a table may be accessed using <code>[]</code> square brackets, or by using the <code>.</code> accessor if the key is a string that contains no spaces, and where the first character of that string is not a numeric character. See the examples below:
--
--- @example -- Create an empty table
--- @example example_table = {}
--- @example 
--- @example -- Assign a string value to table t at a numeric key
--- @example example_table[5] = "5"
--- @example 
--- @example -- Assign a string value to table t at a numeric key
--- @example example_table[5] = "5"
--- @example 
--- @example -- Assign a number value to table t at a string key
--- @example example_table["10"] = 10
--- @example 
--- @example -- Assign a string value to table t at a string key
--- @example example_table["five"] = "five"
--- @example 
--- @example -- Also assign a string value to table t at a string key, using the shorthand method of accessing string keys
--- @example example_table.ten = "ten"
--- @example 
--- @example -- Function to access and print data from the table
--- @example function print_key_and_value(t, key)
--- @example 	print("key is " .. tostring(key) .. " of type " .. type(key) .. ", corresponding value is " .. tostring(t[key]) .. " of type " .. type(t[key]))
--- @example end
--- @example 
--- @example print_key_and_value(example_table, 5)
--- @example print_key_and_value(example_table, "5")		-- doesn't exist - no value corresponding to the string key "5" was written - value will be nil
--- @example print_key_and_value(example_table, 10)			-- doesn't exist - no value corresponding to the number key 10 was written - value will be nil
--- @example print_key_and_value(example_table, "five")
--- @example print_key_and_value(example_table, "ten")
--- @example 
--- @example -- This use of shorthand accessors will work, as the keys "five" and "ten" are strings
--- @example print(example_table.five)
--- @example print(example_table.ten)
--- @example 
--- @example -- This, however, is illegal, as while the key "10" is a string, it starts with a numeric char
--- @example print(example_table.10)
--- @result key is 5 of type number, corresponding value is 5 of type string
--- @result key is 5 of type string, corresponding value is nil of type nil
--- @result key is 10 of type number, corresponding value is nil of type nil
--- @result key is five of type string, corresponding value is five of type string
--- @result key is ten of type string, corresponding value is ten of type string
--- @result five
--- @result ten
--
--- @new_example
...
--- @example t["age"] = 25
--- @example
--- @example -- Alternative shorthand method for assigning a value to a string key within a table.
--- @example -- Here, the string value "Peter" is stored at the string key "name".
--- @example t.name = "Peter"
--- @example 
--- @example -- The alternative shorthand method of accessing table elements cannot be used for keys that are not strings, however.
--- @example t.false = "warning!"			-- The value "warning!" will be stored at the string key "false", not the boolean key false
--- @example t.2 = "error!"					-- This attempt to use table assignment shorthand is illegal and will generate a syntax error
--- @example t["2"] = "okay"				-- The proper method of accessing an element with a string key that starts with a number char
--- @example
--- @example -- Assign a value to a key, but where we evaluate the key from another variable.
--- @example -- The string value "brown" will be stored at the string key "hair_colour"
...
--- @desc Before a table can be used it must be created with a table constructor, which is a pair of <code>{}</code> braces. Values, or key/value pairs, can be included within the constructor so that they are stored in the table from creation.
--- @desc Within the <code>{}</code> braces which denote the new table, key/value combinations that specify data to add to the table on construction can be given in a variety of formats:
--- @desc <ol><li>Keys can be explictly given by enclosing them in square brackets <code>[]</code>.</li>
--- @desc <li>If a key name is given but not enclosed within square brackets, it is assumed to be a @string.</li>
--- @desc <li>If the key is omitted altogether, then the first ascending free integer is used as the key. This shorthand is useful for creating @"table:Indexed Tables".</li></ol>
--
--- @new_example Create an empty table
--- @example t_empty = {}
--
--- @new_example Create an indexed table with data values by omitting the keys
--- @desc If data values are supplied without keys, separated by commas, then each value is inserted into the table at the first available integer key, starting at 1. See the section on @"table:Indexed Tables" below.
--- @example t_indexed = {
--- @example 	"first",		-- The string value "first" is stored with a numeric key 1 
--- @example 	"second",		-- The string value "second" is stored with a numeric key 2
--- @example 	"third",		-- The string value "third" is stored with a numeric key 3
--- @example 	"fourth"		-- The string value "fourth" is stored with a numeric key 4
--- @example }
--
--- @new_example Create a table with explicit key/values
--- @example t_explicit_keys = { [10] = "ten", [20] = "twenty", [30] = "thirty", ["40"] = "a trap" }		-- observe that ["40"] is actually a string key, rather than a number
--
--- @new_example Shorthand way of creating string keys during construction
--- @example t_string_keys = {
--- @example 	monday = "first",					-- string key "monday", string value "first"
--- @example 	tuesday = "second",					-- string key "tuesday", string value "second"
--- @example 	["wednesday"] = "third",			-- the 'longhand' way of specifying string key "wednesday", string value "third"
--- @example 	[thursday] = "fourth",				-- probably an error! This is not specifying a string key, but will evaluate the expression <i>thursday</i> and substitute the result
--- @example 	1st_january = "new_year",			-- this is illegal - when using the shorthand method of creating a string key, the first character must be a letter
--- @example 	["1st_january"] = "new_year",		-- legal version, using the longhand form of key specification - generally, it's recommended to avoid string table keys starting with number chars
--- @example 	25th december = "christmas",		-- also illegal - when using shorthand methods of specifying a string key, they may not contain spaces
--- @example 	["25th december"] = "christmas" 	-- legal version, using the longhand form of string key specification
--- @example }
--
--- @new_example Create a table with key/values, but where the key is evaluated
--- @example key_name = "title"
--- @example t = { [key_name] = "doctor" }
--- @example print(t.title)				-- print the value corresponding to the string key "title"
--- @result doctor


--- @section Tables and Memory
--- @desc Tables are always assigned by reference, and passed by reference if supplied as function arguments. Multiple references to the same table can exist at the same time. When no more references to a table exist, the lua garbage collector will eventually delete it and reclaim the memory it uses.
--- @example -- Declare function to modify table
--- @example function modify_table(t)
--- @example 	print("modify_table() called, table is " .. tostring(t))
--- @example 	print("\tadding table.parting")
--- @example 	t.parting = "goodbye"
--- @example end
--- @example 
--- @example -- Declare table
--- @example example_table = { greeting = "hello" }
--- @example print("example_table created, memory address is " .. tostring(example_table))
--- @example 
--- @example modify_table(example_table)
--- @example 
--- @example print("example_table.greeting is " .. example_table.greeting .. ", example_table.parting is " .. example_table.parting)
--- @example 
--- @example example_table_2 = example_table
--- @example 
--- @example example_table = "not a table any more"
--- @example 
--- @example print("example_table is " .. tostring(example_table))
--- @example print("example_table_2 is " .. tostring(example_table_2))
--- @example 
--- @example example_table_2 = 5
--- @result example_table created, memory address is table: 0x75cd70
--- @result modify_table() called, table is table: 0x75cd70
--- @result 	adding table.parting
--- @result example_table.greeting is hello, example_table.parting is goodbye
--- @result example_table is not a table any more
--- @result example_table_2 is table: 0x75cd70


--- @section Indexed Tables
--- @desc Tables are commonly used in one of two manners. Our internal nomenclature for these is <code>Indexed</code> vs @"table:Lookup Tables". There is no difference in the manner of creation between the two, and indeed one table can work in both modes at once, although this is rarely done.
--- @desc Indexed tables, as we call them, contain elements stored at keys in an ascending numerical sequence, starting from <code>1</code>. Tables arranged in this manner are easy to create, easy to iterate over in sequence, and many functions within the table library and elsewhere require tables to be arranged in an indexed manner in order to work. Additionally, the sizeof operator <code>#</code> will only produce accurate results for an indexed table.
--- @desc Indexed tables can be iterated-over with a manual for-loop (see the section on @"table:Standard Traversal"), or with the @global:ipairs iterator. The @global:pairs iterator can also be used, but the order of iteration will not be predictable or deterministic.
--- @desc It is advantageous to create an indexed table when the problem being solved would benefit from storing and working with data in a listed, fixed order. If the relative order in which elements are stored in the table is important, especially if iterating over the table, or if the table's use case demands that the data within it will be accessed via iteration more than directly by key, then chances are it should be set up as an indexed table. Examples include stacks, queues, trees or other data structure where the order of elements needs to be preserved.
--
--- @new_example Indexed table creation
--- @desc If no keys are given during table creation, then the table is indexed by default.
--
--- @example -- First value is stored at numeric key 1
--- @example days_of_week = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"}
--- @example 
--- @example for i = 1, #days_of_week do
--- @example 	print(days_of_week[i])
--- @example end
--- @example 
--- @example -- sizeof operator # only works with indexed tables
--- @example print("There are " .. #days_of_week .. " days of the week")
--- @example 
--- @example -- table.concat(t, spacer) is an example of a table library function that requires an indexed table in order to work
--- @example str = table.concat(days_of_week, ", ")
--- @example print(str)
--- @example 
--- @result Monday
--- @result Tuesday
...
--- @result Saturday
--- @result Sunday
--- @result There are 7 days of the week
--- @result Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday


--- @section Lookup Tables
--- @desc Tables containing keys that are not defined in an ascending integer sequence tend to be used as lookup tables. Data in a lookup table is more commonly looked up directly by key, rather than by iteration. If data within the table is accessed more commonly by iteration, or if it's important to maintain an order between the records stored in the table, then consider creating an indexed table with ascending numeric keys instead.
--- @desc Lookup tables are most useful for flat data sets with no order of precedence or operation between the data records. Data sets akin to database tables are ideal for storing in lookup tables.
--- @desc Lookup tables cannot be iterated over with the @global:ipairs iterator - instead, the @global:pairs iterator may be used. The downside to @global:pairs is that order of iteration cannot be relied on to be the same on different machines running the same script, or even on the same machine running the same script multiple times.
--
--- @new_example Lookup table example, showing nested tables
--- @example months = {
--- @example 	January = {					-- within months table: string key "January", value is a table
--- @example 		max_temp = 8.19,		-- within January table: string key "max_temp", value is number 8.19
--- @example 		min_temp = 2.37
--- @example 	},
--- @example 	February = { max_temp = 8.37, min_temp = 2.25 },
--- @example 	March = { max_temp = 10.48, min_temp = 3.56 },
--- @example 	April = { max_temp = 13.12, min_temp = 5.15 },
--- @example 	May = { max_temp = 16.31, min_temp = 8.11 },
--- @example 	June = { max_temp = 19.08, min_temp = 10.95 },
--- @example 	July = { max_temp = 20.94, min_temp = 13.02 },
--- @example 	August = { max_temp = 21.03, min_temp = 13.06 },
--- @example 	September = { max_temp = 18.92, min_temp = 10.92 },
--- @example 	October = { max_temp = 15.49, min_temp = 8.38 },
--- @example 	November = { max_temp = 11.61, min_temp = 5.21 },
--- @example 	December = { max_temp = 8.87, min_temp = 2.81 }
--- @example }
--- @example 
--- @example flowers = {
--- @example 	Sunflowers = { colour = "yellow", peak_month = "August" },
--- @example 	Roses = { colour = "red", peak_month = "July" },
--- @example 	Daffodils = { colour = "yellow", peak_month = "April" },
--- @example 	Chrysanthemums = { colour = "pink", peak_month = "September" },
--- @example 	Snowdrops = { colour = "white", peak_month = "February" }
--- @example }
--- @example 
--- @example function print_flower_info(flower_key)
--- @example 	local flower_record = flowers[flower_key]			-- look up flower sub-table within flowers table, based on the key
--- @example 	if flower_record then
--- @example 		local month_key = flower_record.peak_month
--- @example 		local month_record = months[month_key]			-- look up month sub-table within months table
--- @example 		if month_record then
--- @example 			local colour = flower_record.colour			-- look up colour value from the relevant flower record
--- @example 
--- @example 			-- look up min_temp and max_temp values from the relevant month record
--- @example 			local min_temp = month_record.min_temp
--- @example 			local max_temp = month_record.max_temp
--- @example 			print(flower_key .. " are " .. colour .. " and are best seen in " .. month_key .. ", when the temperature varies between " .. min_temp .. " and " .. max_temp .. " celcius")
--- @example 		end
--- @example 	end
--- @example end
--- @example 
--- @example print_flower_info("Daffodils")
--- @example print_flower_info("Chrysanthemums")
--- @result Daffodils are yellow and are best seen in April, when the temperature varies between 5.15 and 13.12 celcius
--- @result Chrysanthemums are pink and are best seen in September, when the temperature varies between 10.92 and 18.92 celcius


--- @section Standard Traversal
--- @desc Tables are commonly traversed/iterated over with a for-loop with manually-defined limits and steps. This is possible if the table keys are arranged in a suitable numeric order - usually an ascending integer sequence (see the section on @"table:Indexed Tables").
--- @desc The size of a table where values are assigned at ascending numeric integer keys can be accessed with the <code>#</code> operator. This reports the first integer key <code>n</code> for which no value is stored at key <code>n + 1</code>.
--
...
--- @desc The lua language provides the iterator functions @global:ipairs and @global:pairs for iterating over tables where @"table:Standard Traversal" may not be possible or desirable.
--- @desc The @global:ipairs iterator behaves much the same as traversing over a table in ascending numeric sequence. Like other iterator functions, @global:ipairs automatically creates local variables for the key and (optionally) the value for each table element it traverses. The @global:ipairs iterator starts at the table element corresponding to a numeric key value of 1, increments the key by 1 for each loop iteration, and stops at the first integer key it finds for which a value is not assigned.
--- @desc Unlike @global:ipairs and numeric traversal, the @global:pairs iterator allows traversal over all elements in a table, regardless of key type or any 'gaps' in the data. The main caveat to usage is that the order of traversal is not deterministic, either between machines or on the same machine making multiple passes over the same table. As such, the @global:pairs iterator is unsafe to use in multiplayer scripts as two different machines will traverse the same table in different orders.
--- @desc Also provided within the scripting libraries are:
--- @desc <ul><li>@global:dpairs, which is a deterministic version of @global:pairs. This eliminates the problem of not being able to use @global:pairs in multiplayer, but is written in lua and is quite computationally slow, so use this sparingly.</li>
--- @desc <li>@global:ipairs_reverse is an equivalent to @global:ipairs which iterates in reverse order.</li><ul>
--- @desc See also the section on @"global:Game Object Iterators", for iterators that work on non-table game objects.


--- @function ipairs
--- @target_override global
--- @desc Iterates over a numerically indexed table. An indexed table is one containing keys in an ascending numeric sequence, starting from 1. The iterator will query the key/value pair where the key is 1, and then 2, 3 and so on, until it finds a key/pair that doesn't exist, at which point the iterator will terminate.
--- @desc The ipairs function returns three values - an iterator function, the table to iterate over, and the control variable. When used with a for loop the iterator can be used to get the key/value of each element in the table in sequence. See the examples below for more information on usage.
--- @desc Unlike the @global:pairs iterator, ipairs disregards values stored at non-numeric keys, but the order in which values are returned is deterministic, meaning ipairs is safe to use in multiplayer mode.
--- @desc The ipairs iterator is interchangeable with using the size of table operator <code>#</code> to manually control the loop iteration.
--- @p @table subject table
--- @r @function iterator function
--- @r @table subject table
--- @r @number control variable
--
--- @new_example Standard ipairs example
--- @example t = {"january", "february", "march", "april", "may" } 		-- table created with ascending numeric keys
...
--- @example end
--- @result key: 1, value: january
--- @result key: 2, value: febraury
--- @result key: 3, value: march
--- @result key: 4, value: april
--- @result key: 5, value: may
--
--- @new_example Ipairs example with gap in numeric sequence
...
--
--- @new_example Ipairs example with table containing non-numeric keys
--- @example t = {
...
--- @example 
--- @example for key, value in ipairs(t) do
--- @example 	-- loop will continue until it fails to find a value corresponding to key 3, and then stop
--- @example 	print("key: " .. key .. ", value: " .. value);
--- @example end
--- @result key: 1, value: first
--- @result key: 2, value: second
...
--- @function pairs
--- @target_override global
--- @desc Iterates over any table returning key/value pairs in a non-deterministic order. The pairs iterator function is the main way to query each record in a table that is not numerically indexed. However, the order of iteration is not guaranteed, so the pairs iterator should be used with caution in multiplayer games where a different order of modifications to the model between clients can cause the game to desync.
--- @desc The pairs function returns three values - an iterator function, the table to iterate over, and the control variable. When used with a for loop the iterator can be used to get the key/value of each element in the table in sequence. See the examples below for more information on usage.
--- @p @table subject table
--- @r @function iterator function
--- @r @table subject table
--- @r @number control variable
...
-- internal iteration function, used by ipairs_reverse
local function ipairs_reverse_iter(t, i)
	i = i - 1
	if i ~= 0 then
		return i, t[i]
	end
...
--- @function ipairs_reverse
--- @target_override global
--- @desc Iterator function that allows @"table:Indexed Tables" to be iterated over in reverse order. This function can be used effectively with a for-loop to traverse backwards over an indexed table - see the example below.
--- @p @table table
--- @r @function iterator function
--- @r @table table
--- @r @number initial search key
-- 
--- @example local t = {"monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"}
--- @example for index, day in ipairs_reverse(t) do
--- @example 	print("Day " .. index .. " is " .. day);
--- @example end;
--- @result Day 7 is sunday
--- @result Day 6 is saturday
--- @result Day 5 is friday
--- @result Day 4 is thursday
--- @result Day 3 is wednesday
--- @result Day 2 is tuesday
--- @result Day 1 is monday
function ipairs_reverse(t)
	return ipairs_reverse_iter, t, #t + 1;
end
...
function table.tostring(t, for_campaign_savegame, max_level, tab_level, prepend_tabs, add_line_break)
	...
	for key, value in pairs(t) do
		-- skipping __index in order to process tables with metatables correctly, otherwise infinite recursion will occur
		if key ~= "__index" and (not for_campaign_savegame or is_table(value) or is_string(value) or is_number(value) or is_boolean(value)) then
...
--- @function ceil_to
--- @desc Returns the smallest multiple of the supplied multiple value, which is larger than the supplied subject value.
--- @p @number subject value
--- @p [opt=10] @number multiple value
--- @r @number ceiling value
--- @example -- print the smallest multiple of 50 that is larger than 128
--- @example print(math.ceil_to(128, 50))
--- @result 150
function math.ceil_to(subject_value, multiple)
    multiple = multiple or 10;
    return math.ceil(subject_value * (1 / multiple) ) * multiple;
end


--- @function clamp
--- @desc Clamps the supplied numeric value to be between the supplied min and max values. The clamped value is returned.
--- @p @number value
--- @p @number min
--- @p @number max
--- @r @number clamped value
...
--- @function floor_to
--- @desc Returns the largest multiple of the supplied multiple value, which is smaller than the supplied subject value.
--- @p @number subject value
--- @p [opt=10] @number multiple value
--- @r @number ceiling value
--- @example -- print the largest multiple of 50 that is smaller than 128
--- @example print(math.floor_to(128, 50))
--- @result 100
function math.floor_to(subject_value, multiple)
    multiple = multiple or 10;
    return math.floor(subject_value * (1 / multiple) ) * multiple;
end
...
--- @function map_range
--- @desc Map a numeric value from one given range of numbers to another. The returned value will be the same relative to the output range, as the input number is to the specified input range. Negative numbers, input values outside of the input range, and ranges that run backwards are all supported.
--- @p @number input value
--- @p @number input range start
--- @p @number input range end
--- @p @number output range start
--- @p @number output range end
--- @r @number output value
--- @new_example Simple map_range example
--- @desc For an input range between 0 to 10, and an input value of 1 (i.e. 10% of the difference between 0 and 10), show the value mapped on to the output range 50 to 60
--- @example print(math.map_range(1, 0, 10, 50, 60))
--- @result 51.0
--- @new_example Further map_range example
--- @desc For an input range between 20 to 30, and an input value of 25 (50% of the difference between 20 and 30), show the value mapped on to the output range 50 to 100
--- @example print(math.map_range(25, 20, 30, 50, 100))
--- @result 75.0
--- @new_example Negative numbers
--- @desc For an input range between -100 to -50, and an input value of -25, show the value mapped on to the output range 50 to 100
--- @example print(math.map_range(-25, -100, -50, 50, 100))
--- @result 125.0
function math.map_range(input_value, input_range_start, input_range_end, output_range_start, output_range_end)
	local slope = (output_range_end - output_range_start) / (input_range_end - input_range_start)
	return output_range_start + (slope * (input_value - input_range_start))
end
...
--- @function round
--- @desc Returns the nearest integer value to the supplied number.
--- @p @number input
--- @r @number rounded output
--- @example print(math.round(10.49))
--- @example print(math.round(10.51))
--- @example print(math.round(-10.49))
--- @example print(math.round(-10.51))
--- @result 10
--- @result 11
...
--- @function to_bool
--- @desc Converts the given data to a boolean. Only "nil" and "false" will return false.
--- @p value data
