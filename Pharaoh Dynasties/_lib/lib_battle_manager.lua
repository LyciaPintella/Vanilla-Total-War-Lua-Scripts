...
battle_manager = {
	...
	adc_suspended_for_queue_advisor = false,
	suspend_adc_count = 0,
...
-- set battle manager to be of custom type TYPE_BATTLE_MANAGER
...
function battle_manager:new()
	...
	local bm = {
		...
		fort_wall_buildings = {},
		building_grid = {},
		script_units_loaded = false
	};
	
	-- set 'bm' to derive first from self 'battle_manager' and then from 'b' (battle interface from code)
	...
	core:set_ui_root(UIComponent(UIComponent(b:ui_component("hud_battle")):Parent()));
	...
	bm.script_units_loaded = true;

	bm:cache_buildings_on_startup();
	...
	force_require("troy_battle_help_pages");
	
	--
	-- Setup info overlays - assuming libraries are loaded
	--
	require("troy_battle_info_overlays");
	...
	-- Create a key steal manager
	self.ksm = key_steal_manager:new();
	...
	-- notify any listening scripts of this
	core:trigger_event("ScriptEventGlobalBattleManagerCreated");
	...
	if core:svr_load_bool("disable_unit_banner_description_on_battle_end") then
		bm:register_phase_change_callback(
			"Complete",
			function()
				common.set_pref_integer("ui_unit_banner_description", 0)
				core:svr_save_bool("disable_unit_banner_description_on_battle_end", false)
				core:remove_listener("unit_battle_description_banner_restore")
			end
		)

		-- safe guard for restoring the settings if the player exits to desktop in the middle of the battle
		core:add_listener(
			"unit_battle_description_banner_restore",
			"UIDestroyed",
			true,
			function(context)
				common.set_pref_integer("ui_unit_banner_description", 0)
	...
	core:add_listener(
		"bm_continue_with_campaign_tutorial",
		"ComponentLClickUp",
		function(context) return context.string == "button_next_tutorial" end,
		function(context)
...
--- @desc Queries the UI and returns true if any unit cards are selected.
...
--- @desc Queries the UI and returns true if all unit cards are selected.
...
function battle_manager:assault_equipment_exists(key)
	...
	local vehicle_count = assault_equipment:vehicle_count();
...
--- @desc Returns <code>true</code> if the battle type is <code>settlement_unfortified</code>, indicating a minor settlement battle.
--- @return @boolean is minor settlement battle
function battle_manager:is_minor_settlement_battle()
	return self.battle:battle_type() == "settlement_unfortified";
end;


--- @function is_major_settlement_battle
--- @desc Returns <code>true</code> if the battle type is <code>settlement_standard</code>, indicating a major settlement battle.
--- @return @boolean is major settlement battle
function battle_manager:is_major_settlement_battle()
	return self.battle:battle_type() == "settlement_standard";
end;
...
function battle_manager:random_number(max_value)
	...
	return math.floor(self.battle:random_number() * max_value) + 1;
end;
...
--- @desc Registers a function to be called when a specified phase change occurs. Phase change notifications are sent to the script by the game when the battle changes phases, for example from "Deployment" to "Deployed" and on to "VictoryCountdown" and "Complete". The battle manager writes output to the console whenever a phase change occurs, regardless of whether any callback has been registered for it. A script event, <code>ScriptEventBattlePhaseChanged</code>, is also triggered when a phase change occurs, and supplies the name of the new phase as a context string.
...
function battle_manager_phase_change(phase)
	...
	for i = 1, #bm.phase_change_callback_list do	
		if bm.phase_change_callback_list[i].phase_name == phase_name then
...
--- @function winner_alliance_id
--- @desc Returns the winning alliance id. Only makes sense after the battle is completed.( I.e. in `Complete` phase)
--- @return @number winning alliance id
function battle_manager:winner_alliance_id()
	return self.battle:winner_alliance_id()
end
...
--- @desc The @key_steal_manager now does the work involved with stealing keys and handling them being pressed, but these pass-through functions are provided on the battle manager for ease of use.
----------------------------------------------------------------------------
...
--- @desc This function maps to the underlying @key_steal_manager:steal_escape_key_with_callback function which actually does the work now.
--- @p string callback name
--- @p function callback
...
function battle_manager:steal_escape_key_with_callback(name, callback, is_persistent)
	return self.ksm:steal_escape_key_with_callback(name, callback, is_persistent);
end;
...
--- @desc This function maps to the underlying @key_steal_manager:release_escape_key_with_callback function which actually does the work now.
--- @p string callback name to cancel
function battle_manager:release_escape_key_with_callback(name)
	return self.ksm:release_escape_key_with_callback(name);
end;
...
-- local function to actually play the advice
local function play_advice(advice_key)
	common.advance_scripted_advice_thread(advice_key, 1);
	get_infotext_manager():notify_of_advice(advice_key);
end;
...
function battle_manager:play_next_advice()
	...
	if #self.advisor_list == 0 then
		...
		-- Unwind adc suspension placed when the first advice in this sequence was played
		self.adc_suspended_for_queue_advisor = false;
		self:suspend_adc(false);
	...
	if current_advice.is_debug then
		...
		-- Cache whether the adc is suppressed, if we haven't done so already, and then suppress it
		if not self.adc_suspended_for_queue_advisor then
			self.adc_suspended_for_queue_advisor = true;
			self:suspend_adc(true);
		end;
		
		if advice_offset > 0 then
			self:callback(
				function() 
					play_advice(current_advice.advisor_string);
				end, 
				advice_offset, 
...
function battle_manager:modify_advice(show_progress_button, highlight)
	...
	if not find_uicomponent(core:get_ui_root(), "advice_interface") then
		self:callback(function() self:modify_advice(show_progress_button, highlight) end, 200, self.modify_advice_str);
		return;
	end;
...
function battle_manager:set_infotext_auto_clear_timer(...)
...
-----------------------------------------------------------------------------
--- @section Aide-de-Camp
-----------------------------------------------------------------------------


--- @function suspend_adc
--- @desc Suspends or un-suspends the aide-de-camp. This function wraps the @battle:suspend_adc but provides lock levels - if multiple calls to suspend the adc are made, followed by multiple calls to unsuspend it, the adc won't actually get unsuspended until each suspend call is undone with an unsuspend call. This means that every call to suspend the adc must be matched with an equivalent call to unsuspend it.
--- @p @boolean should suspend
function battle_manager:suspend_adc(should_suspend)

	if should_suspend then
		if self.suspend_adc_count == 0 then
			self.battle:suspend_adc(true);
		end;

		self.suspend_adc_count = self.suspend_adc_count + 1;
	else
		if self.suspend_adc_count > 1 then
			self.suspend_adc_count = self.suspend_adc_count - 1;
		else
			if self.suspend_adc_count == 1 then
				self.battle:suspend_adc(false);
			end;

			self.suspend_adc_count = 0;
		end;
	end;
end;
...
function battle_manager:show_start_battle_button(value, is_multiplayer)
	...
	if value then
		...
		self.restore_ui_hiding_action_list["show_start_battle_button"] = function()
			self:show_start_battle_button(value, immediate);
		end;
	end;
end;
...
--- @function show_ui
--- @desc Shows/hides the army panel, portrait panel, top bar and radar frame from script.
...
function battle_manager:show_portrait_panel(value, immediate)
	local uic = find_uicomponent(core:get_ui_root(), "porthole_parent");
...
function battle_manager:show_radar_frame(value, immediate)
	...
	if value == false then
		if self.active_ui_overrides["radar_frame"] then
			...
			if immediate then
				out("@show_radar_bar() is hiding radar frame immediately");
				uic:PropagateVisibility(false);
			else
				out("@show_radar_bar() is hiding radar frame");
			end;
			uic:TriggerAnimation("tut_hide");
		end;
	else
		if self.active_ui_overrides["radar_frame"] then
			out("@show_radar_bar() is showing radar frame");
			self.active_ui_overrides["radar_frame"] = false;
			uic:PropagateVisibility(true);
			uic:TriggerAnimation("tut_show");

			-- When disabling the ui_override make sure to hide multiplayer elements
			if not bm:is_multiplayer() then 
				local multiplayer_uic_keys = {
					"telestration_parent",
					"mp_speed_voting"
				}
			
				for i = 1, #multiplayer_uic_keys do 
					local mp_uic = bm:ui_component(multiplayer_uic_keys[i])
					mp_uic:PropagateVisibility(false);
				end
			end
		else
...
function battle_manager:enable_camera_movement(value)
	if value == false then
		out("@enable_camera_movement() is disabling camera functions");
		self:camera():disable_functionality("CAMERA_ALL_FUNCTIONS");
	else
		out("@enable_camera_movement() is enabling camera functions");
		self:camera():enable_functionality("CAMERA_ALL_FUNCTIONS");
	end;
end;
...
function battle_manager:disable_time_speed_controls(value)
	-- context command not present in Pharaoh
	--[[
	if value == false then
...
function battle_manager:disable_unit_details_panel(value)
	local uic_unit_details_button = find_uicomponent(core:get_ui_root(), "hud_battle", "battle_orders", "battle_orders_pane", "card_panel_docker", "cards_panel", "porthole_parent", "button_toggle_info_panel_old");
	if not uic_unit_details_button then
		script_error("ERROR: disable_unit_details_panel() called but uic_unit_details_button could not be found - dumping UI to Lua UI console spool");
...
--- @desc <ul><li>tScriptEventBattleArmiesEngaging, when the two sides close to within 100m or once more than 40% of the player's army is under fire.</li>
--- @desc <li>ScriptEventPlayerGeneralWounded, if the player's general is wounded (they must be invincible).</li>
--- @desc <li>ScriptEventPlayerGeneralDies, if the player's general dies (not invincible).</li>
--- @desc <li>ScriptEventEnemyGeneralWounded, if the enemy general is wounded (they must be invincible).</li>
--- @desc <li>ScriptEventEnemyGeneralDies, if the enemy general dies (not invincible).</li>
--- @desc <li>ScriptEventPlayerGeneralRouts, if the player's general routs.</li>
--- @desc <li>ScriptEventEnemyGeneralRouts, if the enemy general routs.</li>
--- @desc <li>ScriptEventPlayerUnitRouts, if one of the player's units routs.</li>
--- @desc <li>ScriptEventPlayerUnitRallies, if one of the player's units rallies.</li>
--- @desc <li>ScriptEventEnemyUnitRouts, if one of the enemy units routs.</li></ul>
...
function battle_manager:engagement_monitor_battle_starts()
	...
	--
	-- very hacky/fudgy listener for the player getting into the city/close to the victory point in a siege battle. Rewrite all this when we get a better interface.
	--
	if self:is_siege_battle() then
		core:add_listener(
			"victory_point_approach_watch",
			"BattleAideDeCampEvent",
			function(context) return context.string == "adc_own_attacking_gates" or context.string == "adc_own_ladders_docked" end,
			function(context)
				local player_engaged_event = "ScriptEventPlayerEngagedInSiegeBattle";
				local post_player_engaged_event = "ScriptEventPlayerApproachingVictoryPoint";

				local countdown_time = 120000;
				self:out("<<< Received BattleAideDeCampEvent with context " .. context.string .. ", triggering event " .. player_engaged_event .. ". Will trigger " .. post_player_engaged_event .. " in " .. countdown_time .. "ms >>>");
				
				self:callback(
					function()
						self:out("<<< triggering event " .. post_player_engaged_event .. " >>>");
						core:trigger_event(post_player_engaged_event);
					end,
					countdown_time
				);

				core:trigger_event(player_engaged_event);
			end,
			false
		);
	end;
...
--- @desc On startup, the battle manager 
-----------------------------------------------------------------------------


local BUILDING_GRID_RESOLUTION = 10;

-- return the building grid index which corresponds with the supplied position value
local function building_grid_index_from_position(position)
	return math.floor(position / BUILDING_GRID_RESOLUTION)
end;


-- return the possible building grid min/max position ranges that can correspond with the supplied index value
local function building_position_range_from_grid_index(index)
	return index * BUILDING_GRID_RESOLUTION, ((index + 1) * BUILDING_GRID_RESOLUTION) - 1;
end;


local function get_building_grid_cell(x_index, y_index)
	local building_grid = bm.building_grid;

	if building_grid[x_index] then
		return building_grid[x_index][y_index];
	end;
end;


-- Internal function called on startup
function battle_manager:cache_buildings_on_startup()
	...
	local fort_tower_buildings = self.fort_tower_buildings;
	local fort_gate_buildings = self.fort_gate_buildings;
	local fort_wall_buildings = self.fort_wall_buildings;

	local building_grid = self.building_grid;

	building_grid.count = 0;

	local buildings_count = buildings:count();
	for i = 1, buildings_count do
		local current_building = buildings:item(i);

		-- Add the building to fort/gate/wall lists if it's the right type
		...
		-- Put the building in to a cell in the building grid
		local current_building_position = current_building:central_position();
		local building_grid_x = building_grid_index_from_position(current_building_position:get_x());
		local building_grid_y = building_grid_index_from_position(current_building_position:get_z());

		-- Building grid is a nested series of tables within tables. The outer tables represent a row in x, which contains multiple tables representing
		-- each cell in y (or z, as battle co-ords get flipped around). Buildings belonging to that x/y cell are stored in the inner table.

		-- The min and max values in each parent container represent the range of actual values stored in that container. This is so that a process that iterates over
		-- all buildings in the grid does not have to start and end at each corner of the map if the buildings are clustered towards the centre, for example.

		-- If the outer or inner table does not exist, create it. Only outer/inner table combinations that actually contain buildings will be created.
		if not building_grid[building_grid_x] then
			-- We are inserting a building in a cell x row that hasn't already been created

			building_grid[building_grid_x] = {};

			building_grid.min = building_grid_x;
			building_grid.max = building_grid_x;

			building_grid[building_grid_x][building_grid_y] = {};

			building_grid[building_grid_x].min = building_grid_y;
			building_grid[building_grid_x].max = building_grid_y;

		elseif not building_grid[building_grid_x][building_grid_y] then
			-- We are inserting a building in a pre-existing x row, but where the y column has not already been created
			...
			if building_grid_x < building_grid.min then
				building_grid.min = building_grid_x;
			elseif building_grid_x > building_grid.max then
				building_grid.max = building_grid_x;
			end;

		else
			-- We are inserting the building in a grid cell that already exists
			if building_grid_x < building_grid.min then
				building_grid.min = building_grid_x;
			elseif building_grid_x > building_grid.max then
				building_grid.max = building_grid_x;
			...
			local building_grid_row = building_grid[building_grid_x];

			if building_grid_y < building_grid_row.min then
				building_grid_row.min = building_grid_y;
			elseif building_grid_y > building_grid_row.max then
				building_grid_row.max = building_grid_y;
			end;
		end;

		-- Perform the insertion
		table.insert(building_grid[building_grid_x][building_grid_y], current_building);

		building_grid.count = building_grid.count + 1;
	end;

	self:out("Added " .. building_grid.count .. " buildings to the building grid in " .. os.clock() - start_timestamp .. "s");
end;


--- @function print_buildings_grid
--- @desc Dump console spool output related to the buildings grid.
function battle_manager:print_buildings_grid()
	self:out("");
	self:out("==============================================================================");
	self:out("Printing Building Grid");
	self:out("==============================================================================");
	self:out("");

	local building_grid = self.building_grid;

	if building_grid.count == 0 then
		self:out("Building grid is empty!");
	else
		for i = building_grid.min, building_grid.max do
			local building_grid_row = building_grid[i];
	
			if building_grid_row then
				for j = building_grid_row.min, building_grid_row.max do
					local cell = building_grid_row[j];

					if cell then
						local x_min, x_max = building_position_range_from_grid_index(i);
						local y_min, y_max = building_position_range_from_grid_index(j);

						self:out("Buildings found in grid cell with index [" .. i .. ", " .. j .. "] corresponding with [" .. x_min .. " to " .. x_max .. " x, " .. y_min .. " to " .. y_max .. " y]:");
						for k = 1, #cell do
							local current_building = cell[k];
							self:out("\tbuilding at " .. v_to_s(current_building:central_position()) .. " with name " .. current_building:name() .. " of category " .. current_building:category());
						end;
					end;
				end;
			end;
...
--- @function get_buildings_from_grid
--- @desc Returns a table of buildings that exist within a given range of a given position from the cached building grid. This lookup is much more computationally efficient than measuring the distance of every building on the battlefield to the given position manually.
--- @desc An optional filter function may be provided. If supplied, this function is called for each eligible building, with the building passed in as a single argument. The function should return a value that evaluates to <code>true</code> for the building to be added to the returned list. If no filter function is provided then all buildings within range of the position are added to the returned list.
--- @desc Distances between the supplied position and each tested building may be measured in two or three dimensions. By default, distances are measured in two dimensions.
--- @p @battle_vector position, Position to test against.
--- @p @number range, Range from the position to test buildings against. Buildings within this range of the position are added to the returned list, assuming they either pass the supplied filter function or no filter function is supplied.
--- @p [opt=nil] @function filter, Optional filter function. If supplied, each in-range building is supplied to this function, which must return a value that evaluates to <code>true</code> for that building to be added to the returned list. If no filter function is supplied then all buildings within range of the position are added.
--- @p [opt=false] @boolean test 3D, Test distances between each candidate building and the supplied position in three dimensions rather than two dimensions.
--- @r @table table of @battle_building objects within range
function battle_manager:get_buildings_from_grid(position, range, filter, measure_distances_in_three_dimensions)

	if not validate.is_vector(position) then
		return false;
	end;

	if not validate.is_positive_number(range) then
		return false;
	end;

	if filter and not validate.is_function(filter) then
		return false;
	end;

	-- If there are no buildings in the grid then just return an empty list
	if self.building_grid.count == 0 then
		return {};
	end;

	local pos_x = position:get_x();
	local pos_y = position:get_z();

	-- min/max x/y grid index define a box of cells within range of the target position. Only the buildings within these cells will be tested against the target position, rather than every building on the battlefield.
	local min_grid_index_x = building_grid_index_from_position(pos_x - range);
	local max_grid_index_x = building_grid_index_from_position(pos_x + range);

	local min_grid_index_y = building_grid_index_from_position(pos_y - range);
	local max_grid_index_y = building_grid_index_from_position(pos_y + range);

	-- List of buildings to return
	local retval = {};

	-- Define a building test. This will be different depending on whether we have a filter function and whether we measure 3d or 2d distances
	local building_test;
	if measure_distances_in_three_dimensions then
		if filter then
			function building_test(building, position, range, filter)
				return building:central_position():distance(position) <= range and filter(building);
			end;
		else
			function building_test(building, position, range)
				return building:central_position():distance(position) <= range;
			end;
		end;
	else
		if filter then
			function building_test(building, position, range, filter)
				return building:central_position():distance_xz(position) <= range and filter(building);
			end;
		else
			function building_test(building, position, range)
				return building:central_position():distance_xz(position) <= range;
			end;
		end;
	end;

	-- Iterate over all buildings in the cells within our min/max x/y sub-grid and test them against the target position and optional filter. Add them to the returned buildings list if they pass.
	for i = min_grid_index_x, max_grid_index_x do
		for j = min_grid_index_y, max_grid_index_y do
			local cell = get_building_grid_cell(i, j);

			if cell then
				for k = 1, #cell do
					local current_building = cell[k];

					if building_test(current_building, position, range, filter) then
						table.insert(retval, current_building);
					end;
				end;
			end;
		end;
...
--- @function get_buildings_close_to_building_from_grid
--- @desc Returns a table of buildings that exist within a given range of a supplied building from the cached building grid. The supplied building itself is not returned as part of this list.
--- @desc An optional filter function may be provided. If supplied, this function is called for each eligible building, with the building passed in as a single argument. The function should return a value that evaluates to <code>true</code> for the building to be added to the returned list. If no filter function is provided then all buildings within range of the position are added to the returned list.
--- @desc Distances between the supplied building and each tested building may be measured in two or three dimensions. By default, distances are measured in two dimensions.
--- @p @battle_building subject building, Subject building to test against.
--- @p @number range, Range from the subject building to test other buildings against. Buildings within this range of the subject are added to the returned list, assuming they either pass the supplied filter function or no filter function is supplied.
--- @p [opt=nil] @function filter, Optional filter function. If supplied, each in-range building is supplied to this function, which must return a value that evaluates to <code>true</code> for that building to be added to the returned list. If no filter function is supplied then all buildings within range of the subject are added.
--- @p [opt=false] @boolean test 3D, Test distances between each candidate building and the subject in three dimensions rather than two dimensions.
--- @r @table table of @battle_building objects within range
function battle_manager:get_buildings_close_to_building_from_grid(subject_building, range, filter, measure_distances_in_three_dimensions)

	if not validate.is_building(subject_building) then
		return false;
	end;
	...
	if filter then
		...
		-- Function declared local above (we have an additional filter passed in)
		function inner_filter(building)
			return building ~= subject_building and filter(building);
		end;
	else
		-- Function declared local above (no additional filter was passed in)
		function inner_filter(building)
			return building ~= subject_building;
		end;
	end;
	
	return self:get_buildings_from_grid(subject_building:position(), range, inner_filter, measure_distances_in_three_dimensions);
end;
