...
----------------------------------------------------------------------------
--- @section Automatic advancement monitor
----------------------------------------------------------------------------

--- @function automatic_nts_advancement_monitor
--- @desc Returns a @boolean result depending on if conditions for advancing the tour are met.
--- @desc When the function is called, it will trigger a watch for wheter any army is routed or shattered or they have reached their hitpoint threshold.
--- @desc This function must always have a corresponding call to @function stop_automatic_nts_advancement_monitor() 
--- @p @navigable_tour the tour that is supposed to use that function.
--- @p @number Unary hitpoints treshold that will trigger next section
--- @p [opt=0] @number an amount of time that the watcher will wait before triggering the next stage 
--- @p [opt=false] @function filter function, Filter function. This should take a @battle_building object as a single argument and return a boolean result.
--- @r @boolean has the conditions for the automatic advancement been met
function st_helper.automatic_nts_advancement_monitor(navigable_tour, percentage_hp_threshold, time_delay)
	if not navigable_tour then
		script_error("ERROR: automatic_nts_advancement_monitor() called but supplied tour [" .. tostring(navigable_tour) .. "] doesn't exist");
		return false;
	end


	if not navigable_tour.sunits_player then
		script_error("ERROR: automatic_nts_advancement_monitor() called but supplied tour [" .. tostring(navigable_tour) .. "] doesn't have player script_units collection");
		return false;
	end

	if not navigable_tour.sunits_enemy then
		script_error("ERROR: automatic_nts_advancement_monitor() called but supplied tour [" .. tostring(navigable_tour) .. "] doesn't have enemy script_units collection");
		return false;
	end


	local sunits_player = navigable_tour.sunits_player;
	local sunits_enemy = navigable_tour.sunits_enemy;
	local army_routing_or_dead = false; 
	local army_under_threshold = false;

	local average_player_hp = sunits_player:unary_hitpoints()
	local average_enemy_hp = sunits_enemy:unary_hitpoints()
	local player_skip_step_threshold = average_player_hp - (average_player_hp / percentage_hp_threshold)
	local enemy_skip_step_threshold = average_enemy_hp - (average_enemy_hp / percentage_hp_threshold)
	if not sunits_player:are_any_routing_or_dead() and not sunits_enemy:are_any_routing_or_dead() then
		bm:watch(
			function()
				army_under_threshold = sunits_player:unary_hitpoints() <= player_skip_step_threshold or sunits_enemy:unary_hitpoints() <= enemy_skip_step_threshold
				return army_under_threshold
			end,
			time_delay,
			function()
				if army_under_threshold and navigable_tour:next_section_exists() then 
				
					navigable_tour:start_next_section();
					st_helper.stop_automatic_nts_advancement_monitor()
				end
			end,
			"advance_tour_on_conditions_met"
		);
	end;
end;

--- @function stop_automatic_nts_advancement_monitor
--- @desc Removes the process triggered by @function automatic_nts_advancement_monitor
function st_helper.stop_automatic_nts_advancement_monitor()
	bm:remove_process("advance_tour_on_conditions_met");
end;
...
function st_helper.capture_location_exists()
	...
	return bm:capture_location_manager():count() > 0;
end;
...
function st_helper.capture_location_with_gate_exists()
	...
	for i = 1, clm:count() do
		local cl = clm:item(i);
		local linked_buildings = clm:item(i):linked_buildings();

		for j = 1, linked_buildings:count() do
			if linked_buildings:item(j):has_gate() then
				return true;
			end;
		end;
	end;
...
function st_helper.capture_location_of_type_exists(capture_location_type, match_partial)
	...
	if match_partial then
		for i = 1, clm:count() do
			local cl_type = clm:item(i):type();

			if string.find(cl_type, capture_location_type) then
				return true;
			end;
		end;
	else
		for i = 1, clm:count() do
			...
			if cl_type == capture_location_type then
				return true;
			end;
		end;
	end;
...
--- @desc Returns the closest capture location of the specified type to the supplied position. If the type is left blank then any capture location matches.
...
--- @p [opt=false] @boolean partial match, Perform a partial string match. This would allow a supplied capture location search string <code>"major_key_building"</code> to match capture locations with types such as <code>"major_key_building_magic"</code> or <code>"major_key_building_missile"</code>.
--- @r @battle_capture_location capture location
function st_helper.get_closest_capture_location_of_type(pos, capture_location_type, match_partial)
	...
	for i = 1, clm:count() do
		...
		if not type_match then
			if match_partial then
				type_match = string.find(cl:type(), capture_location_type);
			else
				type_match = cl:type() == capture_location_type;
			end;
		end;

		if type_match then
			local current_distance = cl:position():distance(pos);
			if current_distance < closest_distance then
				closest_distance = current_distance;
				closest_location = cl;
			end;
...
function st_helper.get_offset_camera_positions_from_sunit(sunit, x_offset, z_offset, vertical_bearing, horizontal_bearing_delta)
	...
	return unit_position, st_helper.get_offset_camera_positions_by_offset_and_bearing(unit_position, x_offset, z_offset, d_to_r(sunit.unit:bearing()), vertical_bearing, horizontal_bearing_delta);
end;
...
function st_helper.get_offset_camera_positions_from_sunits(sunits, x_offset, z_offset, horizontal_bearing, vertical_bearing, horizontal_bearing_delta, relaxed_camera_pose)
	...
	local cam_pos_to_target_distance = math.sqrt((x_offset * x_offset) + (z_offset * z_offset));

	local centre_pos;

	if relaxed_camera_pose then
		centre_pos = v_offset(sunits:centre_point(), 0, math.min(cam_pos_to_target_distance / 6, 30), 0);			-- artificially raise the centre point above the sunits, which works better with the relaxed v bearing
	else
...
--- @desc A filter function may be provided which, if specified, is called with each capture location. The function must return true for the capture location to be considered in the results. If no filter callback is provided then all capture locations are considered eligible.
--- @p [opt=nil] @function filter callback
--- @r @battle_vector camera target
--- @r @battle_vector first camera position
--- @r @battle_vector second camera position
--- @p @battle_capture_location closest capture location
function st_helper.get_offset_camera_positions_from_capture_location(cl_filter)

	if cl_filter and not validate.is_function(cl_filter) then
		return false;
	end;
	
	local clm = bm:capture_location_manager();
	local clm_count = clm:count();
	...
	-- pick the closest capture location to the camera
	local camera_pos = cam:position();

	local closest_cl = false;
	local closest_distance = 5000;
	
	for i = 1, clm_count do
		local cl = clm:item(i);

		if not cl_filter or cl_filter(cl) then
			local current_distance = cl:position():distance(camera_pos);
	...
	if not closest_cl then
		script_error("ERROR: get_offset_camera_positions_from_capture_location() could find no capture locations that match supplied filter");
		return false;
	end;

	local pos_capture_location = closest_cl:position();
	...
	return pos_capture_location, start_cam_pos, end_cam_pos, closest_cl;
end;
...
--- @r @battle_vector first camera position
--- @r @battle_vector second camera position
--- @p @battle_capture_location chosen capture location
function st_helper.get_offset_camera_positions_from_gate_capture_location()
	...
	local closest_cl;
	local closest_cl_gate_building;
	local closest_distance = 5000;
	
	for i = 1, clm_count do
		local cl = clm:item(i);
		local buildings = cl:linked_buildings();
		local cl_gate_building = false;

		for j = 1, buildings:count() do
			if buildings:item(j):has_gate() then
				cl_gate_building = buildings:item(j);
				break;
			end;
		end;

		if cl_gate_building then
			local current_distance = cl:position():distance(camera_pos);
			
			if current_distance < closest_distance then
				closest_distance = current_distance;
				closest_cl = cl;
				closest_cl_gate_building = cl_gate_building;
			end;
		end;
	end;
	...
	local start_cam_pos, end_cam_pos = st_helper.get_offset_camera_positions_by_bearing(
		pos_capture_location, 																					-- cam target vector
		70,																										-- distance
		get_bearing(closest_cl_gate_building:central_position(), pos_capture_location) - (math.pi / 4)			-- bearing, computed as the bearing from the gate building to the capture location but rotated by -45 degrees
	);
...
--- @function get_offset_camera_positions_from_victory_point
--- @desc Returns camera offset positions for a major victory point. The subject victory point is chosen automatically.
--- @r @battle_vector camera target
--- @r @battle_vector first camera position
--- @r @battle_vector second camera position
--- @r @battle_capture_location capture location
function st_helper.get_offset_camera_positions_from_victory_point()

	local victory_points_types = {
		"victory_point_fountain",
		"victory_point_marketplace",
		"victory_point_palace",
		"victory_point_plaza",
		"victory_point_resource",
		"victory_point_smithy",
		"victory_point_temple",
		"victory_point_tower",
	}
	local cl = nil

	for i = 1, #victory_points_types do 
		cl = st_helper.get_closest_capture_location_of_type(cam:position(), victory_points_types[i]);
		if cl then 
			break
		end
	end
	...
	local target_position = cl:position();

	-- return offset positions looking at the target from the direction of the nearest gate
	local cam_start, cam_end = st_helper.get_offset_camera_positions_by_bearing_vector(
		target_position,											-- cam target vector
		cam:position(),												-- from vector for bearing
		90															-- distance
	);

	return target_position, cam_start, cam_end, cl;
end;
...
--- @function get_offset_camera_positions_for_building_list
--- @desc Returns camera offset positions from a list of supplied buildings. Camera positions offset from the closest of the supplied buildings to the current camera position are returned.
--- @p @table building list, List of buildings, supplied as an indexed table.
--- @r @battle_vector camera target
--- @r @battle_vector first camera position
--- @r @battle_vector second camera position
--- @r @battle_building target building
function st_helper.get_offset_camera_positions_for_building_list(building_list)

	if not is_table(building_list) or #building_list == 0 then
		script_error("get_offset_camera_positions_for_building_list() called but supplied building list [" .. tostring(building_list) .. "] is not a populated table");
		return false;
	end;

	-- go through all supplied buildings, get the closest building to the camera
	local camera_pos = cam:position();

	local closest_building;
	local closest_building_distance = 5000;

	for i = 1, #building_list do
		local building = building_list[i];
		local distance = building:central_position():distance_xz(camera_pos);

		if distance < closest_building_distance then
	...
	local pos_building = closest_building:central_position();
	...
	return pos_capture_location, start_cam_pos, end_cam_pos, closest_building;
end;
...
--- @p [opt=6000] @number destroy the windowed video after interval.
function st_helper.navigable_tour_section_battle_factory(section_name, camera_positions_generator, advice_key, infotext, advice_infotext_delay, objective_key, objective_completion_test, leave_objective_after_completion, windowed_movie_path, movie_duration)
	...
	if movie_duration then
		if not is_non_negative_number(movie_duration) then
			script_error("ERROR: navigable_tour_section_battle_factory() called but supplied movie duration [" .. tostring(movie_duration) .. "] is not a positive number");
			return false;
		end;
	else
		movie_duration = nil;
		...
		function()
			...
			if is_function(camera_positions_generator) then
				...
				if cam_start and cam_target then
					cam:move_to(cam_start, cam_target, nts.camera_scroll_time_s, false, -1);

					-- store some values for use later in this section
					nts.cam_start = cam_start;
	...
	local function show_windowed_movie()
		...
		if windowed_movie_path then
			wmp = windowed_movie_player:new_centred("wmp_" .. section_name, windowed_movie_path, 16/9);
			...
			wmp:set_show_close_button(true);
	...
	nts:action(
		function()
			...
			if nts.cam_end and nts.cam_target then
				-- Also check if we have arrived correctly at cam_pos/cam_targ. If we haven't arrived at cam_pos/cam_targ,
				-- the assumption is that the player has moved the camera so we shouldn't attempt any further movements.
				if nts.cam_start:distance(cam:position()) < 1 and nts.cam_target:distance(cam:target()) < 1 then
					cam:move_to(nts.cam_end, nts.cam_target, 30, true);
				else
					bm:out(nts.name .. " is not moving camera to end position as the player has moved the camera");
				end;
			end;
		...
		(nts.camera_scroll_time_s + 0.2) * 1000								-- add a little extra time on to ensure that the camera arrives fully
	);
	...
	if movie_duration then 
		nts:action(
			...
			movie_duration								-- destroy the windowed movie
		);
	end
