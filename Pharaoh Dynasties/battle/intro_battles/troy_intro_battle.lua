...
out("troy_intro_battle.lua loaded");
...
function intro_battle_camera_positions_advice:start()
	...
	bm:callback(
		function() 
			bm:queue_advisor(self.starting_advice, 0, self.is_debug_enabled);
...
intro_battle_selection_advice = {
	...
	end_callback = false,
	end_callback_delay = 1000,
	should_enable_unit_cards = false,
	should_disable_camera = false,
	should_take_control_of_other_units = true,
	marker_position = false,
	second_marker_position = false,
	marker_visible = false,
	enable_orders_on_end = true,
	disable_orders_on_end = false,
	show_advisor_close_button = false,
	is_debug_enabled = false
};


function intro_battle_selection_advice:new(advice_key, cam_pos, cam_targ, selection_sunits, objective_key, end_callback, --[[optional--]]end_callback_delay)
	...
	if is_number(end_callback_delay) then
		sa.end_callback_delay = end_callback_delay
	end
	sa.infotext = {};
...
function intro_battle_selection_advice:set_should_take_control_of_other_units(value)
	if value == false then
		self.should_take_control_of_other_units = false;
	else
		self.should_take_control_of_other_units = true;
	end;
end;
...
function intro_battle_selection_advice:set_disable_orders_on_end(value)
	if value == false then
		self.disable_orders_on_end = false;
	else
		self.disable_orders_on_end = true;
	end;
end;
...
function intro_battle_selection_advice:set_marker_position(pos)
	...
	if self.marker_position == false then
		self.marker_position = pos;
	elseif self.second_marker_position == false then
		self.second_marker_position = pos;
	end
end;
...
function intro_battle_selection_advice:start()
	...
	local cutscene_selection = cutscene:new(
		...
		function()
			...
			if self.selection_sunits ~= self.all_player_sunits and self.should_take_control_of_other_units then
	...
	if self.advice_key then
		local play_advice_func = function()
			bm:stop_advisor_queue();
			bm:clear_infotext();
			bm:queue_advisor(self.advice_key,  0, self.is_debug_enabled);
...
function intro_battle_selection_advice:show_infotext_and_objective()
	...
	local show_marker_func = function()
		...
		if self.second_marker_position then
			self.marker_visible = true;
			bm:add_ping_icon(self.second_marker_position:get_x(), self.second_marker_position:get_y(), self.second_marker_position:get_z(), 14, false);
		end;
	end;
...
function intro_battle_selection_advice:check_all_units_selected()
	...
	if count == self.num_selection_sunits then
		-- clean up
		for i = 1, selection_sunits:count() do
			selection_sunits:item(i).is_selected = nil;
		end;
...
function intro_battle_selection_advice:all_units_selected()
	...
	if self.marker_visible then
		...
		if (self.second_marker_position) then
			bm:remove_ping_icon(self.second_marker_position:get_x(), self.second_marker_position:get_y(), self.second_marker_position:get_z());
		end
	end;
	...
	if self.disable_orders_on_end then
		bm:disable_orders(true);
	elseif self.enable_orders_on_end then
		bm:disable_orders(false);
	end;
	...
	bm:callback(self.end_callback, self.end_callback_delay);
end;
		...
		bm:queue_advisor(
			...
			function()
				...
				local objective_func = function()
					if not self.sunits_are_moving or self.end_on_marker_reached then
...
function intro_battle_movement_advice:attempt_to_complete()
	if self.enemy_contacted or (self.objective_shown and (self.marker_reached or (self.sunits_are_moving and not self.end_on_marker_reached))) then
		self:complete();
	end;
end;
...
intro_battle_routing_advice = {
	...
	is_debug_enabled = false,
	enemy_units_routing_treshold = 0,
	player_units_routing_treshold = 0
};
...
function intro_battle_routing_advice:set_enemy_units_routing_treshold(value)
	if not is_number(value) or value < 0 then
		script_error("ERROR: attempt was made to set enemy units routing treshold but supplied value [" .. tostring(value) .. "] is not a positive number");
		return false;
	end;
	
	self.enemy_units_routing_treshold = value;
end

function intro_battle_routing_advice:set_player_units_routing_treshold(value)
	if not is_number(value) or value < 0 then
		script_error("ERROR: attempt was made to set player units routing treshold but supplied value [" .. tostring(value) .. "] is not a positive number");
		return false;
	end;
	
	self.player_units_routing_treshold = value;
end
...
function intro_battle_routing_advice:start()
	...
	bm:watch(
		function()
			return num_units_routing(sunits_enemy) >= self.enemy_units_routing_treshold
		end,
		2000,
		function()
	...
	bm:watch(
		function()
			return num_units_routing(sunits_player) > self.player_units_routing_treshold
		end,
		0,
		function()
			bm:remove_process("intro_battle_routing_advice");
...
function intro_battle_routing_advice:enemy_sunits_routing()
	...
	cutscene_router:set_close_advisor_on_end(false)
